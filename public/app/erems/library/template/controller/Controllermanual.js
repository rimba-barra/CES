Ext.define('Erems.library.template.controller.Controllermanual', {
    ...
    gridItemDblClickdetail: function (el, record, item, index, e, eOpts) {
        el.up('grid').getSelectionModel().select(index);
    ...
    gridItemContextMenu: function (el, record, item, index, e, eOpts) {
        console.log('gridItemContextMenu ctrl manual')
        e.stopEvent();
        var me = this;
        var menucount = 0;
    ...
    formDataShow: function (el, act, action) {
        console.log('formDataShow CTRL manual')
        var me = this;
            ...
            win = desktop.createWindow({
                ...
                listeners: {
                    boxready: function () {
                        win.body.mask('Loading...');
                        var tm = setTimeout(function () {
                            win.add(Ext.create('Erems.view.' + me.controllerName + '.FormImport'));
                            win.center();
                            win.body.unmask();
                            clearTimeout(tm);
                        }, 1000);
