...
//            console.log("[PURCHLIBERROR] Tidak ada prolibs");
        ...
        if(is_balloon === 1){
            ...
            for(var i=0; i < ball[0].length ;i++){            
                if(ball[0][i]['billingrules_id'] === billing_id){
                    listBalloon[counter] = ball[0][i];
                    counter++;
                    if(ball[0][i]['schedule_type_balloon'] === 5){
                        vUM = true;
                    }                
                    if(ball[0][i]['schedule_type_balloon'] === 3){
                        vIH = true;
                    }
                }
            }
                ...
                if(vUM === true){
                    ...
                    for (var i = 0; i < listBalloon.length; i++) {
                        if(listBalloon[i]['schedule_type_balloon'] === 5){
                            ...
                            terminUM    = listBalloon[i]['term_angsuran'];                            
                            for (var j = 0; j < terminUM; j++) {
                                totalUM += Math.floor(amountUM[i]/terminUM);
                                if(me.uangMuka.times === counterUM){ //tagihan terakhir
                                    tagihanUM = awalUM-totalUM+Math.floor(amountUM[i]/terminUM);
                                }
                                else{
                                    tagihanUM = Math.floor(amountUM[i]/terminUM);                                
                                }
                                s.add({
                                    amount                     : tagihanUM,
                                    termin                     : counterUM,
            ...
            if(vIH === true){
                ...
                for (var p = 0; p < listBalloon.length; p++) {
                    if(listBalloon[p]['schedule_type_balloon'] === 3){
                        ...
                        terminIH    = listBalloon[p]['term_angsuran'];                            
                        for (var j = 0; j < terminIH; j++) {                                
                            totalIH+=Math.floor(amountIH[p]/terminIH);
                            if(me.sisa.times === counterIH){ //tagihan terakhir
                                tagihanIH = me.sisa.amount-totalIH+Math.floor(amountIH[p]/terminIH);
                            }
                            else{
                                tagihanIH = Math.floor(amountIH[p]/terminIH);                                
                            }
                            s.add({
                                termin: counterIH,
                                amount                     : tagihanIH,
            ...
            for (var i = 0; i < me.sisa.times; i++) {
				...
				if(is_ppndtp == 1 && (i === me.sisa.times - 1)){
					pembulatan_sisa = pembulatan_sisa - Math.ceil((totalPPN/nppndtp));
				}
