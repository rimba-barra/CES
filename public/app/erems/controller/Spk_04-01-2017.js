Ext.define('Erems.controller.Spk_04-01-2017', {
    extend: 'Erems.library.template.controller.Controller2',
    alias: 'controller.Spk_04-01-2017',
    ...
    deleteUnitFromGrid: function(row) {
        var me = this;
        var gu = me.getGridu();
        var id = me.tools.intval(gu.getStore().getAt(row).get("spkdetail_id"));
        if (id > 0) {
            me.tools.gridHelper(me.getGrid()).maindetailUpdateDeletedRows(me.getFormdata(), gu.getStore().getAt(row).get("spkdetail_id"));
        }
        ...
        if (me.browseHandler) {
            me.browseHandler.selectItem(function(rec) {
                ...
                if (rec) {
                    ...
                    if (indexExist > -1) {
                        //  me.tools.alert.error("Unit already added in grid");
                        ...
                        if (me.tools.intval(rec.get("eo_spkactive")) >= maxActive) {
                            //  me.tools.alert.error("Max spk active per unit is " + maxActive);
                            me.alertError("Max spk active per unit is " + maxActive);

                        } else if (me.tools.intval(rec.get("eo_spkcount")) >= maxSpkforUnit) {
                            // me.tools.alert.error("Max spk per unit is " + maxSpkforUnit);
