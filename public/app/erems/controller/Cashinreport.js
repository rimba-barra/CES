Ext.define('Erems.controller.Cashinreport', {
    extend: 'Erems.library.template.controller.Controllerreporttb', 
    alias: 'controller.Cashinreport',
    views: ['cashinreport.Panel', 'cashinreport.FormData', 'masterreport.Panel'],
    refs: [
        {
            ref: 'panel',
            selector: 'cashinreportpanel'
        },
        {
            ref: 'formdata',
            selector: 'cashinreportformdata'
        }

    ],
    controllerName: 'cashinreport',
    formWidth: 750,
    fieldName: 'name',
    comboBoxIdEl: [],
    bindPrefixName: 'Cashinreport',
    localStore: {
        detail: null
    },
	project_name: null,
    ...
    init: function(application) {
        ...
        this.control({
            ...
            'cashinreportformdata': {
                afterrender: this.formDataAfterRender
            },
            'cashinreportformdata button[action=save]': {
                click: this.mainDataSave
            },
            'cashinreportformdata button[action=cancel]': {
                click: this.formDataClose
            },
            'cashinreportformdata button[action=process]': {
                click: function() {
                    me.processReport();
                }
            },
			'cashinreportformdata button[action=reset]': {
				click: this.dataReset
            },
			'cashinreportformdata [name=cbf_pricetype_id]': {
                change: me.checkboxChange
            },
			'cashinreportformdata [name=pricetype_id]': {
    ...
    processReport: function() {
        ...
        //if (win) {
			...
			params["pt_id"] = apps.pt;
                        ...
                        var cbf_akad = me.getFormdata().down("[name=cbf_akad]").getValue();
                        if(cbf_akad == '1'){
				params["cbf_akad"] = '1';
			}
			else{
	...
	exportData: function(params){
		...
		Ext.Ajax.request({
			url: 'erems/cashinreport/export/?action=schema',
			params: {
				...
				try{
					...
					if(resp) {
						...
						if(info.success == true){
							...
							Ext.Msg.show({
								title: 'Failure',
								msg: 'Error: Create Report Cash In Failed.',
								icon: Ext.Msg.ERROR,
								buttons: Ext.Msg.OK
							});
						}
    ...
    panelAfterRender: function(el) {	
        ...
        //   me.loadReport(el, 'erems/cashinreport/all');
    },
