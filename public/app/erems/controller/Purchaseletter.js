Ext.define('Erems.controller.Purchaseletter', {
	...
	setParamPL     : {
		...
		plrevision_supervisor               : 0,
		disable_printspt                    : 0,
		sourcemoney_default                 : null,
	...
	init : function (application) {
		...
		this.control({
			...
			'purchaseletterformdata button[action=genschedule]': {
				click: function () {
					...
					if (me.getFormdata().down("[name=is_ppndtp]").getValue() == 1) {
						// added by rico 01/08/2024
						var price_harga_ppntanah      = parseFloat(accounting.unformat(me.getFormdata().down("[name=price_harga_ppntanah]").getValue()));
						var price_harga_ppnbangunan   = parseFloat(accounting.unformat(me.getFormdata().down("[name=price_harga_ppnbangunan]").getValue()));
						var price_harga_ppnsubsidi_dp = parseFloat(accounting.unformat(me.getFormdata().down("[name=price_harga_ppnsubsidi_dp]").getValue()));
						var price_harga_ppninterior   = parseFloat(accounting.unformat(me.getFormdata().down("[name=price_harga_ppninterior]").getValue()));

						var totPPN   = price_harga_ppntanah + price_harga_ppnbangunan + price_harga_ppnsubsidi_dp + price_harga_ppninterior;
					...
					if(me.getFormdata().down("[name=price_source]").getValue() == 2){ /// Add by Erwin.St 03082021
						...
						me.setParamPL.processor.firstplDate   = me.getFormdata().down("[name=firstpurchase_date]").getValue() ? me.getFormdata().down("[name=firstpurchase_date]").getValue() : me.getFormdata().down("[name=purchase_date]").getValue();
			...
			'purchaseletterformdata [name=internalmemo_id]': {
				select : function () {
					...
					_myAppGlobal.getController('Purchaseletterreward').change_im(me.localStore.data_masterimdetail);
				}
			},
		...
		var arF = [
			...
			'price_subsidi_dp',
			'price_harga_interior',
			'price_harga_ppntanah',
			'price_persen_ppntanah',
			'price_harga_ppnbangunan',
			'price_persen_ppnbangunan',
			'price_harga_ppnsubsidi_dp',
			'price_persen_ppnsubsidi_dp',
			'price_harga_ppninterior',
			'price_persen_ppninterior',
			'price_harga_ppnbm',
			'price_persen_ppnbm',
			'price_harga_pph22',
			'price_persen_pph22',
	...
	addNewPPNDTP: function (f, g) {
		...
		var fpd = me.getFormdata().down("[name=firstpurchase_date]").getValue();

		if(new Date(fpd) == 'Invalid Date'){
			var fpd         = fpd.split('-');
			var firstplDate = fpd[2] + '-' + fpd[1] + '-' + fpd[0];
		}
		else{
			var firstplDate = fpd;
		}
		var rencanaStDate = f.down("[name=rencanaserahterima_date]").getValue();
		var nppndtp       = me.setParamPL.processor.persentasePpndtp({pldate : new Date(firstplDate), rencanastdate : new Date(rencanaStDate)});
		...
		if (ppndtp > 0) {
			var amountPPNDTPN = 0;
			if(nppndtp > 0){
				amountPPNDTPN = Math.ceil(ppndtp/nppndtp);
			}

			lastRec.beginEdit();
			lastRec.set({
				remaining_balance : lastRec.get('remaining_balance') - amountPPNDTPN,
				amount            : lastRec.get('amount') - amountPPNDTPN,
			});
			lastRec.endEdit();
	...
	showPrintSPTWindow : function (flagDraft=0) {
		...
		if(flagDraft == 0){
			var template = me.setParamPL.flagprintoutspt ? me.setParamPL.templatePrintmrt : me.setParamPL.templatePrintout;
		}
		else{
				...
				me.insSave({
					...
					finalData : function (data) {
						...
						data["addonparams"] = {
							...
							isEditPersenPPNSubsididp     : me.setParamPL.calculator.isEditPersenPPNSubsididp,
							isEditAmountPPNSubsididp     : me.setParamPL.calculator.isEditAmountPPNSubsididp,
							isEditPersenPPNInterior      : me.setParamPL.calculator.isEditPersenPPNInterior,
							isEditAmountPPNInterior      : me.setParamPL.calculator.isEditAmountPPNInterior,
						...
						data["price_subsidi_dp"]              = accounting.unformat(data["price_subsidi_dp"]);
						data["price_harga_interior"]          = accounting.unformat(data["price_harga_interior"]);
	...
	priceTypeOnSelect : function () {
		...
		if(price_source == 2 && pricelist_id != ''){
			var arrFieldreset = ['price_tanahpermeter', 'price_harga_tanah', 'price_kelebihantanah', 'price_harga_kelebihantanah', 'price_bangunanpermeter', 'price_harga_bangunan', 'price_subsidi_dp', 'price_harga_interior', 'price_harga_jualdasar', 'price_harga_bbnsertifikat', 'price_harga_bphtb', 'price_harga_bajb', 'biaya_asuransi'];
	...
	isSh1Featured : function (params) {
		var me = this;
		if (me.setParamPL.is_sh1 === 1) {
			// params.form.down("[name=promo]").setVisible(true);
		}
	},
	isRewardFeatured : function (params) {
		var me = this, visibleDetail = false, visibleCustomer = false, visibleSales = false, visibleTambahan = false, form = params.form;
		...
		if (Boolean(me.setParamPL.show_reward)) {
			if(me.setParamPL.show_reward === 1){ //// show Detail
				...
				form.down("[name=rewardtambahan_code]").setValue("");
				form.down("[name=rewardsales_code]").setValue("");
				form.down("[name=rewardcustomer_code]").setValue("");

				me.bindStoreComboboxNonModel(me.localStore.data_rewardtambahan, form.down("[name=rewardtambahan_reward_id]"));
				me.bindStoreComboboxNonModel(me.localStore.data_rewardcustomer, form.down("[name=rewardcustomer_reward_id]"));
				me.bindStoreComboboxNonModel(me.localStore.data_rewardsales, form.down("[name=rewardsales_reward_id]"));
			}
		}

		form.down("#detailrewardPanelID").setVisible(visibleDetail);
		form.down("#rewardcustomerPanelID").setVisible(visibleCustomer);
		form.down("#rewardsalesPanelID").setVisible(visibleSales);
		form.down("#rewardtambahanPanelID").setVisible(visibleTambahan);
	},
	fdar : function () {
		var me = this;
		var f  = me.getFormdata();
		...
		var x = {
			init : function (state) {
				...
				cf.fields = {
					...
					_subsidi_dp                 : 'price_subsidi_dp',
					_harga_interior             : 'price_harga_interior',
					...
					_ppn_subsidi_dp             : 'price_persen_ppnsubsidi_dp',
					_tot_ppn_subsidi_dp         : 'price_harga_ppnsubsidi_dp',
					_ppn_interior               : 'price_persen_ppninterior',
					_tot_ppn_interior           : 'price_harga_ppninterior',
				...
				f.el.dom.addEventListener('keyup', function (e) {
				...
				f.el.dom.addEventListener('change', function (e) {
					var el_name = e.srcElement.name;
			...
			create: function (state) {
				...
				me.tools.ajax({
					...
					success : function (data, model) {
						...
						me.setParamPL.processor.detailBalloon      = me.localStore.data_billingrulesballoon;
						...
						for (var i in me.localStore.data_sourcemoney) {
							if (me.tools.intval(me.localStore.data_sourcemoney[i].sourcemoney_id) == me.tools.intval(me.setParamPL.sourcemoney_default)) {
								me.setParamPL.sourceMoneyDefault.id   = me.localStore.data_sourcemoney[i].sourcemoney_id;
								me.setParamPL.sourceMoneyDefault.name = me.localStore.data_sourcemoney[i].sourcemoney;
							}
						}
						...
						var downlines    = me.localStore.data_downline;
						...
						ApliJs.loadHtmlB(me, f.down("#PLCONotherInformationID"), 'sby_form_add', viewParams);
						// console.log(f.down("#PLCONotherInformationID"))

						var viewParams2 = { test : 0 };
						ApliJs.loadHtmlB(me, f.down("#PLCONstatusInformationID"), 'sby_form_add_status', viewParams2);

						me.isSh1Featured({ form: f });
						...
						_myAppGlobal.getController('Purchaseletterreward').generateComboboxIM(me.localStore.data_masterim, f.down('[name=purchase_date]').getValue());
							...
							callback : function (rec, op) {
								...
								me.tools.ajax({
									...
									success : function (schdata, schmodel) {
										...
										me.balanceCalculate(f, me.getSchedulegrid());
										...
										if (me.setParamPL.plrevision_supervisor) {
											f.down("[action=save]").setDisabled(false);
								...
								ApliJs.loadHtmlB(me, f.down("#testModalID"), 'browse_unit_modal', viewParams3);
				...
				if(typeof me.getGrid().getSelectedRecord() != 'undefined'){
					...
					me.tools.ajax({
						...
						success : function (data, model) {
							...
							for (var i in me.setParamPL.templatePrintout) {
								me.setParamPL.templatePrintout[i].value = me.setParamPL.templatePrintout[i].value.replace(/.docx/, "_draft.docx");
							}
	...
	fillFormComponents : function (form) {
        ...
        me.bindStoreComboboxNonModel(me.localStore.data_cac, form.down("[name=cac_cac_id]"));
        me.bindStoreComboboxNonModel(me.localStore.data_bank, form.down("[name=bank_bank_id]"));
        me.bindStoreComboboxNonModel(me.localStore.data_billingrules, form.down("[name=billingrules_billingrules_id]"));
        me.bindStoreComboboxNonModel(me.localStore.data_collector, form.down("[name=collector_employee_id]"));
        me.bindStoreComboboxNonModel(me.localStore.data_mediapromotion, form.down("[name=mediapromotion_mediapromotion_id]"));
        me.bindStoreComboboxNonModel(me.localStore.data_saleslocation, form.down("[name=saleslocation_saleslocation_id]"));
        me.bindStoreComboboxNonModel(me.localStore.data_salesman, form.down("[name=salesman_employee_id]"));
        me.bindStoreComboboxNonModel(me.localStore.data_citraclub, form.down("[name=citraclub_citraclub_id]"));
        me.bindStoreComboboxNonModel(me.localStore.data_employee, form.down("[name=upline_upline_id]"));
        me.bindStoreComboboxNonModel(me.localStore.data_city, form.down("[name=city_city_name]"));
        me.bindStoreComboboxNonModel(me.localStore.data_purposebuy, form.down("[name=purposebuy_purposebuy_id]"));
	},
	...
	apiAci: function () {
		...
		var txt_loading = record.get('api_aci') === 1 ? 'Menonaktifkan ACI' : 'Mengaktifkan ACI';
		var mode_read   = record.get('api_aci') === 1 ? 'apiaci' : 'apiacis';

		panel.setLoading(txt_loading);
		me.tools.ajax({
	...
	mainPanelBeforeRender : function (el) {
		...
		me.tools.ajax({
			params  : {},
			success : function (data, model) {
				var globalParam   = data['others'][0][0]['GLOBALPARAMSPARAMS'];
				var prolibsFile   = data['others'][0][0]['PROLIBFILE'];
				me.reportFileView = data['others'][0][0]['filePaymentScheme']; // added by erwin.st 20042022

				me.setParamPL.globalParams      = globalParam;
				me.setParamPL.useLunasTandaJadi = me.tools.intval(globalParam.PURCHASELETTER_PRINT_LUNASTANDAJADI);
				...
				me.setParamPL.plrevision_supervisor               = data['others'][0][0]['PLREVISION_SUPERVISOR'];
				me.setParamPL.disable_printspt                    = data['others'][0][0]['DISABLE_PRINTSPT'];
				me.setParamPL.sourcemoney_default                 = data['others'][0][0]['SOURCEMONEY_DEFAULT'];
				me.setParamPL.schedule_pembulatan                 = data['others'][0][0]['SCHEDULE_PEMBULATAN'];
				me.setParamPL.issupervisor                        = data['others'][0][0]['ISSUPERVISOR'];
				...
				me.setParamPL.templatePrintout                    = data['others'][0][0]['TEMPLATEPRINTOUT'];
				...
				me.setParamPL.show_reward                         = data['others'][0][0]['SHOW_REWARD'];
				...
				me.setParamPL.use_rumusbiayaprolibspurchaseletter = data['others'][0][0]['USE_RUMUSBIAYAPROLIBSPURCHASELETTER'];
				me.setParamPL.isSH2                               = data['others'][0][0]['isSH2'];
				...
				grid.down('#hargaNettoKomisiGrid').setVisible(me.setParamPL.hargaNettoKomisi == 1 ? true : false);

				/////////////////// LOAD MASTER CAC/MEMBER
				Ext.Ajax.request({
					url     : 'erems/purchaseletter/read',
					params  : { mode_read : 'assetDropdown' },
					success : function (res) {
						var result = Ext.JSON.decode(res.responseText);

						Object.keys(result).forEach(key => {
							var sttripos = key.indexOf("data");
							if(sttripos > -1){
								me.localStore = Object.assign(me.localStore, {[key] : result[key]});
							}
						});
					},
				});
	...
	loadPricelistkoefisien : function(){
		...
		me.calculatorPrice('price_subsidi_dp');
		me.calculatorPrice('price_harga_interior');

		me.calculatorPrice('price_persen_ppnsubsidi_dp');
		me.calculatorPrice('price_persen_ppninterior');
	...
	generateFormsalesgroup : function(){
		...
		var fields = me.localStore.data_salesgroup;
		for (var x in fields){
	...
	change_purchase_date : function(){
		...
		if(form.up('window').state == 'create'){
			...
			_myAppGlobal.getController('Purchaseletterreward').generateComboboxIM(me.localStore.data_masterim, form.down('[name=purchase_date]').getValue());
		}
	},
