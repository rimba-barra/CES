Ext.define('Erems.controller.Aftersalespdamkwhreport', {
    extend: 'Erems.library.template.controller.Controllerreporttb', 
    alias: 'controller.Aftersalespdamkwhreport',
    views: ['aftersalespdamkwhreport.Panel', 'aftersalespdamkwhreport.FormData', 'masterreport.Panel'],
	stores: ['Mastercluster','Mastertype','Unitstatus'],
    models: ['Mastercluster','Mastertype'],
    refs: [
        {
            ref: 'panel',
            selector: 'aftersalespdamkwhreportpanel'
        },
        {
            ref: 'formdata',
            selector: 'aftersalespdamkwhreportformdata'
        }

    ],
    controllerName: 'aftersalespdamkwhreport',
    formWidth: 750,
    fieldName: 'name',
    comboBoxIdEl: [],
    bindPrefixName: 'Aftersalespdamkwhreport',
    localStore: {
        detail: null
    },
	project_name: null,
    ...
    init: function(application) {
        var me = this;
        this.control({
            'aftersalespdamkwhreportpanel': {
                beforerender: me.mainPanelBeforeRender,
                afterrender: this.panelAfterRender

            },
            'aftersalespdamkwhreportformdata': {
                afterrender: this.formDataAfterRender
            },
            'aftersalespdamkwhreportformdata button[action=save]': {
                click: this.mainDataSave
            },
            'aftersalespdamkwhreportformdata button[action=cancel]': {
                click: this.formDataClose
            },
            'aftersalespdamkwhreportformdata button[action=process]': {
                click: function() {
                    me.processReport();
                }
            },
			'aftersalespdamkwhreportformdata button[action=reset]': {
				click: this.dataReset
            },
			'aftersalespdamkwhreportformdata [name=cbf_cluster_id]': {
                change: me.checkboxChange
            },
			'aftersalespdamkwhreportformdata [name=cluster_id]': {
                select: me.comboboxChange
            },
			'aftersalespdamkwhreportformdata [name=cbf_type_id]': {
                change: me.checkboxChange
            },
			'aftersalespdamkwhreportformdata [name=type_id]': {
                select: me.comboboxChange
            },
			'aftersalespdamkwhreportformdata [name=cbf_unitstatus_id]': {
                change: me.checkboxChange
            },
			'aftersalespdamkwhreportformdata [name=unitstatus_id]': {
                select: me.comboboxChange
            },
			'aftersalespdamkwhreportformdata [name=cbf_statuslunas]': {
                change: me.checkboxChange
            },
			'aftersalespdamkwhreportformdata [name=lunasstatus_id]': {
                select: me.comboboxChange
            },
        });
    },
    ...
    processReport: function() {
        ...
        if (win) {
			...
			//var reportFile = "Aftersalespdamkwh";
			var reportFile = null;
			var select_laporan = me.getFormdata().down("[name=radiogroup_laporantype]").getValue().radio_laporantype;
			if(select_laporan == "pdam"){
       			reportFile = 'Aftersalespdam';
			} else if(select_laporan == "kwh") {
				reportFile = 'Aftersaleskwh';
			}
    ...
    panelAfterRender: function(el) {	
        ...
        Ext.Ajax.request({
            url: 'erems/aftersalespdamkwhreport/read',
        ...
        //   me.loadReport(el, 'erems/aftersalespdamkwhreport/all');
    }
