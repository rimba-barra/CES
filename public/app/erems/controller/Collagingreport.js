Ext.define('Erems.controller.Collagingreport', {
	extend: 'Erems.library.template.controller.Controllerreporttb',
	requires: 'Erems.library.template.component.Clustercombobox',
	alias: 'controller.Collagingreport',
	views: ['collagingreport.Panel', 'collagingreport.FormData', 'masterreport.Panel'],
	stores: ['Sourcemoney', 'Masterdata.store.Projectpt','Mastercluster'],
	models: ['Sourcemoney', 'Masterdata.model.Projectpt'],
	refs: [
		{
			ref: 'panel',
			selector: 'collagingreportpanel'
		},
		{
			ref: 'formdata',
			selector: 'collagingreportformdata'
		}

	],
	controllerName: 'collagingreport',
	formWidth: 750,
	fieldName: 'name',
	comboBoxIdEl: [],
	bindPrefixName: 'Collagingreport',
	localStore: {
		detail: null
	},
	project_name: null,
	...
	init: function (application) {
		var me = this;
		this.control({
			'collagingreportpanel': {
				beforerender: me.mainPanelBeforeRender,
				afterrender: this.panelAfterRender

			},
			'collagingreportformdata': {
				afterrender: this.formDataAfterRender
			},
			'collagingreportformdata button[action=save]': {
				click: this.mainDataSave
			},
			'collagingreportformdata button[action=cancel]': {
				click: this.formDataClose
			},
			'collagingreportformdata button[action=process]': {
				click: function () {
			...
			'collagingreportformdata button[action=reset]': {
				click: this.dataReset
			},
			'collagingreportformdata [name=cbf_buildingclass]': {
				change: me.checkboxChange
			},
			'collagingreportformdata [name=buildingclass]': {
				select: me.comboboxChange
			},
			'collagingreportformdata [name=cbf_sourcemoney_id]': {
				change: me.checkboxChange
			},
			'collagingreportformdata [name=sourcemoney_id]': {
				select: me.comboboxChange
			},
			'collagingreportformdata [name=cbf_pt_id]': {
				change: me.checkboxChange
			},
			'collagingreportformdata [name=pt_id]': {
				select: me.comboboxChange
			},
			'collagingreportformdata [name=cbf_cluster_id]': {
				change: me.checkboxChange
			},
			'collagingreportformdata [name=cluster_id]': {
				select: me.comboboxChange
			},
		});
	},
	setData: function () {
		...
		values["cluster_id"] = form.down("[name=cluster_id]").getValue();
		var cbf_checked3 = form.down("[name=cbf_cluster_id]").getValue();
		if (cbf_checked3 == '1' || !values["cluster_id"]) {
			values["cluster_display"] = 'ALL';
		} else {
			values["cluster_display"] = form.down("[name=cluster_id]").getRawValue();
		}
	...
	panelAfterRender: function (el) {
		...
		//   me.loadReport(el, 'erems/collagingreport/all');
