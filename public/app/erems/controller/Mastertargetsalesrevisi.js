Ext.define('Erems.controller.Mastertargetsalesrevisi', {
	extend: 'Main.library.Controller',
	alias: 'controller.Mastertargetsalesrevisi',
	models: ['Mastertargetsalesrevisi'],
	stores: ['Mastertargetsalesrevisi'],
	views: [
		'mastertargetsalesrevisi.Panel',
		'mastertargetsalesrevisi.FormSearch',
		'mastertargetsalesrevisi.Grid',
		'mastertargetsalesrevisi.FormData'
	],
	listPurpose: [],
	listProject: [],
	...
	init: function (application) {
		...
		me.control({
			'#btnGenerateTargetSalesrevisi': {
				click: function () {
					var projectName = '';
					if (me.listProject.length > 0)
						var projectName = '<b>' + me.getMainGrid().down('[name=radiogroup_view_grid]').getChecked()[0].boxLabel + '</b><br/>';
					Ext.MessageBox.prompt('Tahun', projectName + 'Generate untuk Tahun :', generateTargetSalesrevisi);
				}
			},
		});

		function generateTargetSalesrevisi(btn, text) {
			var postdata = new Object();
			if (btn == 'ok') {
				if (Math.floor(text) == text && $.isNumeric(text) && text > 1945 && text < 2100) {
					postdata.tahun = text;
					postdata.view_grid_param = me.getMainFormSearch().getForm().findField('view_grid_param').getSubmitValue();
					Ext.Ajax.request({
						url: 'erems/mastertargetsalesrevisi/read',
						params: {
...
//
						if (info.listProject.success != true) {
...
//				});
	...
	dataExport: function (el, tahun, view_grid_param) {
		...
		Ext.Ajax.request({
			url: 'erems/mastertargetsalesrevisi/export',
			params: {
				tahun: tahun,
