Ext.define('Erems.controller.Legalstatusakadreport', {
    extend: 'Erems.library.template.controller.Controllerreporttb', 
    alias: 'controller.Legalstatusakadreport',
    views: ['legalstatusakadreport.Panel', 'legalstatusakadreport.FormData', 'masterreport.Panel'],
    requires:[
    ...
    refs: [
        {
            ref: 'panel',
            selector: 'legalstatusakadreportpanel'
        },
        {
            ref: 'formdata',
            selector: 'legalstatusakadreportformdata'
        }

    ],
    controllerName: 'legalstatusakadreport',
    formWidth: 750,
    fieldName: 'name',
    comboBoxIdEl: [],
    bindPrefixName: 'Legalstatusakadreport',
    localStore: {
        detail: null
    },
	project_name: null,
    ...
    init: function(application) {
        var me = this;
        this.control({
            'legalstatusakadreportpanel': {
                beforerender: me.mainPanelBeforeRender,
                afterrender: this.panelAfterRender

            },
            'legalstatusakadreportformdata': {
                afterrender: this.formDataAfterRender
            },
            'legalstatusakadreportformdata button[action=save]': {
                click: this.mainDataSave
            },
            'legalstatusakadreportformdata button[action=cancel]': {
                click: this.formDataClose
            },
            'legalstatusakadreportformdata button[action=process]': {
                click: function() {
                    me.processReport();
                }
            },
			'legalstatusakadreportformdata button[action=reset]': {
				click: this.dataReset
            },
			'legalstatusakadreportformdata [name=cbf_buildingclass]': {
                change: me.checkboxChange
            },
			'legalstatusakadreportformdata [name=buildingclass]': {
                select: me.comboboxChange
            },
			'legalstatusakadreportformdata [name=cbf_cluster_id]': {
                change: me.checkboxChange
            },
			'legalstatusakadreportformdata [name=cluster_id]': {
                select: me.comboboxChange
            },
			'legalstatusakadreportformdata [name=cbf_pricetype_id]': {
                change: me.checkboxChange
            },
			'legalstatusakadreportformdata [name=pricetype_id]': {
                select: me.comboboxChange
            }
        });
    },
    ...
    processReport: function() {
        ...
        if (win) {
			...
			var reportFile = "Legalstatusakad";
    ...
    panelAfterRender: function(el) {	
        ...
        Ext.Ajax.request({
            url: 'erems/legalstatusakadreport/read',
        ...
        //   me.loadReport(el, 'erems/legalstatusakadreport/all');
    }
