...
//        }).read('checkrealisasicashier');
    ...
    printoutdoc: function () {
        ...
        if (!me.validChangeName) {
            me.tools.alert.warning("Biaya Admin Ganti Nama Belum Lunas.");
            return;
        }
                ...
                $.ajax({
                    ...
                    for (var i in units) {
                        rows += "<tr unit_id='" + units[i]["unit"]["unit_id"] + "'>" +
                                ...
                                "<td style='width:70px;'><button class='btn btn-primary btn-sm select_unit' unit_id='" + units[i]["unit"]["unit_id"] + "'>select</button></td>" +
                                "</tr>";
                        count++;
                    }
