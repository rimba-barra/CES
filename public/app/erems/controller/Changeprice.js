Ext.define('Erems.controller.Changeprice', {
					...
					me.tools.ajax({
						...
						success : function (schdata, schmodel) {
							...
							if (tigaSekawanAnCancel.length === 0) {
								...
								f.down("[name=firstpurchase_date]").setValue(psRec.get("firstpurchase_date"));
	...
	addNewPPNDTP : function(){
		var me 			= this,
			...
			s       	= g.getStore(),
			totalRecord = s.getCount(),
			lastRec     = s.getAt(totalRecord - 1);
		...
		var purchase_date = f.down("[name=purchaseletter_purchase_date]").getValue();
		...
		var fpd = f.down("[name=firstpurchase_date]").getValue();
		...
		var rencanaStDate = f.down("[name=rencana_serahterima_date]").getValue();
		var nppndtp       = me.setParamCP.processor.persentasePpndtp({pldate : new Date(firstplDate), rencanastdate : new Date(rencanaStDate)});
		...
		if (ppndtp > 0) {
			...
			me.scheduleGrid.addNewSchedule(me, {
				ppndtp 				: {duedate : newDate, amount : amountPPNDTPN},
