Ext.define('Erems.controller.Purchaseletter', {
	...
	reportFileView : null,
	setParamPL     : {
		use_rumusbiayaprolibspurchaseletter : false,
		show_reward                         : 0,
		issupervisor                        : false,
		schedule_pembulatan                 : 0,
		sourcemoney_default                 : 0,
	...
	init : function (application) {
		...
		this.control({
			...
			'purchaseletterformdata [name=internalmemo_id]': {
				select : function () {
					_myAppGlobal.getController('Purchaseletterreward').change_im(me.localStore.mImdetail);
				}
			},
								...
								me.tools.ajax({
									...
									success : function (avdata, avmodel) {
										...
										me.discountInput().enable({
											globalparams : me.setParamPL.globalparams,
	...
	isRewardFeatured : function (params) {
		...
		if (Boolean(me.setParamPL.show_reward)) {
			if(me.setParamPL.show_reward === 1){ //// show Detail
				...
				me.tools.wesea(me.localStore.mRtambahan, params.form.down("[name=rewardtambahan_reward_id]")).comboBox();
				me.tools.wesea(me.localStore.mRcustomer, params.form.down("[name=rewardcustomer_reward_id]")).comboBox();
				me.tools.wesea(me.localStore.mRsales, params.form.down("[name=rewardsales_reward_id]")).comboBox();
			}
		}
	...
	fdar : function () {
		...
		if(f.up('window').state != 'create'){
			recMain     = grid.getSelectedRecord();
			plId        = recMain.get('purchaseletter_id');
		...
		var x = {
			init : function (state) {
				...
				me.setParamPL.processor.setForm(f);
				me.setParamPL.processor.setC(me);
				me.setParamPL.processor.setScheduleGrid(sg);
				...
				//// other information and status information
				ApliJs.loadHtml(me, me.getFormdata().down("#PLCONotherInformationID"), 'sby_form_add');
				ApliJs.loadHtml(me, me.getFormdata().down("#PLCONstatusInformationID"), 'sby_form_add_status');

				me.fillFormComponents(f);
			...
			create: function (state) {
				...
				me.tools.ajax({
					...
					success : function (data, model) {
						...
						me.localStore.mJenisbiaya = typeof(data['others'][0][0]['JENIS_BIAYA_PURCHASELETTER'][0]) != 'undefined' ? data['others'][0][0]['JENIS_BIAYA_PURCHASELETTER'][0] : [];
						...
						me.setParamPL.calculator.isGenerateValue   = me.setParamPL.use_rumusbiayaprolibspurchaseletter;
						me.setParamPL.processor.detailBalloon      = data['others'][1];
						me.setParamPL.processor.schedulePembulatan = me.setParamPL.schedule_pembulatan;

						/// get source money default text
						for (var i in me.localStore.mSourcemoney.data) {
							if (me.tools.intval(me.localStore.mSourcemoney.data[i].sourcemoney_id) == me.tools.intval(me.setParamPL.sourcemoney_default)) {
								me.setParamPL.sourceMoneyDefault.id   = me.localStore.mSourcemoney.data[i].sourcemoney_id;
								me.setParamPL.sourceMoneyDefault.name = me.localStore.mSourcemoney.data[i].sourcemoney;
							}
						}
						...
						////// combobox master im
						_myAppGlobal.getController('Purchaseletterreward').generateComboboxIM(me.localStore.mIm, f.down('[name=purchase_date]').getValue());
					...
					success : function (data, model) {
						...
						me.localStore.detail.load({
							...
							callback : function (rec, op) {
								...
								me.hitungPorsikepemilikancustomer();
								...
								var paymentAmount = me.tools.floatval(rec.get("payment_payment"));
								...
								me.tools.ajax({
									...
									success : function (schdata, schmodel) {
										...
										if (me.getTotalPayment() > 0) {
											if (!me.setParamPL.globisAuthorizedUser) {
										...
										if (me.setParamPL.disable_printspt) {
											f.down("[action=printout]").setDisabled(true);
										}

										if (me.setParamPL.isSupervisor) {
										...
										sg.addListener('beforeedit', function () {
											return false;
										});
								...
								if(rec.get("price_source") == 2){
									...
									sg.down('#colms_persentase_npv').setVisible(true);
								}
								...
								me.generateJenisbiaya();
								me.setValueotherinformationstatus('update', rec);
							}
						});
					...
					success : function (data, model) {
						...
						me.localStore.detail.load({
							...
							callback : function (rec, op) {
								...
								ApliJs.loadHtmlB(me, me.getFormdata().down("#testModalID"), 'browse_unit_modal', {});

								////// combobox master im
								_myAppGlobal.getController('Purchaseletterreward').generateComboboxIM(me.localStore.mIm, f.down('[name=purchase_date]').getValue());

								me.setValueotherinformationstatus('updateDraft', rec);
	...
	fillFormComponents : function (form) {
		var me = this;

		me.tools.wesea(me.localStore.mBank, form.down("[name=bank_bank_id]")).comboBox();
		me.tools.wesea(me.localStore.mBillingrules, form.down("[name=billingrules_billingrules_id]")).comboBox('', function () {
		...
		me.tools.wesea(me.localStore.mCollector, form.down("[name=collector_employee_id]")).comboBox();
		me.tools.wesea(me.localStore.mMediapromotion, form.down("[name=mediapromotion_mediapromotion_id]")).comboBox();
		me.tools.wesea(me.localStore.mSaleslocation, form.down("[name=saleslocation_saleslocation_id]")).comboBox();
		me.tools.wesea(me.localStore.mSalesman, form.down("[name=salesman_employee_id]")).comboBox();
		me.tools.wesea(me.localStore.mCitraclub, form.down("[name=citraclub_citraclub_id]")).comboBox();
		me.tools.wesea(me.localStore.mEmployee, form.down("[name=upline_upline_id]")).comboBox();
		me.tools.wesea(me.localStore.mCac, form.down("[name=cac_cac_id]")).comboBox();
		me.tools.wesea(me.localStore.mCity, form.down("[name=city_city_name]")).comboBox();
		me.tools.wesea(me.localStore.mPurposebuy, form.down("[name=purposebuy_purposebuy_id]")).comboBox();
	},
	...
	apliJsFuncsby_form_add : function () {
		...
		var x = {
			afterRender: function (tpl, params) {
				...
				var downlines    = me.localStore.mDownline;
				for (var i in downlines) {
					downlineList += "<option value='" + downlines[i]["downline_id"] + "'>" + downlines[i]["name"] + "</option>";
				}
				Ext.query("#plFormID select[name='downline_id']")[0].innerHTML = downlineList
	...
	mainPanelBeforeRender : function (el) {
		...
		me.tools.ajax({
			params  : {},
			success : function (data, model) {
				var dataOthers 	= data['others'][0][0],
					prolibsFile = dataOthers.PROLIBFILE;
				
				me.setParamPL.calculatorJs                        = dataOthers.CALCULATORJS;
				me.setParamPL.purchaseletterJs                    = dataOthers.PURCHASELETTERJS;
				me.setParamPL.isRSCHApproveUser                   = dataOthers.RESCHEDULEAPPROVEUSER;
				me.setParamPL.groupuser                           = dataOthers.GROUPUSER;
				me.setParamPL.param_generate_notes                = dataOthers.PARAM_GENERATE_NOTES;
				me.setParamPL.checkDataCustomer                   = dataOthers.checkDataCustomer == 1 ? true : false;
				me.setParamPL.checkCanSPTDraft                    = dataOthers.checkCanSPTDraft;
				me.setParamPL.userkpraccdate                      = dataOthers.userkpraccdate;
				me.setParamPL.surveyConfig                        = dataOthers.surveyConfig;
				me.setParamPL.verification_approval               = dataOthers.verification_approval;
				me.setParamPL.visible_insentif_pajak              = dataOthers.visibleInsentifPajak == 1 ? true : false;
				me.setParamPL.visibleTahanBatal                   = dataOthers.visibleTahanBatal;
				me.setParamPL.getPurcheletterSendWaActive         = dataOthers.getPurcheletterSendWa.active;
				me.setParamPL.getPurcheletterSendWaPhone          = dataOthers.getPurcheletterSendWa.phone;
				me.setParamPL.getPurcheletterSendWaText           = dataOthers.getPurcheletterSendWaText;
				me.setParamPL.printoutsptmrt                      = dataOthers.printoutsptmrt;
				me.setParamPL.visible_vida                        = dataOthers.visibleVida == 1 ? true : false;
				me.setParamPL.visible_fest40                      = dataOthers.visibleFest40 == 1 ? true : false;
				me.setParamPL.activePricesource                   = dataOthers.activePricesource;
				me.setParamPL.typeCalculaterounding               = dataOthers.typeCalculaterounding;
				me.setParamPL.visible_extend_schedule             = dataOthers.visibleExtendSchedule == 1 ? true : false;/// Added Rico 07122021
				me.setParamPL.getPurcheletterSurveyOnlineText     = dataOthers.getPurcheletterSurveyOnline.text; // added by rico 17012022
				me.setParamPL.getPurcheletterSurveyOnlineLink     = dataOthers.getPurcheletterSurveyOnline.link; // added by rico 17012022
				me.setParamPL.rencanaST_enddate                   = dataOthers.rencanaST_enddate; // added by rico 17012022
				me.setParamPL.hargaNettoKomisi                    = dataOthers.hargaNettoKomisi; // added by rico 09032022
				me.setParamPL.ShowMoreCustomerOnGrid              = dataOthers.ShowMoreCustomerOnGrid; // added by erwin.st 27072022
				me.setParamPL.ppn_value                           = dataOthers.ppn_value; // added by erwin.st 27072022
				me.setParamPL.visible_blokir                      = dataOthers.visibleBlokir == 1 ? true : false; // added by rico 28112022
				me.setParamPL.visible_disc_karyawan               = dataOthers.visibleDiscKaryawan == 1 ? true : false; // added by rico 19012023
				me.setParamPL.visible_promo                       = dataOthers.showPromoPurchaseletter == 1 ? true : false; // added by rico 19012023
				me.setParamPL.useJenisBiayaPurchaseletter         = dataOthers.useJenisBiayaPurchaseletter;
				me.setParamPL.purchaseletterRencanaSerahTerimaNew = dataOthers.purchaseletterRencanaSerahTerimaNew;
				me.setParamPL.showKuasaCustomerPurchaseletter     = dataOthers.showKuasaCustomerPurchaseletter;
				me.setParamPL.disablePromoPurchaseletter          = dataOthers.disablePromoPurchaseletter;
				me.reportFileView                                 = dataOthers.filePaymentScheme; // added by erwin.st 20042022

				me.setParamPL.globisAuthorizedUser                = dataOthers.ISAUTHORIZEDUSER;
				me.setParamPL.schedule_pembulatan                 = dataOthers.SCHEDULE_PEMBULATAN;
				me.setParamPL.globalparams                        = dataOthers.GLOBALPARAMSPARAMS;
				me.setParamPL.useLunasTandaJadi                   = me.tools.intval(me.setParamPL.globalparams.PURCHASELETTER_PRINT_LUNASTANDAJADI);
				me.setParamPL.pl_revision                         = dataOthers.PLREVISION_SUPERVISOR;
				me.setParamPL.disable_printspt                    = dataOthers.DISABLE_PRINTSPT;
				me.setParamPL.isSupervisor                        = dataOthers.ISSUPERVISOR;
				me.setParamPL.sourcemoney_default                 = dataOthers.SOURCEMONEY_DEFAULT;
				me.setParamPL.templatePrintSPTDraft               = dataOthers.TEMPLATEPRINTOUTSPTDRAFT;
				me.setParamPL.templatePrint                       = dataOthers.TEMPLATEPRINTOUT;
				me.setParamPL.templatePrintmrt                    = dataOthers.TEMPLATEPRINTOUTMRT;
				me.setParamPL.templatePrintPayScheme              = dataOthers.TEMPLATEPRINTOUTPAYSCHEME;
				me.setParamPL.show_reward                         = dataOthers.SHOW_REWARD;
				me.setParamPL.approveNowRsch                      = dataOthers.APPROVENOW_RSCH;
				me.setParamPL.isFlashPrint                        = dataOthers.IS_FLASHPRINT;
				me.setParamPL.isPurchaseprintktsim                = dataOthers.IS_PURCHASEPRINTKTPSIM;
				me.setParamPL.use_rumusbiayaprolibspurchaseletter = dataOthers.USE_RUMUSBIAYAPROLIBSPURCHASELETTER;

				// me.setParamPL.filePaymentScheme               = dataOthers['filePaymentScheme']; // added by erwin.st 20042022
				...
				me.tools.ajax({
					params  : {},
					success : function (data, model) {
						me.localStore.mBank           = typeof data.bank != 'undefined' ? data.bank : null;
						me.localStore.mBillingrules   = typeof data.billingrules != 'undefined' ? data.billingrules : null;
						me.localStore.mCac            = typeof data.cac != 'undefined' ? data.cac : null;
						me.localStore.mCitraclub      = typeof data.citraclub != 'undefined' ? data.citraclub : null;
						me.localStore.mCollector      = typeof data.collector != 'undefined' ? data.collector : null;
						me.localStore.mEmployee       = typeof data.employee != 'undefined' ? data.employee : null;
						me.localStore.mMediapromotion = typeof data.mediapromotion != 'undefined' ? data.mediapromotion : null;
						me.localStore.mPurposebuy     = typeof data.purposebuy != 'undefined' ? data.purposebuy : null;
						me.localStore.mRcustomer      = typeof data.rewardcustomer != 'undefined' ? data.rewardcustomer : null;
						me.localStore.mRsales         = typeof data.rewardsales != 'undefined' ? data.rewardsales : null;
						me.localStore.mRtambahan      = typeof data.rewardtambahan != 'undefined' ? data.rewardtambahan : null;
						me.localStore.mSalesgroup     = typeof data.salesgroup.data != 'undefined' ? data.salesgroup.data : [];
						me.localStore.mSaleslocation  = typeof data.saleslocation != 'undefined' ? data.saleslocation : null;
						me.localStore.mSalesman       = typeof data.salesman != 'undefined' ? data.salesman : null;
						me.localStore.mSourcemoney    = typeof data.sourcemoney != 'undefined' ? data.sourcemoney : null;

						me.localStore.mDownline = typeof data.others[0][0]['DOWNLINE'][1] != 'undefined' ? data.others[0][0]['DOWNLINE'][1] : [];
						me.localStore.mIm       = data.others[0][0].MASTER_IM.success ? data.others[0][0].MASTER_IM.data : [];
						me.localStore.mImdetail = data.others[0][0].MASTER_IM_DETAIL.success ? data.others[0][0].MASTER_IM_DETAIL.data : [];
					}
				}).read('allmaster');
			}
	...
	generateFormsalesgroup : function(){
		...
		var fields = me.localStore.mSalesgroup;	
		for (var x in fields){
	...
	change_purchase_date : function(){
		...
		if(form.up('window').state == 'create'){
			...
			_myAppGlobal.getController('Purchaseletterreward').generateComboboxIM(me.localStore.mIm, form.down('[name=purchase_date]').getValue());
		}
	},
    ...
    setValueotherinformationstatus : function(act, rec){
    	var me = this;

    	Ext.query("#plFormID select[name='downline_id']")[0].value = rec.get("downline_id");
		Ext.query("#plFormID [name='keterangan_bayar']")[0].value  = rec.get("keterangan_bayar");
		Ext.query("#plFormID [name='keterangan_1']")[0].value      = rec.get("keterangan_1");
		Ext.query("#plFormID [name='keterangan_2']")[0].value 	   = rec.get("keterangan_2");
		Ext.query("#plFormID [name='keterangan_3']")[0].value 	   = rec.get("keterangan_3");
		Ext.query("#plFormID [name='house_advisor']")[0].value     = rec.get("house_advisor");
		Ext.query("#plFormID [name='manager']")[0].value 		   = rec.get("manager");
		Ext.query("#plFormID [name='hs_keuangan']")[0].value 	   = rec.get("hs_keuangan");
		Ext.query("#plFormID [name='action']")[0].value 	       = act;

		Ext.query("#PLCONstatusInformationID [name='aftersales_serahterima_date']")[0].value = rec.get("aftersales_serahterima_date") ? moment(rec.get("aftersales_serahterima_date")).format("DD-MM-YYYY") : '';
		Ext.query("#PLCONstatusInformationID [name='buktipemilik_imb_date']")[0].value 	  	 = rec.get("buktipemilik_imb_date") ? moment(rec.get("buktipemilik_imb_date")).format("DD-MM-YYYY") : '';
		Ext.query("#PLCONstatusInformationID [name='total_ajb']")[0].value 	       		  	 = rec.get("total_ajb");
    },
    hitungPorsikepemilikancustomer : function(){
    	var me = this, f = me.getFormdata();
