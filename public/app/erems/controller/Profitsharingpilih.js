Ext.define('Erems.controller.Profitsharingpilih', {
//	extend: 'Erems.library.template.controller.Controller',
	extend: 'Erems.library.template.controller.Controlleralt',
	alias: 'controller.Profitsharingpilih',
	requires: [
		'Erems.library.template.component.Clustercombobox',
		'Erems.library.template.component.Profitsharingcombobox',
	],
	views: ['profitsharingpilih.Panel', 'profitsharingpilih.Grid', 'profitsharingpilih.GridDetail', 'profitsharingpilih.GridLookup', 'profitsharingpilih.FormSearch', 'profitsharingpilih.FormData', 'profitsharingpilih.FormDataView'],
	stores: ['Profitsharingpilih', 'Masterparameterglobal', 'Mastercluster', 'Masterblock', 'Masterprofitsharing', 'Profitsharingpilihpurchaseletterdetail', 'Profitsharingpilihdetail'],
	models: ['Profitsharingpilih', 'Masterparameterglobal', 'Masterblock', 'Masterprofitsharing', 'Profitsharingpilihpurchaseletterdetail', 'Profitsharingpilihdetail'],
	refs: [
		{
			ref: 'grid',
			selector: 'profitsharingpilihgrid'
		},
		{
			ref: 'formsearch',
			selector: 'profitsharingpilihformsearch'
		},
		{
			ref: 'formdata',
			selector: 'profitsharingpilihformdata'
		},
		{
			ref: 'formdataview',
			selector: 'profitsharingpilihformdataview'
		},
		{
			ref: 'formdatadetail',
			selector: 'profitsharingpilihformdatadetail'
		},
		{
			ref: 'griddetail',
			selector: 'profitsharingpilihgriddetail'
		},
		{
			ref: 'gridlookup',
			selector: 'profitsharingpilihgridlookup'
		}
	],
	controllerName: 'profitsharingpilih',
	fieldName: 'unit_number',
	bindPrefixName: 'Profitsharingpilih',
	formWidth: 780,
	init: function (application) {
		var me = this;
		this.control({
			'profitsharingpilihpanel': {
				beforerender: me.mainPanelBeforeRender,
				afterrender: this.panelAfterRender

			},
			'profitsharingpilihgrid': {
				afterrender: this.gridAfterRender,
////				itemdblclick: this.gridItemDblClick,
				itemcontextmenu: this.gridItemContextMenu,
				selectionchange: this.gridSelectionChange
			},
			'profitsharingpilihgrid toolbar button[action=update]': {
				click: function () {
					this.formDataShow('update');
				}
			},
			'profitsharingpilihgrid toolbar button[action=viewLookup]': {
				click: function () {
					me.formDataShowView('view');
				}
			},
			'profitsharingpilihgrid toolbar [action=action0]': {
				click: function () {
					me.dataSearchFilterby('');
				}
			},
			'profitsharingpilihgrid toolbar [action=action1]': {
				click: function () {
					me.dataSearchFilterby('1');
				}
			},

			'profitsharingpilihgrid toolbar [action=action2]': {
				click: function () {
					me.dataSearchFilterby('2');
				}
			},
			'profitsharingpilihformsearch': {
				afterrender: this.formSearchAfterRender
			},
			'profitsharingpilihformsearch button[action=search]': {
				click: this.dataSearch
			},
			'profitsharingpilihformsearch button[action=reset]': {
				click: this.dataReset
			},
			'profitsharingpilihgridlookup': {
//				afterrender: this.gridAfterRender,
////				itemdblclick: this.gridItemDblClick,
				itemcontextmenu: this.gridItemContextMenu,
				selectionchange: this.gridLookupSelectionChange
			},
			'profitsharingpilihformdataview': {
//				beforerender: this.formDataBeforeRender,
				afterrender: this.formDataViewAfterRender
			},
			'profitsharingpilihformdataview button[action=print]': {
				click: me.processPrint
			},
			'profitsharingpilihformdata': {
//				beforerender: this.formDataBeforeRender,
				afterrender: this.formDataAfterRender
			},
			'profitsharingpilihformdata [name=cb_set_cluster]': {
				change: me.checkboxSetCluster
			},
			'profitsharingpilihformdata [name=profitsharing_id]': {
				change: me.profitSharing
			},
			'profitsharingpilihformdata button[action=save]': {
				click: me.dataSave
			},
			'profitsharingpilihformdata button[action=cancel]': {
	...
	formDataViewAfterRender: function (el) {
		...
		var plDetailStore = me.getProfitsharingpilihpurchaseletterdetailStore();
...
//				alert(rec[0].get('harga_total_jual'));
	...
	processPrint: function () {
		...
		if (win) {
			...
			var reportFile = 'ProfitSharingPilih';
...
//				me.getGriddetail().getStore().removeAll();
			...
			update: function () {
				...
				me.getGriddetail().getStore().getProxy().setExtraParam('profitsharing_id', record.get('profitsharing_id'));
	...
	dataSaveConfirm: function (store) {
		...
		Ext.Ajax.request({
			url: 'erems/profitsharingpilih/update',
			params: {
	...
	gridSelectionChange: function () {
		...
		if (row.length == 1) {
			grid.down('#btnView').setDisabled(row[0].data.profitsharing_name == "");
		}
//        grid.down('#btnDelete').setDisabled(row.length < 1);
	...
	profitSharing: function (el) {
		var me = this;

		me.getFormdata().down("[name=profitsharing_code]").setValue('');
		me.getGriddetail().getStore().removeAll();
		Ext.Ajax.request({
			url: 'erems/profitsharingpilih/read',
			params: {
				profitsharing_id: el.getValue(),
				mode: 'detail'
			},
			success: function (response) {
				me.getFormdata().up('window').body.unmask();
				...
				me.getFormdata().down("[name=profitsharing_code]").setValue(json.data.code);

				me.getGriddetail().getStore().getProxy().setExtraParam('profitsharing_id', el.getValue());
				me.getGriddetail().getStore().load();
			}
		});
	},
	formDataShowView: function (state) {
		var me = this;
		var formtitle = 'Lookup Sudah di Profit Sharing';
		var formicon = 'icon-search';

		var winId = 'win-profitsharingpilihprosesformdataview';
		var win = desktop.getWindow(winId);
