Ext.define('Erems.controller.Purchaseletter', {
	...
	fillFormComponents : function (data, form) {
		...
		// MEMBER / CAC
		var mField = ['cac_id', 'cac_code', 'cac_name'];
		if(me.localStore.cac.length){
			Object.keys(me.localStore.cac[0]).forEach(key => {
				mField.push(key);
			});
		}

		var cacStore = Ext.create('Ext.data.Store', {
			fields : mField,
			data   : me.localStore.cac
        });
        form.down("[name=cac_cac_id]").bindStore(cacStore);
	},
	...
	mainPanelBeforeRender : function (el) {
		...
		me.tools.ajax({
			...
			success : function (data, model) {
				...
				Ext.Ajax.request({
					url     : 'erems/purchaseletter/read',
					params  : { mode_read : 'assetCAC' },
					success : function (res) {
						var result = Ext.JSON.decode(res.responseText);
						me.localStore.cac = result.data;
					},
				});
			}
