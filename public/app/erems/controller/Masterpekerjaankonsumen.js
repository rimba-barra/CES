Ext.define('Erems.controller.Masterpekerjaankonsumen', {
    extend: 'Main.library.Controller',
    
	alias: 'controller.Masterpekerjaankonsumen',
	
	models: [
        'Masterpekerjaankonsumen'
    ],
    stores: [
        'Masterpekerjaankonsumen'
    ],
    views: [
        'masterpekerjaankonsumen.Panel',
		'masterpekerjaankonsumen.FormSearch',
		'masterpekerjaankonsumen.Grid',
		'masterpekerjaankonsumen.FormData'
    ],
	...
	dataSave: function(o) {	
		...
		if (formData.getForm().isValid()) {		
            ...
            Ext.Msg.show({
                title: 'Failure',
                msg: me.checkRequired(o)+' is required.',
                icon: Ext.Msg.ERROR,
                buttons: Ext.Msg.OK
            });
        }
	...
	/*formDataBeforeRender: function(el) {
		...
		switch(el.state){
			case 'CREATE':
				//me.setComboboxDefault(el.down('#country_id'), 'defaultCountryId');
				el.initData.country_id = el.down('#country_id').getStore().defaultCountryId;
				break;
		}		
	},
			
	dataReset: function() {
		var me = this, countryField = me.getMainFormSearch().down('#country_id');
		me.callParent(arguments);
		countryField.setValue(countryField.getStore().defaultCountryId);
	}	*/
});