Ext.define('Hrd.controller.Plafonkaryawan', {
    ...
    init: function(config) {
        ...
        newEvs['plafonkaryawanformsearch [action=search_plafon]'] = {
            blur: function(el) {

                me.refreshFormData(me.getGrid().getSelectedRecord());
            }
        };
        newEvs['plafonkaryawanformdata [name=is_global]'] = {
            change: function(el) {

                me.opsiYearlyonSelect();
            }
        };
    ...
    addNewRecord: function() {
        ...
        if (rec) {
            ...
            f.down("[name=is_global]").setDisabled(false);
            return true;
        }
        return false;

    },
    opsiYearlyonSelect: function() {
        var me = this;
        var f = me.getFormdata();
        var vs = f.getForm().getValues();
        var isGlobal = vs["is_global"] === 1 ? true : false;

        f.down("[name=yearly_obat]").setReadOnly(isGlobal);
        f.down("[name=yearly_dokter]").setReadOnly(isGlobal);
        f.down("[name=yearly_gigi]").setReadOnly(isGlobal);
        f.down("[name=yearly_lab]").setReadOnly(isGlobal);

        f.down("[name=yearly_global]").setReadOnly(!isGlobal);

        if (isGlobal) {
            f.down("[name=yearly_obat]").setValue(0);
            f.down("[name=yearly_dokter]").setValue(0);
            f.down("[name=yearly_gigi]").setValue(0);
            f.down("[name=yearly_lab]").setValue(0);
        }else{
            f.down("[name=yearly_global]").setValue(0);
        }
        ...
        me.tools.ajax({
            ...
            success: function(data, model) {
               ...
               if(plafon){
                   var prefix = "";
                   for(var i in plafon){
                       for(var j in plafon[i]){
                           if(i!=="plafonkaryawan"){
                               prefix = i+"_";
                           }
                         
                           var el = f.down("[name="+prefix+""+j+"]");
                           if(el){
                               if(el.getXType()==="xmoneyfield"){
                                    el.setValuem(plafon[i][j]);
                               }else{
                                   el.setValue(plafon[i][j]);
                               }
