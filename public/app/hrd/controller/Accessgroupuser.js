Ext.define('Hrd.controller.Accessgroupuser', {
    extend: 'Hrd.library.box.controller.Controller',
    alias: 'controller.Accessgroupuser',
    requires: [
    ...
    controllerName: 'accessgroupuser',
    ...
    init: function () {
        ...
        newEvs['accessgroupusergrid button[action=approve]'] = {
            click: function () {
                me.Approve();
            }
        };
		
        newEvs['accessgroupusergrid button[action=reject]'] = {
            click: function () {
                me.Reject();
            }
        };
		
        newEvs['accessgroupusergrid button[action=submitforapp]'] = {
            click: function () {
                me.Submitforapp();
            }
        };
								...
								if(parseInt(user_index_no) == 0){
									...
									for (var i = 0; i < rows.length; i++) {
										if (user_index_no >= rows[i]['data'].index_no){
											...
											me.tools.ajax({
												params: {
													'accessgroup_user_id': rows[i]['data'].accessgroup_user_id
												},
				...
				fn: function (buttonValue, inputText, showConfig) {
					if (buttonValue == 'yes') {
						...
						for (var i = 0; i < rows.length; i++) {
							...
							all_id += rows[i]['data'].accessgroup_user_id;
            ...
            create: function () {
				me.tools.ajax({
					...
					success: function (data, model) {
						...
						me.tools.wesea(data.accessgroup, f.down("[name=accessgroup_id]")).comboBox();		
					}
				}).read('listcb');
                me.unMask(1);
            },
