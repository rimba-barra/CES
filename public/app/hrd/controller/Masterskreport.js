Ext.define('Hrd.controller.Masterskreport', {
    extend: 'Hrd.library.box.controller.ControllerReport',
    alias: 'controller.masterskreport',
    controllerName: 'masterskreport',
    bindPrefixName: 'Masterskreport',
    dateNow: new Date(),
    ParamRender:null,
    ...
    init: function(application) {
        ...
        newEvs['#MasterskReportPanel #btnExport'] = {
            click: function(el, val) {
                this.exportData();       
            }
        };
    ...
    processParams: function(reportData) {
        ...
        var masterkategoriskId = reportData['params']['masterkategorisk_id'];
        ...
        reportData['params']['end_date_text'] = me.tools.dateFunc(reportData['params']['end_date']).toDMY('-');
         reportData['params']['masterkategorisk_id'] = masterkategoriskId==="999"?"0":masterkategoriskId;
    ...
    zendInitLoaded: function(data) {
        ...
        console.log(data);
        me.tools.wesea(data.masterkategorisk, f.down("[name=masterkategorisk_id]")).comboBox(true);
     ...
     //   me.tools.wesea(data.group, f.down("[name=group_id]")).comboBox(true);
        ...
        f.down("[name=masterkategorisk_id]").setValue('999');
    ...
    cleannullinCombo: function (form, value) {
        if (typeof (form.down("[name=masterkategorisk_id]").getValue()) !== 'number') {
            value['masterkategorisk_id'] = '0';
        }
