Ext.define('Hrd.controller.Overtimereport', {
    ...
    init: function(application) {
        ...
        newEvs['#employeeDatasFormID #overtimereport_reporttypeID'] = {
            change: function (fld, a) {
                me.reportTypeOnChange();
            }
        };
        
        newEvs['#employeeDatasFormID #btnExportOvertime'] = {
            click: function(el, val) {
                this.exportData();       
            }
        };
    ...
    processParams: function(reportData) {
        ...
        var format_laporan = reportData['params']['format_laporan'];
        ...
        reportData['params']['periode'] = me.tools.dateFunc(reportData['params']['start_date']).toDMY('/') + ' - ' + me.tools.dateFunc(reportData['params']['end_date']).toDMY('/');
        reportData['params']['department_id'] = departmentId==="999"?"":departmentId;

        var sdp = new Date(reportData['params']['start_date_process']);
        reportData['params']['start_date_process'] = me.tools.dateFunc(reportData['params']['start_date_process']).toYMD('-');
        reportData['params']['end_date_process'] = me.tools.dateFunc(reportData['params']['end_date_process']).toYMD('-');
        reportData['params']['start_date_process_text'] = me.tools.dateFunc(reportData['params']['start_date_process']).toDMY('/');
        reportData['params']['end_date_process_text'] = me.tools.dateFunc(reportData['params']['end_date_process']).toDMY('/');
        reportData['params']['periode_process'] = me.tools.dateFunc(reportData['params']['start_date_process']).toDMY('/') + ' - ' + me.tools.dateFunc(reportData['params']['end_date_process']).toDMY('/');

        switch (reportData['params']['report_type']) {
            case 'transaksi': 
                //reportData['file'] = 'HrdLemburTransaksi';break;
                reportData['file'] = format_laporan === 'R' ? 'HrdLemburTransaksiRekap' : 'HrdLemburTransaksi';
                break;
            case 'rekap_jam': 
                //reportData['file'] = 'HrdLemburRekap'; Comment by Wulan Sari 2018.05.11
                reportData['file'] = format_laporan === 'R' ? 'HrdLemburRekap' : 'HrdLemburDetail'; // added by Wulan Sari 2018.05.11
                break;
            case 'transaksi_faktor': 
                //reportData['file'] = 'HrdLemburTransaksi';break;
                reportData['file'] = format_laporan === 'R' ? 'HrdLemburTransaksiFaktorRekap' : 'HrdLemburTransaksiFaktor';
                break;
            break;
    ...
    // added by Wulan Sari 2018.05.11
    reportTypeOnChange: function() {
        /*
        var me = this;
        var vs = me.getForm().getValues();
        var m = vs['report_type'];        
        if(m === 'transaksi'){
            me.getForm().down("#formatLaporanID").hide();
        } else if(m === 'rekap_jam'){
            me.getForm().down("#formatLaporanID").show();        
        }
        */
    },
    cleannullinCombo: function (form, value) {
        ...
        if (!form.down("[name=start_date_process]").getValue()) {
            value['start_date_process'] = '1900-01-01';
        }
        if (!form.down("[name=end_date_process]").getValue()) {
            value['end_date_process'] = '3000-12-31';
        }
        return value;
    },
    exportData:function(){
