Ext.define('Hrd.controller.Absentrecord', {
    ...
    refs: [
        ...
        // added by rico 11012024
        {
            ref: 'formviewlogsabsentdetail',
            selector: 'absentrecordformviewlogsabsentdetail'
        },
        {
            ref: 'gridlogsabsentdetail',
            selector: 'absentrecordgridlogsabsentdetail'
        },
    ],
    //unitFormula: null,
    ...
    init: function () {
        ...
        newEvs['absentrecordformcorrectionabsent button[action=view]'] = { // added by rico 12012024
            click: function () {
                me.showCorrectionLog();
            }
        };
        ...
        newEvs['absentrecordformtooltransfer #file_excelabsent'] = {
            change: function (fld, a) {
                me.formAbsentExcelUpload(fld, a, 'mode');
            }
        };

        newEvs['absentrecordformtooltransfer button[action=download_template_excel]'] = {
            click: function () {
                me.downloadTemplateExcel();
            }
        };
        ...
        for (var i = 0; i < rows.length; i++) {
                    ...
                    me.tools.ajax({
                            params: {
                                ...
                                , 'total_lost'   : rows[i]['data'].total_lost,
                                'avg_lost'   : rows[i]['data'].avg_lost
                            },
                            success: function (data, model) {
                                counter++;
                                if (countarray == counter) {
    ...
    modeTransferOnSelect: function () {
        ...
        if (m === "C") {
            ...
            f.down("#file_excelabsent").hide(); 
            f.down("#download_template_excel").hide(); 
        } else if (m === "D" || m === "T") {
            //  f.down("#file_csvabsent").show();
            ...
            f.down("#file_excelabsent").show(); 
            f.down("#download_template_excel").show(); 
        }
    },
    formAbsentCsvUpload: function (fld, a, mode) {
        ...
        form.remove('file_excelabsent'); // added by rico 23012024
        form.remove('download_template_excel'); // added by rico 23012024
    ...
    transferAjaxAbsent: function (form, current, all) {
        var me = this;
        form.setLoading("Progress : " + current + " of " + all);
        me.tools.ajax({
            params: {data: Ext.encode(me.fpnl[current - 1])},
       ...
       // added by wulan sari 20200432
        ...
        me.tools.ajax({
            ...
            success: function (data, model) {
                ...
                var processAbsent = new Hrd.minimodule.absent.ProcessAbsent({ // added by rico 19022024
                    controllerName: me.bindPrefixName
                });

                processAbsent.askFingerPrint(); // added by rico 19022024
            }
    ...
    toolTransfer: function () {
        ...
        var x = {
            ...
            processOnClick: function () {
                ...
                // start added by rico 16022024
                var today  = new Date(); 
                var day    = Math.abs(moment(fromDate).diff(endDate, 'day'));
                var month  = Math.abs(moment(fromDate).diff(endDate, 'month'));
                var yearF  = Math.abs(moment(today).diff(fromDate, 'year'));
                var yearE  = Math.abs(moment(today).diff(endDate, 'year'))
                // end added by rico 16022024

                if((yearF == 0 || yearF == 1) && (yearE == 0 || yearE == 1)){ // added by rico 16022024
                    if(day > 31){
                        Ext.Msg.show({
                             title:'ERROR',
                        ...
                        me.tools.ajax({
                            params: {
                                fromDate: fromDate, // added by rico 16022024
                                endDate: endDate, // added by rico 16022024
                            ...
                            success: function (data, model) {
                                if (data['others'][0][0]['STATUS']) {
                                    ...
                                    me.transferAjaxAbsent(f, 1, me.fpnl.length);
                                } else {
                                    me.tools.alert.warning(data['others'][0][0]['ERRORMSG']);

                                    // added by wulan sari 20200432
                                    me.transferTeamsAbsent();
                                }
                                //  f.setLoading(false);
                                f.up("window").close();
                            }
...
//  me.currentFilter.emRec = me.getGridemployee().getSelectedRecord();
  ...
  */
    ...
    downloadTemplateExcel:function(){ // added by rico 22012024
        window.open(document.URL+"app/hrd/uploads/msexcel/template/template_absen_excel.xlsx");
    },
    gridTimePopup: function(index, dataIndex) {
        ...
        me.tools.ajax({
            ...
            success: function (data, model) {
                var hasil = data['others'][0][0]['HASIL'][0][0];
                if(hasil['total'] > 0){
                    ...
                    if(today.getTime() === firstDay.getTime() || today.getTime() === lastDay.getTime()){
                        me.tools.ajax({
                            ...
                            success: function (data, model) {
                                var temp    = data['others'][0][0]['HASIL'];
                                var prev    = (today.getTime() === firstDay.getTime()) ? temp[0]: null;
                                var next    = (today.getTime() === lastDay.getTime()) ? temp[1]: null;

                                me.timelog(grid, index, hasil, prev, next);
                            }
                        }).read('getprevmonth');
                    }else{
                        me.timelog(grid, index, hasil);
                    }
                }else{
    ...
    timelog: function(grid, index, hasil, prev = null, next = null){
        var me  = this;
        var rec = grid.getStore().getAt(index);

        me.instantWindow("FormCorrectionAbsent", 675, "Correction Absent", "processcorrectionabsent", "toolProcessCorrectionAbsentWinId");

        var tempDate      = new Date(rec.data.date);
        var form          = me.getFormviewlog();
        var timeIn        = me.splitTimeType(rec)[0];
        var timeOut       = me.splitTimeType(rec)[1];
        var timeinType    = me.splitTimeType(rec)[2];
        var timeoutType   = me.splitTimeType(rec)[3];
        ...
        var absentDate    = formattedDate;
        var timeInDate    = formattedDate + " " + timeIn;
        var timeOutDate   = formattedDate + " " + timeOut;

        var prevRec       = (prev == null) ? grid.getStore().getAt((rec.index - 1)) : prev;
        var nextRec       = (next == null) ? grid.getStore().getAt((rec.index + 1)) : next;

        me.tools.ajax({
            params: {
                absentdetail_id: rec.get('absentdetail_id'),
                absentdetail_prev_id: (prev == null) ? prevRec.get('absentdetail_id') : prevRec[0]['absentdetail_id'],
                absentdetail_next_id: (next == null) ? nextRec.get('absentdetail_id') : nextRec[0]['absentdetail_id'],
            },
            success: function (data, model) {
                var hasilShift= data['others'][0][0]['HASIL'];

                var yesterday = new Date(rec.data.date);
                yesterday     = new Date(yesterday.setDate(yesterday.getDate() - 1));
                var yy        = yesterday.getFullYear();
                var my        = ((yesterday.getMonth() + 1) < 10) ? "0" + (yesterday.getMonth() + 1): (yesterday.getMonth() + 1);
                var dy        = (yesterday.getDate() < 10) ? "0" + yesterday.getDate(): yesterday.getDate();
                var fy        = dy + "-" + my + "-" + yy;

                var tomorrow  = new Date(rec.data.date);
                tomorrow      = new Date(tomorrow.setDate(tomorrow.getDate() + 1));
                var yt        = tomorrow.getFullYear();
                var mt        = ((tomorrow.getMonth() + 1) < 10) ? "0" + (tomorrow.getMonth() + 1): (tomorrow.getMonth() + 1);
                var dt        = (tomorrow.getDate() < 10) ? "0" + tomorrow.getDate(): tomorrow.getDate();
                var ft        = dt + "-" + mt + "-" + yt;

                var shiftTd   = hasilShift[0];
                var shiftY    = hasilShift[1];
                var shiftTt   = hasilShift[2];
                ...
                f.down("[name=absentrecordtimein]").setValue(timeIn);
                f.down("[name=absentrecordtimeout]").setValue(timeOut);
                f.down("[name=absentdetail_id]").setValue(rec.data.absentdetail_id);
                f.down("[name=timein_type]").setValue(timeinType);
                f.down("[name=timeout_type]").setValue(timeoutType);
                f.down("[name=timein]").setValue(timeIn);
                f.down("[name=timeout]").setValue(timeOut);
                f.down("[name=timein_id]").setValue(timeIn);
                f.down("[name=timeout_id]").setValue(timeOut);
                ...
                me.tools.ajax({
                    params: {
                        absentdetail_id: rec.get('absentdetail_id'),
                        projectptid_opsi: projectptid_opsi,
                        employee_id: hasil['employee_id'],
                        project_id: hasil['project_id'],
                        pt_id: hasil['pt_id'],
                        fromdate: fy,
                        untildate: ft,
                        page:1,
                    ...
                    success: function (data, model) {
                        var recin   = { data: me.assignTime(data, [rec.data.date, yy + "-" + my + "-" + dy, yt + "-" + mt + "-" + dt], [shiftTd[0].in_time, shiftY[0].out_time], 'timein'), model:model }
                        var recout  = { data: me.assignTime(data, [rec.data.date, yy + "-" + my + "-" + dy, yt + "-" + mt + "-" + dt], [shiftTd[0].out_time, shiftTt[0].in_time], 'timeout'), model:model }

                        // var recin   = { data: me.assignTime(data, [rec.data.date, timeIn, timeOut], prevRec, 'timein'), model:model }
                        // var recout  = { data: me.assignTime(data, [rec.data.date, timeIn, timeOut], nextRec, 'timeout'), model:model }
    ...
    assignTime: function(data, date, shift, type){
        var me          = this;
        var arr         = [];
        ...
        var tomorrow    = new Date(date[2]);
        tomorrow.setHours(0, 0, 0);

        for(var i=0;i<data.length;i++){
            var val   = data[i]['timelog'];
            var tDate = new Date(val['date']);
            tDate.setHours(0, 0, 0);
            var time = Date.parse(val['date']+ ' ' +val['time']);
            
            if(type == 'timein'){
                var shiftIn  = Date.parse(date[0] + ' ' +shift[0]);
                var shiftOut = Date.parse(date[1] + ' ' +shift[1]);

                if(tDate <= tempDate && tDate >= yesterday){
                    if(shift[1] == '00:00:00'){
                        arr.push(val);        
                    }else if(time >= shiftOut){
                        arr.push(val);
                    }
                }
            }else{
                var shiftIn  = Date.parse(date[2] + ' ' +shift[1]);
                var shiftOut = Date.parse(date[0] + ' ' +shift[0]);

                if(tDate >= tempDate && tDate <= tomorrow){
                    if(shift[1] == '00:00:00'){
                        arr.push(val);        
                    }else if(time <= shiftIn){
                        arr.push(val);
                    }
                }
            }
    ...
    splitTimeType: function(records){
        var me      = this;
        var timeIn  = "", timeOut = "", timeIn_type = "", timeOut_type = "";
        var data    = (records.data != undefined) ? records.data: records;

        for(const item in data){
            if((records.data != undefined)){

                switch(true){
                    case item == "in_7_14" && data[item] != '':
                        timeIn = me.tools.dateFunc(data[item]).toHIS();
                        timeIn_type = "in_7_14";
                    break;
                    case item == 'in_15_21' && data[item] != '':
                        timeIn = me.tools.dateFunc(data[item]).toHIS();
                        timeIn_type = "in_15_21";
                    break;
                    case item == 'in_22_6' && data[item] != '':
                        timeIn = me.tools.dateFunc(data[item]).toHIS();
                        timeIn_type = "in_22_6";
                    break;
                    case item == 'out_7_14' && data[item] != '':
                        timeOut = me.tools.dateFunc(data[item]).toHIS();
                        timeOut_type = "out_7_14";
                    break;
                    case item == 'out_15_21' && data[item] != '':
                        timeOut = me.tools.dateFunc(data[item]).toHIS();
                        timeOut_type = "out_15_21";
                    break;
                    case item == 'out_22_6' && data[item] != '':
                        timeOut = me.tools.dateFunc(data[item]).toHIS();
                        timeOut_type = "out_22_6";
                    break;
                }
            }else{
                switch(true){
                    case item == "in_7_14" && data[item] != null:
                        timeIn = data[item];
                        timeIn_type = "in_7_14";
                    break;
                    case item == 'in_15_21' && data[item] != null:
                        timeIn = data[item];
                        timeIn_type = "in_15_21";
                    break;
                    case item == 'in_22_6' && data[item] != null:
                        timeIn = data[item];
                        timeIn_type = "in_22_6";
                    break;
                    case item == 'out_7_14' && data[item] != null:
                        timeOut = data[item];
                        timeOut_type = "out_7_14";
                    break;
                    case item == 'out_15_21' && data[item] != null:
                        timeOut = data[item];
                        timeOut_type = "out_15_21";
                    break;
                    case item == 'out_22_6' && data[item] != null:
                        timeOut = data[item];
                        timeOut_type = "out_22_6";
                    break;
                }
        ...
        return [timeIn, timeOut, timeIn_type, timeOut_type];
    },
    correctionAbsentProcessOnClick: function(){
        ...
        switch(true){
            case !data['timein_id']:
                me.tools.alert.warning('Time In harus diisi!');

                f.setLoading(false);
            break;
            case !data['timeout_id']:
                me.tools.alert.warning('Time Out harus diisi!');

                f.setLoading(false);
            break;
            case !data['reason']:
                me.tools.alert.warning('Reason harus diisi!');
            ...
            default:
                ...
                //added by anas 15032024 | lempar shift untuk perhitungan losttime
                var rec = me.getGrid().getSelectedRecord();
                data['shifttype_shifttype_id'] = rec.get("shifttype_shifttype_id");
                //end added by anas
                ...
                me.tools.ajax({
                    params: data,
                    success: function (data, model) {
                        if (data['others'][0][0]['MSG'] == 'Berhasil mengganti absen') {
                            me.tools.alert.info("Berhasil mengganti absen");
                        } else {
                            me.tools.alert.warning(data['others'][0][0]['MSG']);
                        }
    ...
    showCorrectionLog: function(){
        var me   = this;
        var f    = me.getFormcorrectionabsent();
        var data = f.getForm().getValues();
        
        me.instantWindow("FormViewLogsAbsentDetail", 800, "View Logs Absent Detail", "processlogsabsentdetail", "absentrecordformviewlogsabsentdetail");
        var grid = me.getGridlogsabsentdetail();
        grid.doInit();

        var store = grid.getStore();
        store.getProxy().extraParams = {
            mode_read: 'getlogabsentdetail',
            absentdetail_id :data['absentdetail_id']
        };
        
        store = store.load({
            params: { absentdetail_id :data['absentdetail_id']},
            callback: function (data, model) {
                grid.attachModel(model);
                grid.store.sort({property: 'date', direction: 'ASC'});
                Ext.apply(store, {pageSize: 25});  
    ...
    formAbsentExcelUpload: function(fld, a, mode) {
        var me = this;
        ...
        var form = fld.up("form");
        var p = me.getPanel();
        var fType = fld.value.split('.')[fld.value.split('.').length - 1];

        form.remove('file_csvabsent');
        form.remove('description_format');
        ...
        if(fType == 'xls' || fType == 'xlsx'){
            me.uploadFile({
                ...
                callback: {
                    success: function (fn) {
                        form.setLoading("Transfering absent from excel...");
                        ...
                        me.tools.ajax({
                            params: {
                                ...
                                end_day: 0,
                                is_delete: 0,
                                projectptid_opsi: projectptid_opsi // added by Michael 2021.05.19 
                            },
                            autoAbort: true,
                            success: function (data, model) {
                                me.fpnl      = data['others'][0][0]['FPNUMBERLIST'];
                                var errorMsg = data['others'][0][0]['ERRORMSG'];
                                var status   = data['others'][0][0]['STATUS'];
                                var error    = Object.keys(data['others'][0][0]['ERROR']);

                                function plustwo(num){
                                    return parseInt(num) + 2;
                                }

                                var error_msg = error.length == 2 ? error.map(plustwo).join(" dan ") : error.map(plustwo).join(", ");

                                if (status) {
                                    if(error.length != 0){
                                        Ext.Msg.show({
                                            title: 'WARNING',
                                            msg: "Terdapat kesalahan di beberapa baris data, tolong di perbaiki. <br/> " + error_msg,
                                            buttons: Ext.Msg.OK,
                                            icon: Ext.Msg.WARNING,
                                            fn: function (clicked) {
                                                me.transferAjax(form, 1, me.fpnl.length);
            ...
            me.tools.alert.warning("File harus xls atau xlsx");
