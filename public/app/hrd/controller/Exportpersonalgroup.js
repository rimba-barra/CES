Ext.define('Hrd.controller.Exportpersonalgroup', {
    extend: 'Hrd.library.box.controller.ControllerReport',
    alias: 'controller.exportpersonalgroup',
    controllerName: 'exportpersonalgroup',
    bindPrefixName: 'Exportpersonalgroup',
    otherParamsAT :{leave:0,sick:0,permission:0},
    init: function(application) {
        this.callParent(arguments);
        var newEvs = {};
        var me = this;
        newEvs['#ExportpersonalgroupFormID [name=based]'] = {
            select: function(el, val) {
                me.changeFilterFields(el, val);
            }
        };
        newEvs['#ExportpersonalgroupFormID #searchButtonID'] = {
            click: function(el, val) {
                me.searchEmpClick();
            }
        };
        
        newEvs['#ExportpersonalgroupFormID [name=department_id]'] = {
            select: function(el, val) {
                me.filterEmployeey();
            }
        };
        
        newEvs['#ExportpersonalgroupFormID [name=group_id]'] = {
            select: function(el, val) {
                me.filterEmployeey();
            }
        };        
        
        newEvs['#ExportpersonalgroupFormID button[action=export]'] = {
            click: function (el, val) {
                this.exportData();                
