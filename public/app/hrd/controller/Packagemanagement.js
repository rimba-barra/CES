Ext.define('Hrd.controller.Packagemanagement', {
    ...
    refs: [
        ...
        {
            ref: 'formcopy',
            selector: 'packagemanagementformcopy'
        }
    ...
    init: function () {
        ...
        newEvs['packagemanagementgrid toolbar button[action=copy] '] = {
            click: function () {				
                me.Copy();
            }
        }
		
        newEvs['packagemanagementformcopy'] = {
            afterrender: function () {
				var me, form,datafilter,project_id;
                me = this;
                form = me.getFormcopy();
                me.tools.ajax({
                    params: {},
                    success: function (data, model) {
						if(data.project){
							project_id = data.others[0].user_project_id;	
							datafilter = me.filterProject(data.project, project_id);
							me.tools.wesea(datafilter, form.down("[name=project_id]")).comboBox();
							me.tools.wesea(data.pt, form.down("[name=pt_id]")).comboBox();
						}
                    }
                }).read('headerdata');
			}           
        };
        newEvs['packagemanagementformcopy button[action=save]'] = {
            click: function () {
                me.saveCopy();
            }
        };
		/*
        newEvs['packagemanagementformcopy combobox[name=project_id]'] = {            
            select: function () {
				datafilter = me.filterPt();
				me.tools.wesea(datafilter, form.down("[name=pt_id]")).comboBox();				
			}
        };*/
		/*end wulan edit*/
	...
	/*wulan edit 2017 11 02*/
    filterProject: function (param, project_id) {
		var me, datahasfilter;
		me = this;
	...
	/*
    filterPt: function () {
		var me, datahasfilter, project_id;
		me = this;
		form = me.getFormcopy();
		
        var ptCombo = form.down("[name=pt_id]");
        var store = ptCombo.getStore();
        store.filterBy(function(recod){return recod.data.project_id==project_id;});		
    ...
    mainDataSave: function () {
		...
		if(gds.getCount()==0){
			...
			for(var i=0;i<gds.getCount();i++){
                            if(gds.getAt(i).get("deleted") === true){
                            } else {
    ...
    saveDetailtoStore: function () {
	    ...
	    //percent = parseFloat(store.sum('bobot'));	
            // Wulan update 13 desember 2017
            percent = 0;
            bobot = 0;
            store.each(function (record)
            {
                if(record.data['deleted'] === true){
                    bobot = parseFloat(record.data['bobot']);
                    if(!isNaN(bobot)){
                            percent += bobot;
                    }
                }
    ...
    Copy: function () {
        var me, rows;
        me = this;
		me.formCopy.stateform = 'Copy from other';
		me.dynamicrequest.GenerateFormdata(me.formCopy);
    },
    formCopy: {
        //start formgeneate
        fromlocation: 'Hrd.view.packagemanagement.FormCopy',
        formtitle: 'Form', formicon: 'icon-form-copy',
        formid: 'win-packagemanagementformcopy', formlayout: 'fit',
        formshadow: 'frame', formmask: 'Loading...',
    ...
    saveCopy: function () {
        //copy doc
        var me, form, data, pt_id, project_id;
        me 			= this;
        form 		= me.getFormcopy();
        me.dynamicrequest.formMask(form);
		...
		me.tools.ajax({
			...
			success: function (data, model) {
				...
				me.dynamicrequest.buildSuccessAlert('Copy data finish successfully');
				me.dynamicrequest.formUnmask(form);
				me.dynamicrequest.formClose(form);
				me.getGrid().getStore().reload();
