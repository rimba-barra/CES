Ext.define('Hrd.controller.Absentrecord', {
    ...
    timeProcessOnClick: function () {
        ...
        var data = {};
        var zoneIn = tools.getTimeZone(f.down("[name=time_in]").getValue(), true);
        var zoneOut = tools.getTimeZone(f.down("[name=time_out]").getValue(), false);
        if (zoneIn) {
            data[zoneIn] = f.down("[name=time_in]").getValue();
        } else {
            console.log("gak ada data in");
        }
        if (zoneOut) {
    ...
    toolTransfer: function () {
        ...
        var x = {
            ...
            /*
             ...
             var f = me.getFormtransfer();
             var vs = f.getValues();
             //  f.setLoading("Transfering finger print data...");
             ...
             modal: true,
             style: {
             backgroundImage: 'url("resources/extjs/resources/themes/images/default/grid/loading.gif")'
             },
             ...
             {
             html: '<div style="margin:0 0 0 20px">Loading...</div>'
             },
             {
             xtype: 'button',
             text: 'Cancel',
             ...
             win.show();
             win.down("panel").getEl().setStyle({backgroundImage: 'url(resources/extjs/resources/themes/images/default/grid/loading.gif)', backgroundSize: '16px 16px', border: '0'});
             ...
             autoAbort: true,
             success: function (data, model) {
             console.log("PROCESS TRANSFER LOG");
             console.log(data);
             if (data['others'][0][0]['STATUS']) {
             me.tools.alert.info("Success transfer");
        ...
        if (valid.valid) {
            ...
            s.load({
                ...
                callback: function () {
                    ...
                    var yel = me.getFormsearch().down("[name=year_pick]");
                    if (yel.getStore().getCount() > 0) {
                        var v = yel.getStore().getAt(yel.getStore().getCount() - 1).get('number');
                        yel.setValue(v);
                        me.yearPickOnChange(yel);
                    }
    ...
    filterEmployeeList: function () {
        ...
        /*var f = me.getFormsearch();
         ...
         var y = parseInt(f.down('[name=year_pick]').getValue());
         var m = parseInt(f.down('[name=month_pick]').getValue());
         var d = parseInt(f.down('[name=search_department_id]').getValue());
         
         if (y > 0 && m > 0) {
         s.clearFilter(true);
         var cond = false;
         s.filterBy(function(rec, id) {
         if (d === 999) { // all department
         cond = parseInt(rec.raw.absent.month) === m && parseInt(rec.raw.absent.year) === y;
         } else {
         cond = parseInt(rec.raw.absent.month) === m && parseInt(rec.raw.absent.year) === y && parseInt(rec.raw.department.department_id) === d;
         }
         if (cond) {
         return true;
         }
            ...
            insertToSheet: function () {
                ...
                if (selectedShift) {
                    ...
                    if (selectedDays.length > 0) {
                        ...
                        me.tools.warning("No day selected");
                    }
    ...
    /*panelAfterRender: function(el) {
     ...
     //  cmp.setValue(999);
     me.tools.comboHelper(f.down("[name=" + cb[c] + "]")).setDefaultValue(false);
     me.filterLoaded.department = true;
     me.viewOneEmployeAbsent();
     f.setLoading(false);
     });
     }
        ...
        me.tools.ajax({
            ...
            success: function (data, model) {
                ...
                if (departments) {
                    ...
                    highDepartment = ds.getAt(ds.getCount() - 1).get('department_id');
