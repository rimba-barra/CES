 ...
 * 
 */
Ext.define('Gl.library.box.controller.Controller2', {
    extend: 'Gl.library.box.controller.Controller',
    saveStore: null, /// store untuk proses save record | type data string
    requires: ['Gl.library.box.tools.StoreProcessor', 'Gl.library.box.tools.DefaultConfigfdv', 'Gl.library.box.tools.StoreProcessor', 'Gl.library.box.tools.EventSelector2',
        'Gl.library.box.tools.SimpleGridControl', 'Gl.library.box.tools.Tools', 'Gl.template.ComboBoxFields'],
    isMaximize: true,
    browseHandler: null,
    // added 11/12/2014 
    ...
    constructor: function(configs) {
        var me = this;
        var config = new Gl.library.box.tools.DefaultConfigfdv({
            moduleName: me.controllerName
        });
        config.run(this);
        this.callParent(arguments);
        me.comboBoxFields = new Gl.template.ComboBoxFields();
    },
    // added 5/12/2014
    init: function() {
        var me = this;
        var events = new Gl.library.box.tools.EventSelector2();
    ...
    loadPage: function(store) {
        ...
        if (me.runFuncBeforeLoadGridData) {
            me.runFuncBLGD(store,function(xstore) { // modified 18 Nov 2015
                xstore.loadPage(1, {
                    callback: function(rec, operation, success) {
    ...
    // added 15 Oktober 2015
    fixMoneyFields:function(mode,data,formData){
       var me = this;
       var mf = me.moneyFieldList;
       if(mode===1){ // convert ke format uang
           for(var i in mf){
               formData.down("[name="+mf[i]+"]").setValuem(data.get(mf[i]));
           }
       }else if(mode===2){ // unformat uang
           for(var i in mf){
         
               data[mf[i]] = accounting.unformat(data[mf[i]]);
           }
       }
    }
});