Ext.define('Cashier.library.journal.Journalar', {
    config: null,
...
//                                       td_paymentdetail_temp, bukan remaining balance dari schedule
    ...
    loadAR: function (c, row) {
        ...
        if (row) {
            ...
            if(row.get("remaining_balance") == row.get("amount")){
                remaining_pay  = row.get("amount");
            }else{
                remaining_pay  = row.get("remaining_balance");
            }
            row.set("remaining_pay", remaining_pay);
            ...
            c.isEdit = 1;
            var tab = {name: 'detailjournalgrid'};
            c.journalDetail.resetDetailCoa(c,tab);
        }
    },
        ...
        g.on({
            ...
            edit: function (roweditor, event) {
                ...
                if (accounting.unformat(event.value) > amountunpaid) {
                    ...
                    event.record.set('remaining_pay', amountunpaid);
