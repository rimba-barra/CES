Ext.define('Cashier.library.template.controller.Controller', {
    extend: 'Ext.app.Controller',
    requires: ['Cashier.library.template.field.CurrencyField',
        'Cashier.library.template.field.MoneyField',
        'Cashier.library.template.field.Customepaging',
    ],
    rights: {
    ...
    constructor: function (configs) {
        ...
        if(is_accountingloaded == 0){
            is_accountingloaded = 1;
            Ext.Loader.loadScript({
                //for function accounting in UI.
                url: document.URL + 'app/cashier/library/template/plugin/accounting.min.js',
                onLoad: function () {
                    accounting.settings = {
                        currency: {
            ...
            win = desktop.createWindow({
                ...
                listeners: {
                    boxready: function () {
                        win.body.mask('Loading...');
                        var tm = setTimeout(function () {
                            win.add(Ext.create('Cashier.view.' + me.controllerName + '.FormImport'));
                            win.center();
                            win.body.unmask();
                            clearTimeout(tm);
                        }, 1000);
...
//            });
    ...
    GeneratorStore:function(storename){
      var mystore =  Ext.define('Cashier.library.template.controller.' + storename, {
       extend: 'Ext.data.Store',
       alias: 'store.'+storename,
       requires: [
            'Cashier.library.template.controller.'+storename
        ],  
        proxy: {
            type: 'memory',
            reader: {
