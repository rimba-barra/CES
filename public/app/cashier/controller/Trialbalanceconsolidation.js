Ext.define('Cashier.controller.Trialbalanceconsolidation', {
    extend: 'Cashier.library.template.controller.Controllergl',
    alias: 'controller.Trialbalanceconsolidation',
    requires: [
    ...
    views: [
        'trialbalanceconsolidation.Panel',
        'trialbalanceconsolidation.FormData'
    ],
    ...
    refs: [
        {
            ref: 'formdata',
            selector: 'trialbalanceconsolidationformdata'
        },
        {
            ref: 'paneldata',
            selector: 'trialbalanceconsolidationpanel'
        }
    ],
    controllerName: 'trialbalanceconsolidation',
    fieldName: '',
    bindPrefixName: 'Trialbalanceconsolidation',
    reportfile: 'Trialbalanceconsolidation',
    urlprocess: 'cashier/trialbalanceconsolidation/create',
    ...
    init: function (application) {
        var me = this;
        this.control({
            'trialbalanceconsolidationpanel': {
            ...
            'trialbalanceconsolidationformdata': {
                afterrender: function (panel) {
                    this.formDataAfterRenderCustome();

                }
            },
            'trialbalanceconsolidationformdata [name=from_coa_id] ': {
                'keyup': function () {
                    var me, value;
                    me = this;
            ...
            'trialbalanceconsolidationformdata [name=until_coa_id] ': {
                'keyup': function () {
                    var me, value;
                    me = this;
            ...
            'trialbalanceconsolidationformdata [name=reporttype] ': {
                change: function () {
                    var me, value;
                    me = this;
                    ...
                    if (reporttype == 'DEFAULT') {
                        me.getFormdata().down("[name=typeheader]").setVisible(true);
                    }else{
                        me.getFormdata().down("[name=typeheader]").setVisible(false);
                    }

                },
            },
            'trialbalanceconsolidationformdata button[action=submit]': {
                click: function () {
                   this.processReport();
                }
            },
            'trialbalanceconsolidationformdata [name=pt_id]': {
                change: function (the, newValue, oldValue, eOpts) {
                    this.loadCoabypt(newValue);
                }
            },
            'trialbalanceconsolidationformdata [name=consolidation_id]': {
                change: function (el) {
    ...
    submitReport: function () {
        var me = this;
        me.report = 'Trialbalanceconsolidation';       
    ...
    processReport: function () {
        ...
        if (true) {
            ...
            params["level"] = f.down("[name=level]").getValue();
            params["header"] = f.down("[name=header]").getValue();
            params["typeheader"] = f.down("[name=typeheader]").getValue();

            if (params["typeheader"]) {
                params["typeheader"] = 1;
            }else{
                params["typeheader"] = 0;
            }
           ...
           reportFile = 'Trialbalanceconsolidation';
            ...
            if(reporttype === 'ELIMINASI-ALL') {
                reportFile = 'Reportconsolidationelim_all';
            }
            if(reporttype === 'ELIMINASI-PER-KONSOL') {
                reportFile = 'Reportconsolidationelim_perkonsol';
            }
            ...
            if (me.getFormdata().getForm().isValid()) {
                ...
                //Ext.getBody().mask("Please wait...");
                resetTimer();
                if(reporttype=='DEFAULT' || reporttype=='ELIMINASI-ALL' || reporttype === 'ELIMINASI-PER-KONSOL' ){
    ...
    generateReportexcelthismonth: function(params){       
         ...
         me.senddata = params,
         me.urlrequest = 'cashier/trialbalanceconsolidation/create';
         Ext.getBody().mask("Please wait...");
         me.AjaxRequest();       
    }, 
