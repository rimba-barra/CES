Ext.define('Cashier.controller.Masterrangebagihasil', {
    ...
    views: [
        'masterrangebagihasil.Panel',
        'masterrangebagihasil.Grid',
        'masterrangebagihasil.FormSearch',
        'masterrangebagihasil.FormData',
        'masterrangebagihasil.FormDataDetail',
        'masterrangebagihasil.GridDetail',
    ],
    stores: [
    ...
    refs: [
        { ref: 'panel', selector: 'masterrangebagihasilpanel' },
        { ref: 'grid', selector: 'masterrangebagihasilgrid' },
        { ref: 'formsearch', selector: 'masterrangebagihasilformsearch' },
        { ref: 'formdata', selector: 'masterrangebagihasilformdata' },
        { ref: 'formdatadetail', selector: 'masterrangebagihasilformdatadetail' },
        { ref: 'griddetail', selector: 'masterrangebagihasilgriddetail' },
    ],
				...
				listeners: {
					boxready: function () {
						...
						var tm = setTimeout(function () {
							win.add(Ext.create('Cashier.view.' + me.controllerName + '.FormDataDetail'));
							win.center();
							win.body.unmask();
							clearTimeout(tm);
						}, 1000);
	...
	dataSaveConfirm: function (store) {
		...
		if (rangeValid) {
			...
			Ext.Ajax.request({
				url   : 'cashier/masterrangebagihasil/create',
				params: {
        ...
        if (rows.length < 1) {
            ...
            Ext.Msg.confirm('Delete Data', confirmmsg, function (btn) {
                if (btn == 'yes') {
                    ...
                    store.sync({
                        success: function(s) {
                            var res = Ext.decode(s.operations[0].response.responseText).result;
                            console.log(res);
                            if(res == 0){
                                ...
                                Ext.Msg.show({
                                    title  : 'Failure',
                                    msg    : failmsg + ' <br/>'+Ext.decode(s.operations[0].response.responseText).message+'.',
                                    icon   : Ext.Msg.ERROR,
                                    buttons: Ext.Msg.OK
                                });
                            }else{
