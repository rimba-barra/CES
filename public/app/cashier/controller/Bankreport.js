Ext.define('Cashier.controller.Bankreport', {
        ...
        this.control({
            'bankreportpanel': {
                ...
                afterrender: function (panel) {
                    ...
                    me.windowsWidht(me.bindPrefixName, 900);
                    me.panelAfterRender();
                }
            },
            'bankreportformdata': {
                ...
                boxready: function () {
                    ...
                    $("#bankreportID input[name='fromcoa']").keyup(function()
                    {
                        this.value = this.value.replace(/(\d{2})(\d{2})/,'$1'+'.'+'$2')
                    });

                    $("#bankreportID input[name='untilcoa']").keyup(function()
                    {
                        this.value = this.value.replace(/(\d{2})(\d{2})/,'$1'+'.'+'$2')
                    });
                }
            ...
            'bankreportformdata [name=allamount]': {
                change: function (the, newValue, oldValue, eOpts) {
                    var me = this;
                    var form = me.getFormdata();
                    if (newValue == true) {
                        form.down("[name=fd_amount_start]").setDisabled(true);
                        form.down("[name=fd_amount_end]").setDisabled(true);
                        form.down("[name=fd_amount_start]").setValue('');
                        form.down("[name=fd_amount_end]").setValue('');
                        
                    }else{
                        form.down("[name=fd_amount_start]").setDisabled(false);
                        form.down("[name=fd_amount_end]").setDisabled(false);
                    }
                }
            },
            'bankreportformdata [name=frompt]': {
                change: function () {
                    var me =this;
                    var form = me.getFormdata();
    ...
    formDataAfterRender: function () {
        var me, storecoa, form, allcompany, alldept, allgroup, allcoa, allamount;
        me = this;
        form = me.getFormdata();
    ...
    setSuccessEvent: function () {
        ...
        if (me.info.parameter == 'default') {
            ...
            is_multi = '0';
            if(form.down("[name=allcompany]").getValue()==true){
                is_multi = '1';
            }
            ...
            value["type"] = 'BANK';
            value["is_multi"] = is_multi;
            value['version'] = me.getValRadio(form, "version");

            if(form.down("[name=allamount]").getValue()==true){
                value["is_amount"]      = 1;
                value["amount_start"]   = '0';
                value["amount_end"]     = '0';
            }else{
                value["is_amount"]      = 2;
                value["amount_start"]   = form.down("[name=fd_amount_start]").getValue();
                value["amount_end"]     = form.down("[name=fd_amount_end]").getValue();
            }
