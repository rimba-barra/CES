...
class Cashier_Models_Transaction_JournalDao extends Cashier_Box_Models_App_AbDaoCashier implements Cashier_Box_Models_App_BlackHole {
    ...
    public function save(Cashier_Models_Master_Journal $cs, Cashier_Box_Models_App_HasilRequestRead $req, $detailjournal, $subdetailjournal, $detailar, $detailescrow) {
        ...
        if (count($detailar) and count($subdetailjournal)) {  //jika ada AR dan detail dan subdetail // bayar angsuran
           ...
           // $amounts = rtrim($amounts, '.00');
            ...
            $row = $this->dbTable->SPUpdate('sp_journal_with_ar_sub_create',
                    $cs->getAddBy(),
            ...
            $row = $this->dbTable->SPUpdate('sp_journal_create',
                    $cs->getAddBy(),
                    $req->getXmlDataUnset($unsetDetailforXml),
                    $detailjournal['journaldetail_id'], 
                    $detailjournal['indexdata'], 
            ...
            $row = $this->dbTable->SPUpdate('sp_journal_with_escrow_create',
                    $cs->getAddBy(),
                    $req->getXmlDataUnset($unsetDetailforXml),
                    $detailjournal['journaldetail_id'], 
                    $detailjournal['indexdata'], 
            ...
            $row = $this->dbTable->SPUpdate('sp_journal_with_ar_create',
                    $cs->getAddBy(),
                    $req->getXmlDataUnset($unsetDetailforXml),
                    $detailjournal['journaldetail_id'], 
                    $detailjournal['indexdata'], 
          ...
          //  $amounts = rtrim($amounts, '.00');
            ...
            $row = $this->dbTable->SPUpdate('sp_journal_create', $cs->getAddBy(),
                    $req->getXmlDataUnset($unsetDetailforXml), 
                    $detailjournal['journaldetail_id'], 
                    $detailjournal['indexdata'],
                    $detailjournal['remarks'],
    ...
    public function update(Cashier_Models_Master_Journal $cs, Cashier_Box_Models_App_HasilRequestRead $req, $detailjournal, $subdetailjournal, $detailar, $deletedDetail, $deletedSubDetail) {
       ...
       $unsetDetailforXml = array('detailcoa', 'detailar', 'subdetailcoa', 'id','detailescrow','deletedsubRows'); 
        ...
        if (count($detailar)) { 
          $row = $this->dbTable->SPUpdate('sp_journal_with_ar_sub_update', $cs->getAddBy(),
                    $req->getXmlDataUnset($unsetDetailforXml), 
                    $detailjournal['journaldetail_id'], 
                    $detailjournal['indexdata'],
                    $detailjournal['remarks'],
                ...
                $row = $this->dbTable->SPUpdate('sp_journal_update', $cs->getAddBy(),
                    $req->getXmlDataUnset($unsetDetailforXml), 
                    $detailjournal['journaldetail_id'], 
                    $detailjournal['indexdata'],
                    $detailjournal['remarks'],
    ...
    public function getByProjectPtWithPageSearch(Cashier_Models_Master_Journal $ct, Cashier_Box_Models_App_HasilRequestRead $request) {
        ...
        if (strpos($xmlValue, 'pt_id') !== false){
            ...
            $xmlValue = str_replace("</root>","<pt_id>".$ptId."</pt_id>
                <project_id>".$projectId."</project_id>
                </root>",$xmlValue);
        }

        $hasil = $this->dbTable->SPExecute('sp_all_read_journal', $request->getModeRead(), $request->getModule(), $ct->getProject()->getId(), intval($ptId), $request->getPage(), $request->getLimit(), $xmlValue);
    ...
    public function getSubDetail(Cashier_Models_Transaction_Journalsubdetail $ct, Cashier_Box_Models_App_HasilRequestRead $request) {
        ...
        $pt = $ct->getPt()->getId();
        $ptId = (empty($request->getOthersValue("pt_id"))) ? $pt : $request->getOthersValue("pt_id");
        $hasil = $this->dbTable->SPExecute('sp_all_read', $request->getModeRead(), $request->getModule(), $ct->getProject()->getId(), intval($ptId), $request->getPage(), $request->getLimit(), $request->getXmlValue());
        return $hasil;
    }

    public function getSubeliminasiDetail(Cashier_Models_Transaction_Journalsubdetail $ct, Cashier_Box_Models_App_HasilRequestRead $request) {
        $hasil = array();
        $project = $ct->getProject()->getId();
        $pt = $ct->getPt()->getId();
        $ptId = (empty($request->getOthersValue("pt_id"))) ? $pt : $request->getOthersValue("pt_id");
    ...
    public function getSubDetailPaging(Cashier_Models_Transaction_Journalsubdetail $ct, Cashier_Box_Models_App_HasilRequestRead $request) {
        ...
        $pt = $ct->getPt()->getId();
        $ptId = (empty($request->getOthersValue("pt_id"))) ? $pt : $request->getOthersValue("pt_id");
        $hasil = $this->dbTable->SPExecute('sp_all_read', 'subdetailcoapaging', $request->getModule(), $ct->getProject()->getId(), intval($ptId), $request->getPage(), $request->getLimit(), $request->getXmlValue());
        return $hasil;
    }
    ...
    public function deleteData($user, $budgetdeletedId, Cashier_Box_Models_App_HasilRequestRead $req, $reasondelete = null) {
        $row = 0;

        if (!$user) {
            return $row;
        }
        $row = $this->dbTable->SPUpdate('sp_all_destroy', $req->getModeRead(), $req->getModule(), intval($user), $budgetdeletedId, $reasondelete);
    ...
    public function checkNoJournal(Cashier_Models_Master_Journal $ft, Cashier_Box_Models_App_HasilRequestRead $request) {

        $hasil = 0;

        $hasil = $this->dbTable->SPExecute('sp_validator_read', $request->getModeRead(), $request->getModule(), $ft->getProject()->getId(), $ft->getPt()->getId(), $request->getOthersValue('voucher_no'), $request->getOthersValue('voucher_date'));
        return $hasil;
    }

    public function getQuickDetail(Cashier_Models_Master_Journal $ft, Cashier_Box_Models_App_HasilRequestRead $request) {

        $hasil = 0;

        $hasil = $this->dbTable->SPExecute('sp_journalquickdetail_read', $request->getModeRead(), $request->getModule(), $request->getOthersValue('journal_id'));
        return $hasil;
    }

    public function checkmonthclosing(Cashier_Models_Master_Journal $ft, Cashier_Box_Models_App_HasilRequestRead $request) {
        
        $hasil = 0;

        $hasil = $this->dbTable->SPExecute('sp_validator_read', $request->getModeRead(), $request->getModule(), $ft->getProject()->getId(), $ft->getPt()->getId(), $request->getOthersValue('month'), $request->getOthersValue('year'));
        return $hasil;
    }

    public function journallog($params) {
        $hasil = $this->dbTable->SPExecute('sp_journal_log_read', $params['journal_id']);
        return $hasil;
    }
}
