...
class Cashier_Models_Typeloaninterest extends Zend_Db_Table_Abstract {
    ...
    function init() {
        ...
        $this->setting->_storeprocedure = 'sp_m_typeloan_d_interest';
    }

    function TypeloaninterestRead($param) {
    ...
    public function TypeloaninterestCreate($param) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            try {
                $this->setting->_param = $param;
    ...
    public function TypeloaninterestUpdate($param) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            try {
                $this->setting->_param = $param;
                $this->setting->_paramsql = 'update';
                $this->setting->_iddata = $param['typeloaninterest_id'];
                switch ($this->setting->_param['hideparam']) {
                    case 'detailupdate':
                        $result = $this->setting->executeSP();
    ...
    function TypeloaninterestDelete($param = array()) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            $key_name = 'typeloaninterest_id';
