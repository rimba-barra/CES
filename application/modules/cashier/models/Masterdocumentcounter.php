...
class Cashier_Models_Masterdocumentcounter extends Zend_Db_Table_Abstract {
    ...
    function init() {
        ...
        $this->setting->_storeprocedure = 'sp_documentcounter';
    }
    ...
    public function persentasepajakUpdate($param) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            try {
                $this->setting->_param = $param;
    ...
    function persentasepajakDelete($param = array()) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
