...
class Cashier_Models_VDPostingdetail extends Zend_Db_Table_Abstract {
    ...
    function VDPostingdetailRead($param) {
                ...
                switch ($this->setting->_param['hideparam']) {
                    ...
                    case 'search':
                        ...
                        if (!empty($result[3][0]['RECORD_TOTAL'])) {
                            $data = $result[4];
                            $totalamount = $result[5][0]['totalamount'];
                        } else {
                            $data = null;
                            $totalamount = 0;
                        }
    ...
    public function VDPostingdetailCreate($param) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            try {
                $this->setting->_param = $param;
    ...
    public function VDPostingdetailUpdate($param) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            try {
                $this->setting->_param = $param;
    ...
    function VDPostingdetailDelete($param = array()) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
