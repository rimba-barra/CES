        ...
        if ($dcResult) {
            $row = $this->dbTable->SPUpdate('sp_coaconfig_update', $cs->getId(), $cs->getAddBy(), 
                   $data["project_project_id"], 
                    $data["pt_pt_id"],
                    $cs->getCoaCode(), $cs->getName(), $cs->getDecription(), $cs->getNotes(),
                    $deletedRows, 
            ...
            $row = $this->dbTable->SPUpdate('sp_coaconfig_update', $cs->getId(), $cs->getAddBy(), $cs->getProject()->getId(), $cs->getPt()->getId(), $cs->getCoaCode(), $cs->getName(), $cs->getDecription(), $cs->getNotes(), $deletedRows);
            //$dcResult["coa_config_detail_id"]);
        }
...
//                }
            ...
            if ($template->th_kasbank_id) {
                ...
                $hasil = $this->dbTable->SPExecute('sp_coa_templatev2_read', $template->template, $template->amount, $template->th_kasbank_id, $template->session->getProject()->getId(),1,9999,
...
//                        $this->paymenttype);
                ...
                $hasil = $this->dbTable->SPExecute('sp_coa_config_template_read', 
                        ...
                        $this->scheduleId,
                        $request->getOthersValue('paymenttype_id'),
                        $request->getOthersValue('receipt'), 1,
                        $request->getOthersValue('project_id'));
...
//        var_dump($this->dbTable);
    ...
    public function getDetailByTemplatePajak(Cashier_Models_Master_CoaConfigDao $template, Cashier_Box_Models_App_HasilRequestRead $request, $paymenttype_id = NULL, $amount = NULL) {
        
        
        $hasil = $this->dbTable->SPExecute('sp_coa_config_template_pajak_read', 
                $request->getOthersValue('template_id'),
                $request->getOthersValue('amount'),
                $request->getOthersValue('pt_id'),
                $request->getOthersValue('project_id'),
                $request->getOthersValue('receipt'),
                $request->getOthersValue('remarks'),
                $request->getOthersValue('indexdata'),
                $request->getOthersValue('is_ppn'),
                $request->getOthersValue('is_pph'));
  ...
  public function getDetailByJournalTemplateGen(Cashier_Models_Master_CoaConfigDao $template, Cashier_Box_Models_App_HasilRequestRead $request, $paymenttype_id = NULL, $amount = NULL) {
                ...
                foreach ($unit_ids as $unit_id) {
                        ...
                        if($hasil[0][0]['totalRow'] > 0){
                             ...
                             foreach ($hasil[1] as $has) {
                                ...
                                //di kredit
                                $has['amountc'] = $has['amount'];
                                $has['amount'] = 0;
                                $has['subgl_description'] = $has['kelsub_kelsub'];
                                $has['subgl_subgl_id'] = 999;
                                $details[] = $has;
    ...
    public function getDetailByJournaleliminasiTemplate(Cashier_Models_Master_CoaConfigDao $template, $paymenttype_id = NULL, $amount = NULL) {
        $hasil = array();
        if ($template->template == "1") { //1 untuk installpayment default template, lainnya mutiple template 
            if ($template->th_kasbank_id) {

                $hasil = $this->dbTable->SPExecute('sp_coa_templatejournaleliminasi_read', $template->template, $template->amount, $template->th_kasbank_id, $template->session->getProject()->getId());
