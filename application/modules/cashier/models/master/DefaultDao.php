 ...
 * @author MIS
 */
class Cashier_Models_Master_DefaultDao extends Cashier_Box_Models_App_AbDaoCashier implements Cashier_Box_Models_App_BlackHole {
    ...
    public function getPt(Cashier_Box_Models_App_HasilRequestRead $request, $ses ){
     ...
     public function getReportList(Cashier_Box_Models_App_HasilRequestRead $r, Cashier_Models_Master_Reportbankpaymentvoucher $ut, $ses) {
        $hasil = array();
        $project = $r->getOthersValue('project_id');
        $pt = $r->getOthersValue('pt_id');
        $prefix = $r->getOthersValue('prefix_id');
        $range_id = $r->getOthersValue('range_id');
        $allpaymenttype = $r->getOthersValue('allpayment');
        $hasil = $this->dbTable->SPExecute('sp_report_bankpaymentvoucher', intval($pt), intval($project), intval($prefix), $r->getOthersValue('dataflow'), $r->getOthersValue('fromdate'), $r->getOthersValue('todate'),intval($range_id), 1,1,null,1,1,intval($allpaymenttype));
        return $hasil;
    }
