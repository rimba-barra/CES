...
class Cashier_Models_Masteroffbalancesheet extends Zend_Db_Table_Abstract {
    ...
    function init() {
        ...
        $this->setting->_storeprocedure = 'sp_masteroffbalancesheet';
    }

    public function MasteroffbalanceRead($param) {
    ...
    public function MasteroffbalanceCreate($param) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            try {
    ...
    public function MasteroffbalanceUpdate($param) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            try {
                $this->setting->_param = $param;
                $this->setting->_paramsql = 'update';
                $this->setting->_iddata = $param['off_balancesheet_id'];
    ...
    public function MasteroffbalanceDelete($param = array()) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            $key_name = 'off_balancesheet_id';
    ...
    public function MasteroffbalanceCopy($param) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            try {

                $this->setting->_param = $param;
                $this->setting->_paramsql = 'copy';
                $this->setting->_project_id = $param['project_id'] == "" ? $this->_project_id : $param['project_id'];
                $this->setting->_pt_id = $param['pt_id'] == "" ? $this->_pt_id : $param['pt_id'];
                switch ($this->setting->_param['hideparam']) {
                    case 'copydate':
