...
class Cashier_Models_Masterdebitsource extends Zend_Db_Table_Abstract {
    ...
    function init() {
        ...
        $this->setting->_storeprocedure = 'sp_m_debitsource';
    }

    public function MasterdebitsourceRead($param) {
    ...
    public function MasterdebitsourceCreate($param) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            try {
    ...
    public function MasterdebitsourceUpdate($param) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            try {
                $this->setting->_param = $param;
                $this->setting->_paramsql = 'update';
                $this->setting->_iddata = $param['debitsource_id'];
    ...
    public function MasterdebitsourceDelete($param = array()) {
        $return['success'] = false;
        // echo json_encode($param);die;
        if (is_array($param) && count($param)) {
            $key_name = 'debitsource_id';
            ...
            $debitsource_id = '';
            for ( $i = 0; $i < sizeof($param['debitsource_id']); $i++) { 
                    $debitsource_id .= $param['debitsource_id'][$i] . ',';
            }
            try {
                $this->setting->_iddata = rtrim($debitsource_id, ',');
