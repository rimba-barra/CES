...
class Cashier_Models_Deptaccess extends Zend_Db_Table_Abstract {
    ...
    function init() {
        ...
        $this->setting->_storeprocedure = 'sp_deptaccess';
    }
    ...
    function RoutesAllActions($param) {
        ...
        if (is_array($param) && count($param)) {
            try {
                ...
                if (isset($param['deptaccess_id'])) {
                    if ($param['deptaccess_id'] > 0) {
                        $this->setting->_iddata = $param['deptaccess_id'];
                ...
                switch ($this->setting->_param['hideparam']) {
                    ...
                    case 'create':      
                        ...
                        $data = array();
                        $counter = 1;
                        $message = 'Process create user access dept finish';
                        $valid = true;
                        break;
                    case 'update':
                        $param['user_id_login'] = $param['paramdata'][0]['user_id'];  
                    ...
                    case 'grantalldept':
                        $this->setting->_user_id = $param['user_id'];                         
                        $this->setting->_paramsql = 'grantalldept';
                        $this->setting->_param = $param;
                        $result = $this->setting->executeSP();
                        $data = array();
