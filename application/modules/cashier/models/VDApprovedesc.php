...
class Cashier_Models_VDApprovedesc extends Zend_Db_Table_Abstract {
    ...
    function init() {
        ...
        $this->setting->_storeprocedure = 'sp_td_voucherdesc';
    }

    function VDApprovedescRead($param) {
    ...
    public function VDApprovedescCreate($param) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            try {
                $this->setting->_param = $param;
    ...
    public function VDApprovedescUpdate($param) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            try {
                $this->setting->_param = $param;
                $this->setting->_paramsql = 'update';
                $this->setting->_iddata = $param['voucherdesc_id'];
                switch ($this->setting->_param['hideparam']) {
                    case 'descupdate':
                        $result = $this->setting->executeSP();
    ...
    function VDApprovedescDelete($param = array()) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            $key_name = 'voucherdesc_id';
