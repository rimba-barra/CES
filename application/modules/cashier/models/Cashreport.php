...
class Cashier_Models_Cashreport extends Zend_Db_Table_Abstract {
    ...
    protected function generatedataHeaderwithdetail($param) {
        ...
        if (!empty($result[0])) {
            try {
                ...
                return $this->function->showreportbyuser('sp_report_cashreport', $param);
            } catch (Exception $ex) {
            ...
            try {
                foreach ($result as $row) {
                    ...
                    $record = array(
                        ...
                        "description" =>  $this->txt(str_replace("'", "", $row['description'])),
                        "userid" => $this->_user_id,
                        "reportdate" => $this->txt($this->_curdate),
                    );
                    $return = $this->function->insertdata($record);
    ...
    function generatedatagGrandprojectpttotal($project_id, $pt_id) {
        $rowgrandprojectpt = $this->function->getgrandtotalprojectptkasbank('sp_report_cashreport', $project_id, $pt_id);
    ...
    protected function generatedatagGrandprojecttotal($project_id) {
        $rowgrandproject = $this->function->getgrandprojecttotalkasbank('sp_report_cashreport', $project_id);
        if ($rowgrandproject) {
    ...
    function generatedatasubheaderdepartment($param, $project_id, $pt_id) {
        $result = $this->function->getdeptbyprojectpt('sp_report_cashreport', $param, $project_id, $pt_id);
        if (!empty($result[0])) {
            try {
                foreach ($result as $row) {
                    $record = array(
    ...
    protected function generatedataaccountbydept($param, $data, $project_id, $pt_id, $department_id) {
        $result = $this->function->getcoabydeptprojectpt('sp_report_cashreport', $param, $project_id, $pt_id, $department_id);
        if (!empty($result[0])) {
            try {
                foreach ($result as $row) {
                    $record = array(
    ...
    protected function generatedetailbydeptcoa($param, $data, $project_id, $pt_id, $department_id, $coa_id) {
        $result = $this->function->getkasbankbycoadeptprojectpt('sp_report_cashreport', $param, $project_id, $pt_id, $department_id, $coa_id);
        if (!empty($result[0])) {
            try {
                foreach ($result as $row) {
                    if ($row['dataflow'] == 'I') {
    ...
    protected function totalSubaccountbydept($param, $data, $project_id, $pt_id, $department_id, $coa_id) {
        $result = $this->function->totalsubcoabydept('sp_report_cashreport', $param, $project_id, $pt_id, $department_id, $coa_id);
        if (!empty($result[0])) {
            try {
                foreach ($result as $row) {
    ...
    protected function totalSubdeptprojectpt($param, $data, $project_id, $pt_id, $department_id) {
        $result = $this->function->totalsubbydept('sp_report_cashreport', $param, $project_id, $pt_id, $department_id);
        if (!empty($result[0])) {
            try {
                foreach ($result as $row) {
    ...
    function totalSubcompany($data, $project_id, $pt_id) {
        $rowgrandprojectpt = $this->function->gettotalsubcompany('sp_report_cashreport', $project_id, $pt_id);
        if ($rowgrandprojectpt) {
            $description = $coaname = str_repeat("-", 50) . "SUB TOTAL COMPANY";
            $record = array(
                "project_id" => $project_id,
    ...
    function totalGrandproject($project_id) {
        $rowgrandprojectpt = $this->function->getgrandproject('sp_report_cashreport', $project_id);
        if ($rowgrandprojectpt) {
            $description = $coaname = str_repeat("-", 50) . "GRAND TOTAL";
            $record = array(
                "project_id" => $project_id,
    ...
    protected function generatesubHeaderaccount($param, $project_id, $pt_id) {
        ...
        $result = $this->function->getaccountheaderbyprojectpt('sp_report_cashreport', $param, $project_id, $pt_id);
    ...
    protected function generatedatadetailAccount($param, $data, $project_id, $pt_id, $coa_id) {
        $result = $this->function->getaccountkasbankbyprojectptcoa_id('sp_report_cashreport', $param, $project_id, $pt_id, $coa_id);
        if (!empty($result[0])) {
            try {
                foreach ($result as $row) {
                    if ($row['dataflow'] == 'I') {
                ...
                if (!empty($return[0])) {
                    $this->totalSubaccount($param, $data, $project_id, $pt_id, $coa_id);
                }
            } catch (Exception $ex) {
    ...
    protected function totalSubaccount($param, $data, $project_id, $pt_id, $coa_id) {
        $result = $this->function->totalsubcoa('sp_report_cashreport', $param, $project_id, $pt_id, $coa_id);
        if (!empty($result[0])) {
            try {
                foreach ($result as $row) {
                    $description = $coaname = str_repeat("-", 50) . "SUB TOTAL ACCOUNT";
