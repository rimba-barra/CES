...
class Cashier_Models_Banktransactionreport extends Zend_Db_Table_Abstract {
            ...
            try {
                ...
                switch ($parameter) {
                    ...
                    case 'processreport':
                        ...
                        break;
                    case 'generatefileexcel':
                        $counter = 0;
                        $result = $this->generatefileexcel($param);
                        break;
    ...
    function generatefileexcel($param){
        ...
        $ptname = $datapt[0][0]['name'];
                ...
                $additional['header'] = array( $ptname, 'KASBANK TRANSACTION REPORT', '', 'PERIOD : '.$param['fromdate'].' UNTIL '.$param['untildate']);
                $sheetname = '';
                $sheet = 0 ; 
                $titles = '';
                $sql = "sp_report_posisiKeuangan_Kas_Detail";
                $col = array('voucher_no', 'vid', 'coa','voucher_date','voucher_desc', 'receipt_no', 'debit', 'credit');
                $titles = array('VOUCHER NO', 'VOUCHER. ID', 'COA','VOUCHER DATE','VOUCHER DESCRIPTION', 'RECEIPT NO', 'DEBET','KREDIT');

                $paramdata = array($param['pt_id'],$param['project_id'],$param['fromdate'],$param['untildate'],$param['dept_id'],$param['coastart_id'],$param['coaend_id'],$param['prefix_id'],$param['debet_kredit'],$param['sort_by'],$param['is_liquid'],$param['detail_coa'],$param['paymentmethod'],$param['banktype_id']);
                $sheetname = "Data";
       

        $doc = $this->genReportdataexcel($param, $sql, $sheet,$sheetname, $additional, $paramdata, $col,  $titles, FALSE);
        ...
        //STYLE SHEET 0
            ...
            $doc->getActiveSheet()->getStyle('G3:H100000')->getNumberFormat()->setFormatCode('#,##0.00');
            $doc->getActiveSheet()->getStyle("A2:H2")->applyFromArray($titleArraystyle);
            $doc->getActiveSheet()->getRowDimension('2')->setRowHeight(30);
            $doc->getActiveSheet()->getStyle("A3:H70000")->getFont()->setSize(9);
            $doc->getActiveSheet()->getColumnDimension('A')->setWidth(20);
            $doc->getActiveSheet()->getColumnDimension('B')->setWidth(20);
            $doc->getActiveSheet()->getColumnDimension('C')->setWidth(15);
            $doc->getActiveSheet()->getColumnDimension('D')->setWidth(15);
            $doc->getActiveSheet()->getColumnDimension('E')->setWidth(50);
            $doc->getActiveSheet()->getColumnDimension('F')->setWidth(15);
            $doc->getActiveSheet()->getColumnDimension('G')->setWidth(15);
            $doc->getActiveSheet()->getColumnDimension('H')->setWidth(20);

        $this->genReportdataexcel($param, $sql, $sheet,$sheetname, $additional, $paramdata, $col,  $titles, TRUE);
        return $this->_tmpparam;
    }

    function genReportdataexcel($param, $sql, $sheet, $sheetname, $additional, $paramdata, $col,  $titles, $isReady) {
        ...
        if($isReady == FALSE){
            ...
            if(sizeof($datas)>0){
                $arrayKeys = array('voucher_no', 'vid', 'kasbon_no', 'coa', 'voucher_date', 'voucher_desc', 'receipt_no', 'debit', 'credit');
                // print_r($arrayKeys);die;
            }else{
                $arrayKeys = 0;
            }
           

            $tmp = array();$final = array();
        ...
        if($isReady == TRUE){
            ...
            $filename = "KasbankTransReport".date("Y-m-d_H-i-s").".xls";
    ...
    function generateExcel($param){ // Ini ga tau buat apa buat format excel nya dibuat di generatefileexcel
