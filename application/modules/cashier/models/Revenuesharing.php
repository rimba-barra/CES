...
class Cashier_Models_Revenuesharing extends Zend_Db_Table_Abstract
{
        ...
        if (is_array($param) && count($param)) {
            try {
                $data = array(
                    ...
                    isset($param['block_id']) ? $param['block_id'] : 0,
                    $param['unit_number'],
                    $param['customer_name'],
        ...
        if (is_array($param) && count($param)) {
            try {
                ...
                $data = array(
                    ...
                    $param['rangebagihasil_detail_id'],
                    $this->session->getCurrentProjectId(),
                    $this->session->getCurrentPtId(),
    ...
    function revenuesharingselecteddetailRead($param){
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            try {
                $data = array(
                ...
                $result            = $this->execSP3('sp_revenuesharing_selected_detail_read', $data);
                // echo json_encode($result);die;
                $return['rangebagihasil_detail_id'] = $result[0];
                $return['success']                  = (bool) $result;
            } catch (Exception $e) {
            }
        }
