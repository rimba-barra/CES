...
class Cashier_Models_Mergesubcoasubdetail extends Zend_Db_Table_Abstract {
    ...
    function init() {
        ...
        $this->setting->_storeprocedure = 'sp_mergesubcoasubdetail';
    }

    function MergesubcoasubdetailRead($param) {
            ...
            try {
                ...
                switch ($this->setting->_param['hideparam']) {
                    ...
                    case 'search':
                        ...
                        break;
                     case 'findduplicate':
                        $this->setting->_paramsql = 'findduplicate';
                        $this->setting->_project_id = $param['project_id'];
                        $this->setting->_pt_id = $param['pt_id'];
                        $result = $this->setting->executeSP();
                        $counter = $result[1][0]['RECORD_TOTAL'];
    ...
    public function MergesubcoasubdetailCreate($param) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            try {
                $this->setting->_param = $param;
    ...
    public function MergesubcoasubdetailUpdate($param) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            try {
                $this->setting->_param = $param;
                $this->setting->_paramsql = 'update';
                $this->setting->_iddata = $param['Mergesubcoasubdetail_id'];
                switch ($this->setting->_param['hideparam']) {
    ...
    function MergesubcoasubdetailDelete($param = array()) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            $key_name = 'Mergesubcoasubdetail_id';
