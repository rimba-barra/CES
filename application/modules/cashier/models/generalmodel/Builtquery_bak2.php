...
class Cashier_Models_Generalmodel_Builtquery_bak2 extends Zend_Db_Table_Abstract {
    ...
    public function changeneracaheadertochield($param) {
        $report = $param[2];
        ...
        $sql = "
                IF OBJECT_ID(''tempdb..##tmp_setlevel'') IS NOT NULL DROP TABLE ##tmp_setlevel
                    ...
                    (   
                        ...
                        LEFT JOIN ##tmp_setcalculate b ON  a.coa = b.coa AND a.flag = b.flag
                        WHERE 
                            a.active = 1
                            AND a.deleted = 0
    ...
    function rpt_balancesheet_setcalculate() {
        $sql = "
                 IF OBJECT_ID(''tempdb..##tmp_setcalculatebalancesheet0'') IS NOT NULL DROP TABLE ##tmp_setcalculatebalancesheet0
                    SELECT * INTO ##tmp_setcalculatebalancesheet0 FROM
                    (                    
                       
                        SELECT 
                                *,
                                case when type=''D'' then coalesce(lastmonth_amount,0) else -(coalesce(lastmonth_amount,0)) end AS cal_lastamount,
                                case when type=''D'' then coalesce(thismonth_amount,0) else -(coalesce(thismonth_amount,0)) end AS cal_thisamount,
                                case when type=''D'' then coalesce(budget_amount,0) else -(coalesce(budget_amount,0)) end AS cal_budgetamount,
                                case when type=''D'' then coalesce(lastyear_amount,0) else -(coalesce(lastyear_amount,0)) end AS cal_lastyear
    ...
    function rpt_balancesheet_setcalculate_replace() {
        $sql = "
                 IF OBJECT_ID(''tempdb..##tmp_setcalculatebalancesheet'') IS NOT NULL DROP TABLE ##tmp_setcalculatebalancesheet
                    ...
                    (                    
                        ...
                        SELECT 
                                *,
                                CASE 		
                                        WHEN cal_lastamount < 0 and  coa >=''20.00.000'' then REPLACE(cal_lastamount, ''-'', '''')
                                        WHEN cal_lastamount > 0 and  coa >=''20.00.000'' then -(coalesce(cal_lastamount,0))
                                        ELSE cal_lastamount
                                END as calculate_lastamount,
                                
                                CASE 		
                                        WHEN cal_thisamount < 0 and  coa >=''20.00.000'' then REPLACE(cal_thisamount, ''-'', '''')
                                        WHEN cal_thisamount > 0 and  coa >=''20.00.000'' then -(coalesce(cal_thisamount,0))
                                        ELSE cal_lastamount
                                END as calculate_thisamount,
                                
                                CASE 		
                                        WHEN cal_budgetamount < 0 and  coa >=''20.00.000'' then REPLACE(cal_budgetamount, ''-'', '''')
                                        WHEN cal_budgetamount > 0 and  coa >=''20.00.000'' then -(coalesce(cal_budgetamount,0))
                                        ELSE cal_budgetamount
                                END as calculate_budgetamount,
                                
                                CASE 		
                                        WHEN cal_lastyear < 0 and  coa >=''20.00.000'' then REPLACE(cal_lastyear, ''-'', '''')
                                        WHEN cal_lastyear > 0 and  coa >=''20.00.000'' then -(coalesce(cal_lastyear,0))
                                        ELSE cal_lastyear
