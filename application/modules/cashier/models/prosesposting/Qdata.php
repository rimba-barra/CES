...
class Cashier_Models_Prosesposting_Qdata extends Zend_Db_Table_Abstract {
        ...
        $sql = "
                ...
                SELECT * INTO $this->_tmp_header FROM
                 ( 
                       SELECT 
                             *,
                             case when type=''D'' then  coalesce(calculate_lastmonth,0) else -(coalesce(calculate_lastmonth,0)) end AS factor_lastmonth,
                             case when type=''D'' then  coalesce(calculate_thismonth,0) else -(coalesce(calculate_thismonth,0)) end AS factor_thismonth,
                             case when type=''D'' then  coalesce(calculate_budget,0) else -(coalesce(calculate_budget,0)) end AS factor_budget,
                             case when type=''D'' then  coalesce(calculate_lastyear,0) else -(coalesce(calculate_lastyear,0)) end AS factor_lastyear
                             FROM $this->_tmp_rptincomestatement
                       WHERE 
                            project_id = $this->_project_id 
                            AND pt_id =$this->_pt_id 
