...
class Cashier_Models_Cashfloweditor extends Zend_Db_Table_Abstract {
    ...
    function init() {
        ...
        $this->setting->_storeprocedure = 'sp_cashfloweditor';
    }

    public function CashfloweditorRead($param) {
    ...
    public function CashfloweditorCreate($param) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            try {
    ...
    public function CashfloweditorUpdate($param) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            try {
                $this->setting->_param    = $param;
                $this->setting->_paramsql = 'update';
                if($this->setting->_param['hideparam'] == 'fillcashflow'){
                    $this->setting->_iddata = 1;
                }else{
                    $this->setting->_iddata = $param['journaldetail_id'];
                }
                if ($this->setting->_iddata > 0) {
                    switch ($this->setting->_param['hideparam']) {
                        case 'default': 
                            ...
                            break;
                        case 'fillcashflow': 
                            $this->setting->user_id = $this->_user_id;
                            $result                 = $this->setting->executeSP();
                            $valid                  = $result[0][0]['VALIDDATA'];
                            $counter                = $result[1][0]['RECORD_TOTAL'];
                            $message                = $result[2][0]['MSG'];
    ...
    public function CashfloweditorDelete($param = array()) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
