...
class Cashier_Models_Deptprefixdetail extends Zend_Db_Table_Abstract {
    ...
    function init() {
        ...
        $this->setting->_storeprocedure = 'sp_m_deptprefix_d';
    }

    function DeptprefixdetailRead($param) {
    ...
    public function DeptprefixdetailCreate($param) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            try {
                $this->setting->_param = $param;
    ...
    public function DeptprefixdetailUpdate($param) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            try {
                $this->setting->_param = $param;
                ...
                $this->setting->_project_id = $param['project_id'];
                $this->setting->_pt_id = $param['pt_id'];
                $this->setting->_iddata = $param['deptprefixdetail_id'];                
                switch ($this->setting->_param['hideparam']) {
                    case 'detailupdate':
                        $result = $this->setting->executeSP();
    ...
    function DeptprefixdetailDelete($param = array()) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            $key_name = 'deptprefixdetail_id';
