...
class Cashier_Models_Vdtransaction extends Zend_Db_Table_Abstract {
    ...
    function Create($param) {
      ...
      function generateExcel($param){
        ...
        $ptname = $datapt[0][0]['name'];
                ...
                $col = array('voucher_no', 'coa','voucher_date','department', 'kelsub', 'd_description', 'sub_code','vendorname','dataflow','voucher_no_kasir', 'due_date','status','is_pjk','kasbon_no','amount');
                $titles = array('VOUCHER NO / NO. URUT', 'COA','VOUCHER DATE','DEPARTMENT', 'SUB GROUP', 'KETERANGAN', 'SUB ACC', 'VENDOR NAME','DATAFLOW','VOUCHER NO. KASIR', 'DUE DATE', 'STATUS','TKB/PJK','KASBON NO.','JUMLAH');     

                $paramdata = array($param['pt_id_from'],$param['pt_id_to'],$param['project_id'],$param['fromvendorid'],$param['untilvendorid'],$param['fromdeptid'],$param['untildeptid'],$param['transactiontype'],$param['groupby'],$param['dataflow'],$param['detailcoa'],$param['fromdate'],$param['untildate'],$param['userid'],$param['status']);
        ...
        $highrow = $doc->getActiveSheet()->getHighestRow();

        if ($highrow <= 3) {
            $highrow = 10;
        }

            $doc->getActiveSheet()->getStyle('C3:O'.$highrow)->getNumberFormat()->setFormatCode('#,##0.00');
            $doc->getActiveSheet()->getStyle("A2:O2")->applyFromArray($titleArraystyle);
            $doc->getActiveSheet()->getRowDimension('2')->setRowHeight(30);
            $doc->getActiveSheet()->getStyle("A3:O{$highrow}")->getFont()->setSize(9);
            ...
            $doc->getActiveSheet()->getColumnDimension('O')->setWidth(40);
