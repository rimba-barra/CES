<?php

ini_set("memory_limit", "-1");
ini_set('max_execution_time', 0);

class Cashier_Models_Bankloan extends Zend_Db_Table_Abstract {

    private $setting = null;
    private $function = null;

    function init() {
        date_default_timezone_set('Asia/Jakarta');
        $this->_curdate = date('Y-m-d');
        $this->_curdatetime = date('Y-m-d H:i:s');
        $this->setting = new Cashier_Models_General_Setdata;
        $this->setting->_storeprocedure = 'sp_th_bank_loan_read';
    }

    function RoutesAllActions($param) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            try {
                $this->setting->_param = $param;
                $this->setting->_paramsql = 'read';

                if (isset($param['param_id'])) {
                    if ($param['param_id'] > 0) {
                        $this->setting->_iddata = $param['param_id'];
                        $this->setting->_param['hideparam'] = 'update';
                    } else {
                        $this->setting->_param['hideparam'] = 'create';
                    }
                }

                switch ($this->setting->_param['hideparam']) {
                    case 'default':
                        $result = $this->setting->executeSP();
                        $data = $result[1];
                        $counter = $result[0][0]['RECORD_TOTAL'];
                        $message = null;
                        $valid = true;
                        $bank_loan_id = 0;
                        break;
                    case 'search':
                        $this->setting->_pt_id = $param['pt_id'];
                        $this->setting->_project_id = $param['project_id'];
                        $result = $this->setting->executeSP();
                        $data = $result[1];
                        $counter = $result[0][0]['RECORD_TOTAL'];
                        $message = null;
                        $valid = true;
                        $bank_loan_id = 0;
                        break;
                    case 'create':
                        $this->setting->_pt_id = $param['pt_id'];
                        $this->setting->_project_id = $param['project_id'];
                        $this->setting->_storeprocedure = 'sp_th_bank_loan_create';
                        $this->setting->_paramsql = 'create';
                        $result = $this->setting->executeSP();
                        $data = array();
                        $valid = $result[1][0]['VALIDDATA'];
                        $counter = $result[2][0]['RECORD_TOTAL'];
                        $message = $result[3][0]['MSG'];
                        $bank_loan_id = $result[4][0]['bank_loan_id'];
                        break;
                    case 'update':
                        $this->setting->_pt_id = $param['pt_id'];
                        $this->setting->_project_id = $param['project_id'];
                        $this->setting->_storeprocedure = 'sp_th_bank_loan_update';
                        $this->setting->_paramsql = 'update';
                        $result = $this->setting->executeSP();
                        $data = array();
                        $valid = $result[1][0]['VALIDDATA'];
                        $counter = $result[2][0]['RECORD_TOTAL'];
                        $message = $result[3][0]['MSG'];
                        $bank_loan_id = $result[4][0]['bank_loan_id'];
                        break;
                    case 'generate':
                        $this->setting->_pt_id = $param['pt_id'];
                        $this->setting->_project_id = $param['project_id'];
                        $this->setting->_storeprocedure = 'sp_th_bank_loan_generate';
                        $this->setting->_paramsql = 'generate';
                        $result = $this->setting->executeSP();
                        // echo json_encode($result);die;
                        if ( $result[0][0]['result'] == 0 ) {
                            $bank_loan_id = 0;
                            $valid = false;
                            $counter = 0;
                            $data = [];
                            $message = $result[1][0]['MSG'];
                        }else{
                            $bank_loan_id = 0;
                            $valid = true;
                            $counter = $result[1][0]['RECORD_TOTAL'];
                            $data = $result[2];
                            $message = $result[3][0]['MSG'];
                        }
                    break;
                    case 'allowed_generated':
                        $this->setting->_pt_id = $param['pt_id'];
                        $this->setting->_project_id = $param['project_id'];
                        $this->setting->_storeprocedure = 'sp_th_bank_loan_check';
                        $this->setting->_paramsql = 'check';
                        $result = $this->setting->executeSP();
                        // echo json_encode($result);die;
                        $data = $result[0][0]['result'];
                        $message = $result[1][0]['msg'];
                        $counter = 0;
                        $valid = true;
                        $bank_loan_id = 0;
                    break;
                    case 'get_role':
                        $this->setting->_pt_id = $param['pt_id'];
                        $this->setting->_project_id = $param['project_id'];
                        $this->setting->_storeprocedure = 'sp_th_bank_loan_check';
                        $this->setting->_paramsql = 'check';
                        $result = $this->setting->executeSP();
                        // echo json_encode($result);die;
                        $data = $result[0];
                        $message = '';
                        $counter = 0;
                        $valid = true;
                        $bank_loan_id = 0;
                    break;
                    default:
                        $result = null;
                        $valid = true;
                        $counter = 0;
                        $message = 'no action';
                }

                $return = array(
                    "success" => $valid,
                    "data" => $data,
                    "msg" => $message,
                    "total" => $counter,
                    "counter" => $counter,
                    "parameter" => $param['hideparam'],
                    "bank_loan_id" => $bank_loan_id,
                );
            } catch (Exception $e) {
                
            }
        }
        return $return;
    }

    function deleteData($param = array()) {
        $return['success'] = false;
        if (is_array($param) && count($param)) {
            $key_name = 'bank_loan_id';
            $param[$key_name] = isset($param[$key_name]) ? $param[$key_name] : '';
            foreach ($param as $key => $val) {
                if (is_array($val)) {
                    $param[$key_name] .= $val[$key_name] . ',';
                }
            }
            try {
                $this->setting->_storeprocedure = 'sp_th_bank_loan_delete';
                $this->setting->_iddata = $param[$key_name];
                $this->setting->_paramsql = 'delete';
                $result = $this->setting->executeSP();
                $valid = $result[2][0]['VALIDDATA'];
                $counter = $result[3][0]['RECORD_TOTAL'];
                $message = $result[4][0]['MSG'];

                $return = array(
                    "success" => $valid,
                    "total" => $counter,
                    "msg" => $message,
                );
            } catch (Exception $e) {
                
            }
        }
        return $return;
    }

}
