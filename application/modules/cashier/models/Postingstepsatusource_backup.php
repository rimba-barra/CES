    ...
    private function createNormaldetail($param, $data) {
        ...
        if ($counter > 0) {
            ...
            foreach ($resultdetail[2] as $row) {
                ...
                if ($coa_tmp == 9999999) {
                    $coa_id = 0;
                    $resultcoa = $this->generaldata->getcoa($project_id, $pt_id, $coa);
                    if ($resultcoa > 0) {
                ...
                if ($rowheader > 0) {
                    ...
                    if ($counterdetail > 0) {
                        ...
                        foreach ($resultdetail[2] as $row) {
                            ...
                            $this->setting->_storeprocedure = 'sp_th_journal';
                            $this->setting->_param['hideparam'] = 'journaldetail';
                            $result2 = $this->setting->executeSP();

                            if ($result2[1][0]['VALIDDATA'] == 'true') {
                                $this->createJournalsubdetail($param, $data, $result2[5][0], $iddetailkey, $iddetaildata);
                                $valid = true;
                                $message = "Create journal detail success";
                                $total = 0;
                            } else {
