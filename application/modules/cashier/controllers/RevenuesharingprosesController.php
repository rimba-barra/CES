...
class Cashier_RevenuesharingprosesController extends Zend_Controller_Action {
	...
	function readAction() {
		...
		$model = new Cashier_Models_Revenuesharingproses();
		...
		if ($read_type_mode == 'export_excel') {
			...
			$userDeleteRS = Cashier_Box_Projectptconfig_ProjectPtConfigSelector::getGeneralConfig($this->session->getCurrentProjectId(), $this->session->getCurrentPtId())->getUserDeleteRS();
	...
	function createAction() {
		...
		$paymentflagprosesrs = $model->getGlobalParam($senddata);

		$post_data['payment_flag'] = $paymentflagprosesrs['data'][0]['result'];
	...
	public function exportdata() {
		...
		$param = [
			...
			'payment_flag' => $paymentflagprosesrs['data'][0]['result']
		];
...
//		print_r($result[0]);
		...
		$jsonFile = APPLICATION_PATH . '/../public/app/cashier/json/' . $fileName . '.json';
...
//		 $jsonExcel = new Cashier_Models_Library_JsonToExcelWithTemplate();
		...
		$jsonExcel->fileTemplate = APPLICATION_PATH . '/../public/app/cashier/uploads/exceltemplate/RevenueSharingProsesNew.xlsx';
...
//		 $jsonExcel = new Cashier_Models_Library_JsonToExcelWithTemplate();
		...
		if ($post_data['rg_print'] == "detail_rs") {
			$jsonExcel->fileTemplate = APPLICATION_PATH . '/../public/app/cashier/uploads/exceltemplate/RevenueSharingDetailRS.xlsx';
			...
			$jsonExcel->fileTemplate = APPLICATION_PATH . '/../public/app/cashier/uploads/exceltemplate/RevenueSharingDetailTerjual.xlsx';
			$jsonExcel->fieldAwal = "RowNum";
			$firstRow = 5;
			$columnSumStart[] = ['O', 'AA'];
			$columnSumStart[] = ['AE', 'AK'];
		} else if ($post_data['rg_print'] == "rekap_terjual") {
			$jsonExcel->fileTemplate = APPLICATION_PATH . '/../public/app/cashier/uploads/exceltemplate/RevenueSharingRekapTerjual.xlsx';
			$jsonExcel->fieldAwal = "RowNum";
			$firstRow = 5;
			$columnSumStart[] = ['D', 'AF'];
		}
...
//			$objPHPExcel->getActiveSheet()->getColumnDimension($columnID)->setAutoSize(true);
		...
		$objWriter->save(APPLICATION_PATH . '/../public/app/cashier/downloadfile/msexcel/' . $fileResult);
		$url = 'app/cashier/downloadfile/msexcel/' . $fileResult;
