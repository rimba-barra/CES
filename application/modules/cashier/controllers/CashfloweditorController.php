...
class Cashier_CashfloweditorController extends Zend_Controller_Action {

    function init() {
        $this->session = Zend_Controller_Action_HelperBroker::getStaticHelper('session');
        $this->session->set('selected_dbapps', 'gl_2018');
        $this->_model = new Cashier_Models_Cashfloweditor();
    }
    ...
    function readAction() {
        ...
        $result                      = $this->_model->CashfloweditorRead($post_data);
    ...
    function createAction() {
        ...
        $result = $this->_model->CashfloweditorCreate($post_data);
        echo Zend_Json::encode($result);
        $this->_helper->viewRenderer->setNoRender(true);
    }
    ...
    function updateAction() {
        ...
        $result               = $this->_model->CashfloweditorUpdate($post_data);
        echo Zend_Json::encode($result);
        $this->_helper->viewRenderer->setNoRender(true);
    }
    ...
    function deleteAction() {
        ...
        $result    = array('data' => array(), 'total' => 0, 'success' => false);
        $post_data = Zend_Json::decode($this->getRequest()->getPost('data'));
        $result    = $this->_model->CashfloweditorDelete($post_data);
