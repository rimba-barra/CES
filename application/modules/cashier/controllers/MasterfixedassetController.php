...
class Cashier_MasterfixedassetController extends Zend_Controller_Action {

    function init() {
        $this->session = Zend_Controller_Action_HelperBroker::getStaticHelper('session');
        $this->_model = new Cashier_Models_Masterfixedasset();
    }
    ...
    function readAction() {
        ...
        $post_data['src_description'] = $this->getRequest()->getPost('src_description');
        $post_data['hideparam'] = $this->getRequest()->getPost('hideparam');
        
        $result = $this->_model->MasterfixedassetRead($post_data);
    ...
    function createAction() {
        ...
        $result = $this->_model->MasterfixedassetCreate($post_data);
        echo Zend_Json::encode($result);
        $this->_helper->viewRenderer->setNoRender(true);
    }
    ...
    function updateAction() {
        ...
        $result = $this->_model->MasterfixedassetUpdate($post_data);
        echo Zend_Json::encode($result);
        $this->_helper->viewRenderer->setNoRender(true);
    }
    ...
    function deleteAction() {
        ...
        $result = array('data' => array(), 'total' => 0, 'success' => false);
        $post_data = Zend_Json::decode($this->getRequest()->getPost('data'));
        $result = $this->_model->MasterfixedassetDelete($post_data);
