...
class Cashier_PenerimaancollectionreportController extends Zend_Controller_Action {
	...
	function exportAction() {
		...
		$model = new Cashier_Models_Penerimaancollectionreport();

		$result = $model->penerimaancollectionreportRead($this->getRequest()->getPost());
		$result = $result['data'];
		...
		if (count($result) > 0) {
			...
			$jdl = $this->getRequest()->getPost('radio_filtertype') == 2 ? 'Collection SH2' : 'Penerimaan Collection';

			$fileResult = $jdl . ' Report_' . time() . '.xls';
			$objWriter->save(APPLICATION_PATH . '/../public/app/cashier/uploads/' . $fileResult);
			$url = 'app/cashier/uploads/' . $fileResult;
