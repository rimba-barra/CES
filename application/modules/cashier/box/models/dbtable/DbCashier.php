...
class Cashier_Box_Models_Dbtable_DbCashier extends Zend_Db_Table_Abstract {
    ...
    public function __construct($config = array()) {
        $this->_schema = 'cashier';
   
        return parent::__construct($config);
    }

    public function setSchemaDb($schema)
    {
        $this->_schema = $schema;
    }
    ...
    private function logError() {
        $file = 'cashier_log.txt';
        $file = APPLICATION_PATH . '/../public/app/cashier/log/' . $file;
    ...
    public function logErrorPhp($msg) {
        $file = 'cashier_log.txt';
        $file = APPLICATION_PATH . '/../public/app/cashier/log/' . $file;
        $current = file_get_contents($file);
        $current .= "[" . date("d-m-Y H:i:s") . "][MSG] " . $msg . "\r\n";
        $current .= "------------------------------------------------------------------------------------------------------------------". "\r\n";   
        file_put_contents($file, $current);
    }
    ...
    public function SPUpdateArray($args) {
        $row = 0;
        $args = func_get_args();
    ...
    public function SPUpdateArrayforReal($args) {
        $row = 0;
        $args = func_get_args();
        $stmt = (array) $this->runSP($this->convertParams($args), TRUE);
        
        if ($stmt[0][0]['result'] != 1) {
           $this->logError();
        }
