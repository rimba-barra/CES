 ...
 * @author MIS
 */
class Cashier_Box_Models_App_Models_ReadWorms extends Cashier_Box_Models_App_AbModel {
    ...
    public function __construct($controller, $debug = '') {
        ...
        $this->hasil = new Cashier_Box_Models_App_HasilReadExtraModel();
        ...
        if (key_exists($name, $this->dataList)) {
            ...
            if (count($dataDao) > 0) {
                ...
                foreach ($dl->getList() as $row) {
                    /* added 23 Mei */
                    if($row instanceof Cashier_Box_Models_App_InterInsertable){
    ...
    public function getMasterData(Cashier_Box_Models_App_Masterdata_Main $main){
