 ...
 */
class Erems_Box_Library_Tagihan{
    private $id = NULL;
    ...
    public function addCicilan($c){
        if(get_class($c)=='Erems_Box_Library_Cicilan'){
            $this->list_cicilan[] = $c;
        }
    ...
    public function import_from_array($data = array()){
        ...
        foreach($data as $row){
            ...
            $this->list_cicilan[] = new Erems_Box_Library_Cicilan($row['schedule_id'],$row['amount'],$row['remaining_balance']);
        }
