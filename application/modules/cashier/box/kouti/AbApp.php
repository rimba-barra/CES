...
abstract class Cashier_Box_Kouti_AbApp {
    ...
    public function setHasil(Cashier_Box_Kouti_InterHasil $interHasil) {
        $this->hasil = $interHasil;
    }

    public function setSession(Cashier_Box_Kouti_InterSession $interSession) {
    ...
    protected final function prosesZendSession() {
        ...
        $sesCiputra = new Zend_Session_Namespace('Ciputra');

        $ourSes = $this->session;
        if ($ourSes instanceof Cashier_Box_Kouti_InterSession) {
            if ($ses != NULL) {
                ...
                $ourSes->setSession('group_list', $sesCiputra->common['group']);
                $ourSes->setSession('group', $ses->getCurrentGroupId());
