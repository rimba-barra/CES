...
class IndexController extends Zend_Controller_Action
{
	...
	{	
		echo "Hello";
		die();
		$http_user_agent = get_browser($_SERVER['HTTP_USER_AGENT']); $browserng = 0;
		...
		{	
			...
			if (!is_array($userAgent[$userAgentRestrictMode])) return;
			$userAgentArray = $userAgent[$userAgentRestrictMode];				
			foreach($userAgentArray as $browser)
			{ 
				if (($userAgentRestrictMode=='block' && strcasecmp($browser['browser'],$http_user_agent->browser)==0) || ($userAgentRestrictMode=='allow' && strcasecmp($browser['browser'],$http_user_agent->browser)!=0)) $browserng++; 
			}
	...
	function loginAction()
	{
		...
		if (!isset($_COOKIE['uid'])) { $this->_helper->session->destroy(); }
	}
	...
	function mainAction()
	{
		...
		{						
			...
			if (strlen($apps_name)>=33){ $modulemenu[$module_id]['tooltip'] = $apps_name; }
	...
	function changepasswordAction()
	{
		...
		if ($result){ $newpass = md5($newpass); $this->_helper->session->setUserPassEnc($newpass); echo '{"success":true}'; } else { echo '{"success":false}'; }		
		$this->_helper->viewRenderer->setNoRender(true);
	}
	...
	function homeAction()
	{
		...
		try { $module_config = Zend_Registry::get($info['apps_module'].'_config'); $info['apps_version'] = isset($module_config['module']['version']) ? $module_config['module']['version'] : ''; } catch(Exception $e) { $info['apps_version'] = ''; }
		$model_auth = new Main_Models_Auth();	
		$depend = $model_auth->getAppsDepend($req['apps']); $appsdepend = '';
		if (is_array($depend)){ foreach($depend as $val){ if ($val['active']) { $appsdepend .= ucfirst($val['depend_basename']).":'app/".$val['depend_basename']."',"; } } $appsdepend = preg_replace('/(,)$/','',$appsdepend); }
		
		$menudata = $model_auth->getUserMenu($req['apps'], $req['code']);
		
		$controllers = array();		
		if (count($menudata)){ foreach($menudata as $val){ $con = $val['controller_name'];	if ($con && is_file(APPLICATION_PATH.'/../public/app/'.$info['apps_module'].'/controller/'.$con.'.js')){ $controllers[] = $con; } } $menu = Zend_Json::encode($this->buildHomeMenu($menudata), false, array('enableJsonExprFinder' => true)); } else { $menu = "{text:'--- No Menu ---', disabled:true}"; } $controllers = Zend_Json::encode(array_unique($controllers), false, array('enableJsonExprFinder' => true));
		
		$actiondata = $model_auth->getUserAction($req['apps'], $req['code']); $actions = array();
		foreach($actiondata as $arr) { if (!$arr['disabled']) { $actions[$arr['action_name']] = $arr['action_basename']; } }
		...
		foreach ($arr as $a) {
			...
			if ($id == $a['menu_parent']) {
				...
				{				
					...
					if (count($submenu)) { $b['menu']['items'] = $submenu; } else { 
						$b['leaf'] = true;
						if ($a['widget']){
	...
	protected function buildObjectsArray(&$arr, $id = 0)
	{
		...
		foreach ($arr as $a) {
			...
			if ($id == $a['object_parent'] && !$a['disabled']) {
				...
				if (count($submenu)) { $b[$a['object_name']] = $submenu; } else { $b = $a['object_name']; }
				$result[] = $b;
			}				
		}
		return $result;
