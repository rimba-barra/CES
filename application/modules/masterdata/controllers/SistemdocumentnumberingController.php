...
class Masterdata_SistemdocumentnumberingController extends Zend_Controller_Action
{
	protected $model;
	protected $result = array('total'=>0, 'success'=>false, 'data'=>array(), 'message'=>'');
	
	function init()
	{
		$this->getResponse()->setHeader('Content-Type', 'application/json');
		$this->model = new Masterdata_Models_Sistemdocumentnumbering();
	}
	...
	function getsistemdocnocounterAction()
	{
		$this->result = $this->model->getsistemdocnoCounter($this->getRequest()->getPost());
		echo Zend_Json::encode($this->result);
		$this->_helper->viewRenderer->setNoRender(true);
	}	
	...
	{
		$report_fn = 'report_master_documents.mrt';				
