 ...
 * @author MIS
 */
class Master_Box_Models_App_Hermes_Processor {
    ...
    protected function switchModeRead(Master_Box_Models_App_Models_ReadWorms $app, Master_Box_Models_App_Controller $controller) {
    ...
    public function doRead(Master_Box_Models_App_Hermes_AbstractController $controller) {
        $app = new Master_Box_Models_App_Models_ReadWorms($controller, $this->getSampleData());
    ...
    public function doDelete(Master_Box_Models_App_Hermes_AbstractController $controller) {
        $dataModel = NULL;
        ...
        $app = new Master_Box_Models_App_Models_Delete($controller);
        $app->setIdProperty($dataModel->getIdProperty());
        $app->execute($dataModel->getDao());
        $app->run();
    }

    protected function createProcess(Master_Box_Models_App_Controller $controller, Master_Box_Models_App_Hermes_DataModel $dataModel, Master_Box_Models_App_Models_Create $app) {
        $msg = "Invalid Request";
        $success = FALSE;
        ...
        if ($validator->getStatus()) {
            ...
            if ($payment instanceof Master_Box_Models_App_Hermes_HasDetail) {
                $appData = $app->getData();
    ...
    public function doCreate(Master_Box_Models_App_Controller $controller) {
        $dataModel = NULL;
        ...
        $app = new Master_Box_Models_App_Models_Create($controller, $this->getSampleData("create_object"));
        $this->data = $app->getData();
        $msg = "Invalid Request";
        $success = FALSE;
        if ($dataModel instanceof Master_Box_Models_App_Hermes_DataModel) {
    ...
    protected function createDocNumber($payment) {
        ...
        if ($payment instanceof Master_Box_Models_App_Hermes_Nomorable) {
            $payment->setNomor(Master_Box_Models_App_DocPrefixGenerator::get($payment->getPrefixNumber()));
        }
    }
