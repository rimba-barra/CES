...
class Gl_Models_App_Tools {
    ...
    public static function enchantDelimiter(Gl_Box_Delien_DelimiterCandidate $candidate) {
        $de = new Hrd_Models_App_HrdDelimiterEnhancer();
        $de->setDelimiterCandidate($candidate);
        $de->generate();
    }

    public final static function setArrayTable($data, Gl_Box_Models_ObjectEmbedData $object) {
        $object->setArrayTable($data);
        if ($object instanceof Gl_Box_Kouti_Remora) {
    ...
    public static function checkStatus(Hrd_Models_Master_Status $status, Hrd_Models_Master_StatusInformation $information) {
        ...
        $msg = "";
        if ($status->getId() == Gl_Box_Config::STATUS_PERMANENT) {
        ...
        } else if ($status->getId() == Gl_Box_Config::STATUS_CONTRACT || $status->getId() == Gl_Box_Config::STATUS_CANDIDATE) {
        ...
        } else if ($status->getId() == Gl_Box_Config::STATUS_DAILY_CONTRACT || $status->getId() == Gl_Box_Config::STATUS_DAILY_TEMPORARY) {
        ...
        } else if ($status->getId() == Gl_Box_Config::STATUS_DAILY_PERMANENT) {
