...
/**
 * Description of ReportReadWorms
 *
 * @author MIS
 */
class Gl_Models_App_Box_ReportReadWorms extends Gl_Box_Models_App_Models_ReadWorms {
    ...
    public function prosesObjects($name, $dataDao) {
        ...
        foreach ($dataDao as $object) {
            ...
            if (is_array($object)) {
                if (count($object) > 0) {
                    if ($object[0] instanceof Gl_Box_Models_ObjectEmbedData) {
    ...
    private function getCleanPrefix(Gl_Box_Models_ObjectEmbedData $object) {
        $prefix = str_replace("_", "", $object->getEmbedPrefix());
        return $prefix;
    }

    private function getFilledModel(Gl_Box_Models_ObjectEmbedData $object, $prefix) {
        $modelAr = array();
