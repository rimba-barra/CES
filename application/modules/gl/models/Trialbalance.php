...
class Gl_Models_Trialbalance extends Zend_Db_Table_Abstract {
    ...
    function generateExcel($param) {
        ...
        $dataproject = $this->execSP3("sp_project_byid_read", array($project_id));
        ...
        $filename = "report_TB_".$this->cleanString($ptname).'_'.$param['untildate'].".xlsx";
        $json = [];
        for ($i = 0; $i < $numberofsheet; $i++) {
            $lib->setSheetNumber($i);
            $lib->setTitleSheet($sheetlabel[$i]);

            $lib->setHeader(array($ptname." - ".$projectname, 'TRIAL BALANCE STATEMENT', '', 'PERIOD : '.$param['fromdate'].' UNTIL '.$param['untildate'], 'COA : '.$param['coastart_id'].' - '.$param['coaend_id']));
            ...
            if ($i == 0) {
                ...
                $lib->setSP("cashier.dbo.sp_reporttrialbalance");
                $lib->setSPParam(array(
                    $param['pt_id'],$param['coastart_id'], $param['coaend_id'], $param['fromdate'], 
                    $param['untildate'], $param['headerdata'], 2,1,$project_id,$param['mergebank']
                ));

                $lib->setColumnTitle(array('Account No.', 'Description', 'Begining Balance', 'Debit', 'Credit', 'Ending Balance'));
                ...
                $footer = $this->genReportexcelAdditional($param, $sql, '', '', '', $paramdata, $col,  '', FALSE); 
                $lib->setFooter($footer[0]);

                $lib->setSP("cashier.dbo.sp_reporttrialbalance");
                $lib->setSPParam(array(
                    $param['pt_id'],$param['coastart_id'], $param['coaend_id'], $param['fromdate'], 
                    $param['untildate'], $param['headerdata'], 1,1,$project_id,$param['mergebank'],'excel'
                ));

                $lib->setColumnTitle(array('Account No.', 'Voucher No.', 'Voucher Date', 'COA Name', 'Description', 'Begining Balance', 'Debit', 'Credit', 'Ending Balance'));

                $lib->setConfig('coa', 20);
                $lib->setConfig('voucher_no', 25);
                $lib->setConfig('voucher_date', 25);
                ...
                $lib->setSPParam(array(
                    $param['pt_id'],$param['coastart_id'], $param['coaend_id'], $param['fromdate'], 
                    $param['untildate'],$param['headerdata'], 1,1,$project_id,$param['mergebank']
                ));

                $lib->setColumnTitle(array('Voucher No.', 'Voucher Date', 'Description', 'Debit', 'Credit'));
                ...
                $lib->setCondition('voucher_no', '==', '', true);
        ...
        $base_64_file = 'base64_trialbalance_'.date('YmdHis').'.txt';
