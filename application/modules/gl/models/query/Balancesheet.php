...
class Gl_Models_Query_Balancesheet extends Zend_Db_Table_Abstract {
    ...
    function init() {
        ...
        $this->_m_coa = 'm_coa';
        $this->_m_summry = 'th_summary';
        $this->_tmp_rptbalancesheet = 'tmp_rptbalancesheet';
        //end table from db
        //start create temporary for report 
        $this->_tmp_parent = '##tmp_parent' . $this->_user_id;
        $this->_tmp_union = '##tmp_unionsum' . $this->_user_id;
        $this->_tmp_rpt = '##tmp_balance_' . $this->_user_id;
    ...
    public function count_tmp_rpt() {
        $sql = "SELECT COUNT(rpt_id) as counterdata FROM $this->_tmp_rptbalancesheet";
        $result = $this->_model->customefromquery($sql);
    ...
    public function truncate_tmp() {
        $sql = "TRUNCATE TABLE $this->_tmp_rptbalancesheet ";
        $this->_model->customefromquery($sql);
    }

    public function delete_tmp() {
        $sql = "DELETE FROM $this->_tmp_rptbalancesheet 
                WHERE
                project_id = $this->_project_id 
                AND pt_id =$this->_pt_id 
                AND user_id = $this->_user_id
    ...
    function insert_to_tmp($record) {
        ...
        $sql = "
                 INSERT INTO $this->_tmp_rptbalancesheet ($key) VALUES ($values)  
               ";
    ...
    function update_tmp_reportbyid($report_id, $record) {
        ...
        $data = $result['setdata'];
        $sql = "
                 UPDATE $this->_tmp_rptbalancesheet SET $data
                 WHERE 
                    project_id = $this->_project_id 
                    AND pt_id =$this->_pt_id 
                    AND user_id = $this->_user_id
    ...
    function update_tmp_amounttotal($coa, $record) {
        $result = $this->_model->extract_array($record);
        $data = $result['setdata'];
        $sql = "
                 UPDATE $this->_tmp_rptbalancesheet SET $data
        ...
        $sql = "
                IF OBJECT_ID(''tempdb..$this->_tmp_parent'') IS NOT NULL DROP TABLE $this->_tmp_parent
                    ...
                    (  
                        ...
                        FROM $this->_tmp_rptbalancesheet a
...
//                        FROM $this->_tmp_rptbalancesheet 
//                        WHERE                      
//                            project_id = $this->_project_id
//                        AND pt_id = $this->_pt_id                      
//                        AND user_id = $this->_user_id              
...
//        } else {
    ...
    function get_amount_from_summary($fromdate,$untildate){ 
        $sql = "SELECT                 
                 a.coa,
                 coalesce(SUM(a.net_summary),0) AS net_total               
                 FROM $this->_m_summry a
                 INNER JOIN $this->_tmp_rptbalancesheet b ON a.coa = b.coa     
                 WHERE
                         a.project_id=$this->_project_id
                   AND a.pt_id=$this->_pt_id 
                   AND a.voucher_date BETWEEN ''$fromdate'' and ''$untildate''  
                   AND b.flag=''T''
                   AND b.user_id = $this->_user_id
                   AND a.deleted=0                       
                 GROUP BY a.coa                        
                 ";   
        ...
        $sql = "
                 IF OBJECT_ID(''tempdb..$this->_tmp_rpt'') IS NOT NULL DROP TABLE $this->_tmp_rpt
                    ...
                    (                    
                        SELECT 
                        *,
                         format(coalesce(lastmonth_amount,0), ''#,##0.00'') AS format_lastmonth_amount,
                         format(coalesce(thismonth_amount,0), ''#,##0.00'') AS format_thismonth_amount,
                         format(coalesce(budget_amount,0), ''#,##0.00'') AS format_budget_amount,
                         format(coalesce(lastyear_amount,0), ''#,##0.00'') AS format_lastyear_amount,
                         ...
                         format(coalesce(total_lastmonth,0), ''#,##0.00'') AS format_total_lastmonth,
                         format(coalesce(total_thismonth,0), ''#,##0.00'') AS format_total_thismonth,
                         format(coalesce(total_budget,0), ''#,##0.00'') AS format_total_budget,
                         format(coalesce(total_lastyear,0), ''#,##0.00'') AS format_total_lastyear,
                         format(coalesce(grand_lastmonth,0), ''#,##0.00'') AS format_grand_lastmonth,
                         format(coalesce(grand_thismonth,0), ''#,##0.00'') AS format_grand_thismonth,
                         format(coalesce(grand_budget,0), ''#,##0.00'') AS format_grand_budget,
                         format(coalesce(grand_lastyear,0), ''#,##0.00'') AS format_grand_lastyear
                        FROM $this->_tmp_rptbalancesheet a                            
                        WHERE 
                             a.project_id=$this->_project_id
                            AND a.pt_id=$this->_pt_id                                                    
