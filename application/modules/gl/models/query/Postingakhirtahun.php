...
class Gl_Models_Query_Postingakhirtahun extends Zend_Db_Table_Abstract {
    ...
    public function get_latestlevel($project_id, $pt_id) {
        $sql = "
                select 
                top 1 * 
                ...
                AND pt_id = $pt_id                
                order by report_level desc
                ";
        $result = $this->_model->customefromquery($sql);
        //print_r($result);
    }

    public function getalldata_rptformat($project_id, $pt_id) {
        $report = 'N';
        $level = '20';
        $sql = "
                        SELECT *,
    ...
    public function getalldata_rptformat_byyear($year, $project_id, $pt_id) {
        $report = 'N';
        $dbgl = "gl_" . $year . ".dbo";
        $sql = "
                        SELECT *,
                                 REPLACE(name, ''-'', '' '') as namecoa
                        FROM $dbgl.$this->_m_rptformat 
                        WHERE 
                        active = $this->_active
                        AND deleted = $this->_delete
                        AND project_id = $project_id
                        ...
                        AND report = ''$report''
                        AND report_level = (
                                            SELECT TOP 1 x.report_level FROM $dbgl.$this->_m_rptformat  x
                                                    WHERE 
                                                             active = $this->_active
                                                            AND deleted = $this->_delete
                                                            AND project_id = $project_id
    ...
    function set_parent_byyear($year) {
        $dbgl = "gl_" . $year . ".dbo";
        $sql = "
                IF OBJECT_ID(''tempdb..$this->_tmp_parent'') IS NOT NULL DROP TABLE $this->_tmp_parent
                    ...
                    (  
                        ...
                        FROM $dbgl.$this->_tmp_rptincomestatement a
                        INNER JOIN $dbgl.$this->_m_coa c ON a.coa = c.coa 
                        INNER JOIN (SELECT project_id,pt_id,coa as parent_code FROM $dbgl.$this->_m_coa) b ON a.coa = b.parent_code              
                        WHERE            
                        ...
                        SELECT a.rpt_id,a.coa,a.name,a.flag,a.level,b.parent_code         
                        FROM $dbgl.$this->_tmp_rptincomestatement a
                        INNER JOIN $dbgl.$this->_m_coa b ON a.coa = b.coa              
                        WHERE
    ...
    public function count_tmp_rpt_byyear($year) {
        $dbgl = "gl_" . $year . ".dbo";
        $sql = "SELECT COUNT(rpt_id) as counterdata FROM $dbgl.$this->_tmp_rptincomestatement";
        $result = $this->_model->customefromquery($sql);
        $counter = $result[0][0]['counterdata'];
        return $counter;
    }
    ...
    public function count_tmp_rpt_user_byyear($year) {
        $dbgl = "gl_" . $year . ".dbo";
        $sql = "SELECT COUNT(rpt_id) as counterdata FROM $dbgl.$this->_tmp_rptincomestatement
                WHERE
                project_id = $this->_project_id 
    ...
    function insert_to_tmp_byyear($year,$record) {
        $dbgl = "gl_" . $year . ".dbo";
        $result = $this->_model->extract_array($record);
        $key = $result['key'];
        $values = $result['values'];
        $sql = "
                 INSERT INTO $dbgl.$this->_tmp_rptincomestatement ($key) VALUES ($values)  
               ";
