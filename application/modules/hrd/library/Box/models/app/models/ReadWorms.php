 ...
 * @author MIS
 */
class Box_Models_App_Models_ReadWorms extends Box_Models_App_AbModel {
    ...
    public function __construct($controller, $debug = '') {
        ...
        $this->hasil = new Box_Models_App_HasilReadExtraModel();
        parent::__construct($controller, $debug);
        $this->request = new Box_Models_App_HasilRequestRead($this->getData());
    }
        ...
        if (key_exists($name, $this->dataList)) {
            ...
            if (count($dataDao) > 0) {
                ...
                foreach ($dl->getList() as $row) {
                    /* added 23 Mei */
                    if($row instanceof Box_Models_App_InterInsertable){
    ...
    public function registerDataList($nama, Box_Kouti_AbDataList $dataList) {
    ...
    protected function generateExtJSMOdel() {
        ...
        foreach ($dl as $k => $v) {
            ...
            $groupEmbede = new Box_Models_App_EmbedGroup();
    ...
    public function getMasterData(Box_Models_App_Masterdata_Main $main){
