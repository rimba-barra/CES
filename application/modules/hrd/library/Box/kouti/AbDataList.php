 ...
 * @author MIS
 */
abstract class Box_Kouti_AbDataList {
    ...
    public final function getTotalRow() {
        //return $this->dataDao[0][0]['totalRow'];
        /* start edited by ahmad riadi 05052017 */
        if(!empty($this->dataDao[0])){
            return $this->dataDao[0][0]['totalRow'];
        }else{
            return 0;
        }
         /* end edited by ahmad riadi 05052017 */
    }
    ...
    public final function getList() {
        ...
        if ($data != NULL) {
            foreach ($data as $row) {
                ...
                $converter = new Box_Models_App_Converter($row);
