 ...
 * @author TOMMY-MIS
 */
class Hrd_Models_Training_Trainingattendance_Trainingattendanceprocess extends Box_Models_ObjectEmbedData implements Box_Kouti_Remora,  Box_Arried  {
    ...
    public function __construct() {
        ...
        parent::__construct();
        $this->embedPrefix = "trainingattendanceprocess_";
    }
    ...
    public function setArrayTable($dataArray = NULL) {
        ...
        if(isset ($x['testimonial'])){
           $this->setTestimonial($x['testimonial']); 
        }
        if(isset ($x['suggestion'])){
           $this->setSuggestion($x['suggestion']); 
        }
        if(isset ($x['desc_plan'])){
           $this->setDescPlan($x['desc_plan']); 
        }
        if(isset ($x['time_plan'])){
           $this->setTimePlan($x['time_plan']); 
        }
        if(isset ($x['imp_plan'])){
           $this->setImpPlan($x['imp_plan']); 
        }
        if(isset ($x['nilai'])){
           $this->setNilai($x['nilai']); 
        }
        if(isset ($x['nilai_post'])){
           $this->setNilaiPost($x['nilai_post']); 
        }
    ...
    public function getArrayTable() {
        ...
        if(empty($this->getNilai())){
            $getNilai = '0';
        }else{
            $getNilai = $this->getNilai();
        }

        if(empty($this->getNilaiPost())){
            $getNilaiPost = '0';
        }else{
            $getNilaiPost = $this->getNilaiPost();
        }
        ...
        $x = array(
            ...
            'desc_plan'=>$this->getDescPlan(),
            'time_plan'=>$this->getTimePlan(),
            'imp_plan'=>$this->getImpPlan(),
            'nilai'=>$getNilai,
            'nilai_post'=>$getNilaiPost
        );
    ...
    public function getImpPlan() {
        return $this->imp_plan;
    }
    ...
    public function getNilaiPost() {
        return $this->nilai_post;
    }
    ...
    public function setDescPlan($desc_plan) {
        $this->desc_plan = $desc_plan;
    }

    public function setTimePlan($time_plan) {
        $this->time_plan = $time_plan;
    }

    public function setImpPlan($imp_plan) {
        $this->imp_plan = $imp_plan;
    }
    ...
    public function setNilaiPost($nilai_post) {
        $this->nilai_post = $nilai_post;
    }
