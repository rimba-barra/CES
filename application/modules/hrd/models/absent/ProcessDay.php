 ...
 * @author MIS
 */
class Hrd_Models_Absent_ProcessDay {
    private $msg;
    private $hasil;
    ...
    public function run(Hrd_Models_Absent $absent,  Box_Models_App_Session $ses,  Box_Models_App_HasilRequestRead $request) {
        ...
        if (count($absents) == 0) {
            ...
            foreach ($absents as $rowAbs) {
                foreach ($shifTypes as $rowShift) {
                    if ($rowAbs->getShiftType()->getId() == $rowShift->getId()) {
                        Hrd_Models_Absent_Tools::attachAttendance($rowAbs, $rowShift);
            ...
            $dao = new Hrd_Models_AbsentDao();
            $hasil = $dao->updateAttendance($newAbsent, $ses);
            if ($hasil) {
                $this->hasil = TRUE;
                $this->msg = "Success process day";
            } else {
                $this->hasil = FALSE;
