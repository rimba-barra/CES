 ...
 * @author Ahmad Riadi
 */
class Hrd_Models_Upload_UploadmasterExport extends Zend_Db_Table_Abstract {
...
while ($j < $total_sheet) {
        ...
        $objSheet->getStyle('A1:AW1')->getFont()->setBold(true)->setSize(12);
        ...
        $objSheet->getCell('E1')->setValue('Status Transfer');
        ...
        foreach ($result as $row) {
            ...
            $objSheet->getCell('E' . $i)->setValue(ucfirst($row['status_transfer']));
            ...
            elseif($data['process_api'] == 'position'){
                ...
                $objSheet->getCell('G' . $i)->setValue($row['position_code']);
            }
            elseif($data['process_api'] == 'employee'){
                $objSheet->getCell('F'. $i)->setValue($row['project_name']);
                $objSheet->getCell('G'. $i)->setValue($row['pt_name']);
        ...
        elseif($data['process_api'] == 'employee'){
            $objSheet->getStyle('A1:AX' . $i)->getBorders()->
                getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
            $objSheet->getStyle('A1:AX' . $i)->getBorders()->
                    getOutline()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
            $objSheet->getStyle('A1:AX1')->getBorders()->
                    getBottom()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        }
        else{
        ...
        ob_end_clean();
        $filename = 'exportlog_master_'.$data['process_api'];
