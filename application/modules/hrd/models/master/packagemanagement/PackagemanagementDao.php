...
class Hrd_Models_Master_Packagemanagement_PackagemanagementDao extends Box_Models_App_AbDao implements Box_Models_App_BlackHole {
    ...
    public function getCheckApproval($module_name, $project_id, $pt_id) {
        $hasil = 0;
        $hasil = $this->dbTable->SPExecute('sp_packagemanagement_checkapproval_read', $module_name, $project_id, $pt_id);
        return $hasil;
    }
	public function Insertdetail($project_id, $pt_id, $header_id, $user_id, $record) {
        $hasil = 0;
        $jenisdoc_id = $record['jenisdocument_id'];
        $bobot = $record['bobot'];
        $hasil = $this->dbTable->SPExecute(
                'sp_packagemanagementdetail_create', $project_id, $pt_id, $header_id, $user_id, $jenisdoc_id, $bobot
        );
        return $hasil;
    }
    ...
    public function saveCopy($session, $project_id, $pt_id) {
        $obj = new Hrd_Models_Performancemanagement_Monitoringmatrix();
        $hasil = 0;
        $hasil = $this->dbTable->SPUpdate(
            'sp_packagemanagement_copy', 
            $obj->getUserlogin(), 
