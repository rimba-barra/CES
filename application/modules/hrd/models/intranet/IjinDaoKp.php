 ...
 * @author MIS
 */
class Hrd_Models_Intranet_IjinDaoKp extends Box_Models_App_AbDao implements Box_Models_App_BlackHole {
    ...
    public function sqlQuery($condition, $configintranet) {
        ...
        $sql = "
                    SELECT
                        h.project_id_ces AS project_id_ces,
                        h.pt_id_ces AS pt_id_ces,
                        h.employee_id_ces AS employee_id_ces,
                        a.izin_id,
                        a.project_id, 
                        a.pt_id, 
			    ...
			    GROUP BY xdata.izin_id
                    ...
                    LEFT JOIN $user h ON h.name = a.user_tambah            
                    ";
        */
        $sql = "
                    SELECT
                        ...
                        (
                            SELECT 
                                  GROUP_CONCAT(xdata.employee_id) 
                            FROM td_izinapproval xdata 
			    LEFT JOIN dbmaster.m_employee xemployee ON xdata.employee_id =xemployee.employee_id
                            WHERE
				xdata.izin_id=a.izin_id 
			    ...
			    GROUP BY xdata.izin_id
                    ...
                    ";
        //LEFT JOIN $user h ON h.name = a.user_tambah     echo $sql; exit;
        return $sql;
    }
    ...
    public function SqlCondition($project_id, $pt_id, $condition, $param) {
        ...
        if ($condition == 'default') {
            ...
            if (empty($ijintype) && $filter->ijin != '') {
                $where .= " AND c.izintype like '%".$filter->ijin."%'";
            }
            return $where;
        }
    }
    ...
    public function changeIjinintranettoces($ijintypeintranet) {
        ...
        } else if ($ijintypeintranet == 'gagal_absent_datang') {
            $kodeijinces = "I-GAM";
        } else if ($ijintypeintranet == 'gagal_absent_pulang') {
            $kodeijinces = "I-GAP";
        } else if ($ijintypeintranet == 'sakit_dengan_surat_dokter') {
            $kodeijinces = "S-KD";
        }
        return $kodeijinces;
    }
    ...
    public function changeIjincestointranet($ijintypeces) {
        ...
        // wulan edit 20190206
        } else if ($ijintypeces == 'IJIN LUPA ABSEN PULANG') {
            $ijintype = "lupa_absen_pulang";
        } else if ($ijintypeces == 'IJIN LUPA ABSEN MASUK') {
            $ijintype = "lupa_absen_datang";
        } else if ($ijintypeces == 'IJIN GAGAL ABSEN DATANG') {
            $ijintype = "gagal_absent_datang";
        } else if ($ijintypeces == 'IJIN GAGAL ABSEN PULANG') {
            $ijintype = "gagal_absent_pulang";
        } else if ($ijintypeces == 'SAKIT DENGAN KETERANGAN DOKTER') {
            $ijintype = "sakit_dengan_surat_dokter";
        } else {
            $ijintype = null;
        }
        return $ijintype;
