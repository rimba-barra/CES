 ...
 * @author MIS
 */
class Hrd_Models_Intranet_TukarshiftDao extends Box_Models_App_AbDao implements Box_Models_App_BlackHole {
    ...
    public function sqlQuery($condition, $configintranet) {
        ...
        $sql = "
                    SELECT
                        ...
                        b.izinapproval_id,
                        e.employee_id as employee_id_hod_intranet, 
                    ...
                    FROM $datah a
                    LEFT JOIN $dataapprove b ON b.izin_id = a.izin_id                   
    ...
    public function getAll($project_id, $pt_id, $condition, $limit, $start, $param) {
        if($start == 0){
            $start = 1;
        }
        $hasil = 0;
        if($param == null){
            $hasil = $this->dbTable->SPExecute('sp_tukarshift_read', $project_id, $pt_id, 'default', $limit, $start, '', '1900-01-01', '2999-01-01', null, null, null, null);
            //var_dump($this->dbTable);
        } else {
            if($param['approvefrom'] == ''){
                $param['approvefrom'] = '1900-01-01';
            }
            if($param['approveuntil'] == 0){
                $param['approveuntil'] = '2999-01-01';
            }
  
            $hasil = $this->dbTable->SPExecute('sp_tukarshift_read', $project_id, $pt_id, $condition, $limit, $start, $param['tukarshift_id'], $param['approvefrom'], $param['approveuntil'], $param['status'], $param['deptcode'], $param['hrd_checked'], $param['employee_name']);                
            //var_dump($this->dbTable);
        }
        ...
        if (!empty($hasil[0])) {
            ...
            $mydata = [];
            foreach ($result as $row) {
                $reportoinmultiposition = $this->getreportto_in_multiposition($row['employee_id']);
                $row['reportoinmultiposition'] = $reportoinmultiposition;
                $mydata[] = $row;
                $count++;
            }
            
            $return = array(array(array("totalRow" => $count)), $mydata);
            return $return;
            //return $hasil;
        } else {
    ...
    public function getAll2($project_id, $pt_id, $condition, $configintranet, $limit, $start, $param) {
        $obj = new Hrd_Models_Intranet_Configmysql();
        $config = $obj->getConfigdata($configintranet);
        $mysqlcon = new mysqli(
        ...
        if ($mysqlcon->connect_error) {
            ...
            while ($myrow = $mysqlquery->fetch_array(MYSQLI_ASSOC)) {
                ...
                $ijinceskode = $this->changeTukarshiftintranettoces($myrow['izintype']);
                ...
                $reportoinmultiposition = $this->getreportto_in_multiposition($rowemployee['employee_id']);
    ...
    public function updateTukarshiftIntranet($configintranet, $id, $hrd_comment, $user_id_and_note) {
        $obj = new Hrd_Models_Intranet_Configmysql();
        $config = $obj->getConfigdata($configintranet);
        $mysqlcon = new mysqli(
                $config['host'], $config['user'], $config['password'], $config['database'], $config['port']
    ...
    public function changeTukarshiftintranettoces($ijintypeintranet) {
        if ($ijintypeintranet == 'datang_terlambat') {
            $kodeijinces = "I-ML"; //IZIN MASUK LAMBAT
        } else if ($ijintypeintranet == 'datang_terlambat_lembur') {
            $kodeijinces = "I-MLL"; //IZIN MASUK LAMBAT
    ...
    public function changeTukarshiftcestointranet($ijintypeces) {
    ...
    function Sendemail($configintranet, $data) {
        ...
        $this->updateTukarshiftIntranet($configintranet, $id, $hrd_comment, $user_id);
...
src=https://intranet.ciputragroup.com/production/attachment/thumbnail/ciputra_logo.jpg&h=62&w=62&zc=1" alt="Intranet Request System" />';
                ...
                $mail->setData()->setSubject('[Permohonan Tukar Shift - HRD Comment] No. Permohonan Tukar Shift  #' . $id . ' Tanggal ' . $tanggal);
