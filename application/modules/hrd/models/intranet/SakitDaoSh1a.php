...
/**
 * Description of SakitDao
 *
 * @author MIS
 */
class Hrd_Models_Intranet_SakitDaoSh1a extends Box_Models_App_AbDao implements Box_Models_App_BlackHole {
    ...
    public function sqlQuery($condition, $configintranet) {
        ...
        $datah = $config['database'] . '.th_sakit';
        $dataapprove = $config['database'] . '.td_sakitapproval';
        $m_project = $config['database_master'] . '.m_project';
        $m_type = $config['database'] . '.m_sakittype';
        ...
        $sql = "
                    ...
                    FROM $datah a
                    LEFT JOIN $dataapprove b ON b.sakit_id = a.sakit_id  
                    LEFT JOIN $m_type m ON m.sakittype_id = a.sakittype_id 
                    ...
                    LEFT JOIN $m_project i ON i.project_id = a.project_id
                    ";
    ...
    public function SqlCondition($project_id, $pt_id, $condition, $param) {
        ...
        if ($condition == 'default') {
            ...
            $where .= " AND a.is_deleted = 0";
            $where .= " AND b.is_deleted = 0";
            $where .= " AND a.sakit_id = $id";
            return $where;
        } else {
            $filter = json_decode($param);
            $where .= " AND a.hrd_check = '$filter->hrd_checked'";
            if (!empty($filter->sakit_id)) {
                $where .= " AND a.sakit_id = $filter->sakit_id";
            }
        ...
        if ($mysqlcon->connect_error) {
           ...
           // echo $field . ' ' . $where;
            ...
            while ($myrow = $mysqlquery->fetch_array(MYSQLI_ASSOC)) {
                ...
                $myrow['sakittype_id'] = $myrow['sakittype_id'];
                $myrow['sakittype'] = $myrow['sakittype'];
                $myrow['attachment'] = $myrow['attachment'];

                $myrow['start_date'] = $myrow['start_date'];
                $myrow['end_date'] = $myrow['end_date'];

                $sakitceskode = $this->changeSakitintranettoces($myrow['sakittype']);
                $rowabsenttype = $this->getAbsenttype($sakitceskode);
    ...
    public function updateStatusIntranet($configintranet, $id, $hrd_check, $hrd_comment) {
        ...
        if ($mysqlcon->connect_error) {
            ...
            $date = date('Y-m-d');
            $table = $config['database'] . ".th_sakit";
            if (!empty($hrd_comment)) {
                mysqli_query($mysqlcon, "UPDATE $table SET hrd_check='$hrd_check',hrd_comment='$hrd_comment',comment_date='$date' WHERE sakit_id=$id and is_deleted=0 ");
            } else {
                mysqli_query($mysqlcon, "UPDATE $table SET hrd_check='$hrd_check' WHERE sakit_id=$id and is_deleted=0 ");
            }
            mysqli_close($mysqlcon);
        }
    }
    ...
    public function directDelete(\Box_Models_App_Decan $decan, \Box_Kouti_InterSession $session) {
     ...
     // public function updateSakitIntranet($configintranet, $id, $hrd_comment, $user_id_and_note) {
    // added by michael 17/11/2021
    // comment diatas
    public function updateSakitIntranet($configintranet, $id, $hrd_comment, $user_id_and_note, $sendemail) {
    // end added by michael 17/11/2021
        ...
        if ($mysqlcon->connect_error) {
            ...
            if (!empty($hrd_comment)) {
                // mysqli_query($mysqlcon, "UPDATE $table SET hrd_comment='$hrd_comment',comment_date='$date',user_ubah='$user_id_and_note' WHERE sakit_id=$id and is_deleted=0 ");
                //added by michael 17/11/2021
                //comment diatas
                mysqli_query($mysqlcon, "UPDATE $table SET hrd_comment='$hrd_comment',comment_date='$date',user_ubah='$user_id_and_note', send_email = '$sendemail' WHERE sakit_id=$id and is_deleted=0 ");
                //end added by michael 17/11/2021
            } else {
                mysqli_query($mysqlcon, "UPDATE $table SET user_ubah='$user_id_and_note' WHERE sakit_id=$id and is_deleted=0 ");
            }
            mysqli_close($mysqlcon);
        }
    }
     ...
     function Sendemail($configintranet, $data) {
        ...
        // $this->updateSakitIntranet($configintranet, $id, $hrd_comment, $user_id);

        //added by michael 17/11/2021
        //comment yg diatas
        $this->updateSakitIntranet($configintranet, $id, $hrd_comment, $user_id, $data['sendmail']);
        //end added by michael 17/11/2021
...
src=https://intranet.ciputragroup.com/production/attachment/thumbnail/ciputra_logo.jpg&h=62&w=62&zc=1" alt="Intranet Request System" />';
                ...
                $message .= "<p>Permohonan Ijin Sakit yang dibuat oleh user dari INTRANET REQUEST SYSTEM berikut :</p>";
                ...
                $message .= "Tipe Sakit	: <strong>" . $row['sakittype'] . "</strong><br>";
                $message .= "Dari Tanggal : <strong>" . date('d F Y', strtotime($data['start_date'])) . "</strong><br>";
                $message .= "Sampai Tanggal	: <strong>" . date('d F Y', strtotime($data['end_date'])) . "</strong><br>";
...
//                print_r($email_cc);
                ...
                $mail->setData()->setSubject('[Permohonan Ijin Sakit - HRD Comment] No. Permohonan  #' . $id . ' Tanggal Berangkat ' . date('d F Y', strtotime($data['start_date'])) . ' - Tanggal Kembali ' . date('d F Y', strtotime($data['end_date'])));
  ...
  /* start added by ahmad riadi 02-06-2017 */
   ...
   public function changeSakitintranettoces($sakittypeintranet) {
        if ($sakittypeintranet == 'Sakit dengan keterangan dokter') {
            $kodesakitces = "S-KD"; 
        } else if ($sakittypeintranet == 'Sakit tanpa keterangan dokter') {
            $kodesakitces = "S-TKD"; 
        }
        return $kodesakitces;
    }

    public function getAbsenttype($sakittype) {
        $hasil = 0;
        $hasil = $this->dbTable->SPExecute('sp_absenttype_read', 1, 1, '', $sakittype);
        return $hasil[1][0];
    }
