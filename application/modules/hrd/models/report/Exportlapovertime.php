 ...
 * @author Ahmad Riadi
 */
class Hrd_Models_Report_Exportlapovertime extends Zend_Db_Table_Abstract {
    ...
    function init() {
        ...
        $this->setup->_storeprocedure = 'sp_report_overtime_export';        
    ...
    function exceldata($data) {
        ...
        $objSheet->getCell('B1')->setValue('Uang Makan Extra');
        $objSheet->getCell('C1')->setValue('Nilai Lembur');
        
        $i = 1;
        foreach ($result as $row) {
            $i++;
                
            $objSheet->getCell('A' . $i)->setValue($row['employee_nik']);  
            $objSheet->getCell('B' . $i)->setValue($row['total_makan_extra']);            
            $objSheet->getCell('C' . $i)->setValue($row['total_nilai_lembur']);                
        }
