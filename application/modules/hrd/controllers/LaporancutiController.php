 ...
 * @author MIS
 */
class Hrd_LaporancutiController extends Box_Models_App_Hermes_AbstractController {
    ...
    public function initRead(){
        ...
        foreach ($hasil[1] as $record){
            ...
            $record = preg_replace('/[^A-Za-z0-9\-]/', '', $record);  //edit by wulan sari 20190311, untuk remove special char yang kadang bikin error
        ...
        // added by anas 06112023 | kelompok absensi list
        ...
        $hasil = $dao->getAllWOPL($this->getAppSession(), $data);
    
        $allkelompokabsensi = array();
        foreach ($hasil[1] as $record){    
            $kelompokabsensi = new Hrd_Models_Master_Kelompokabsensi_Kelompokabsensi();
            $kelompokabsensi->setArrayTable($record);
            $allkelompokabsensi[] = $kelompokabsensi;
        }
        //end added by anas  
       ...
       $otherAT = array(array(
        ...
        // updated by anas 06112023 | tambah all kelompok absensi
        $dm->setHasil(array($pt,$project,$allDepartment,$allDivision,$allCategories,$allEmployee,$allStatusAr,$otherAT, $allkelompokabsensi));
