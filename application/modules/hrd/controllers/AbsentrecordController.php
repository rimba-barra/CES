...
class Hrd_AbsentrecordController extends Box_Models_App_Hermes_AbstractController {
                        ...
                        if($check_proses[0][0]['proses'] == 0){
                            ...
                            if($check_leaveentitlements[0][0]['totalRow'] > 0){
                                ...
                                if($check_leaveentitlements[1][0]['description']){
                                    $data['description'] = $check_leaveentitlements[1][0]['description'] .',<br>'. $data['description'];
                                }
                ...
                foreach($check_proses_empcancel[0] as $key => $item){
                    if($item && $item['proses'] == 1){
                        ...
                        if($check_leaveentitlements[0][0]['totalRow'] > 0){
                            ...
                            if($check_leaveentitlements[1][0]['description']){
                                $data['description'] = $check_leaveentitlements[1][0]['description'] .',<br>'. $data['description'].' (Cancel ExtraLeaveId: '. $data['extraleave_id'].') ';
    ...
    public function createreasonRead() {
        ...
        if ($validator->getStatus()) {
            ...
            if (intval($leave->getAbsentType()->getIsCutLeave()) == 1) {
                ...
                if($leave == 'leave_balance_not_enough'){                    
                    $arrayRespon = array(
                        "HASIL" => 0,
                        "MSG" => 'Process Failed. Balance not enough'
                    );
                    return Box_Tools::instantRead($arrayRespon, array());
                }
                //end added by anas
                ...
                if (count($allHc) > 0) {
                    ...
                    //added by anas 08122021
                    //untuk potong cuti kompensasi extra leave
                    if($leaveAfterProcess->getAbsentType()->getCode() == "C-EXT")
                    {
                        $hasilSave = $lDao->saveKompensansiEL($leaveAfterProcess, $processor->getDates(), $processor->getHasilJatahCuti(), $processor->getJatahCutiKaryawan());                        
                    }
                    else
                    {
            ...
            if (isset($dt['start_time']) && ($leave->getAbsentType()->getCode() == "I-LAM" || $leave->getAbsentType()->getCode() == "I-GAM")) {
                ...
                //added by anas 08122021
                || $leave->getAbsentType()->getCode() == "C-EXT") 
            {                
                $count = count($dates);
                for($i = 0; $i<$count; $i++){
                    if($dates[$i] != ''){                        
                        ...
                        if ($shift->getId() == $date->getShiftType()->getId()) {
                            ...
                            if ($shift instanceof Hrd_Models_Master_ShiftType) {
                                ...
                                if (isset($data['basedata'])) {
                                    ...
                                    } else if($data['basedata'] == 'intranet' && $shift->getIsFlexiKpsh3b() == '1') {
                                        //added by mike 14/04/2023 | krn TLK kan inputnya jam shift in & out, sementara flexi kurang dari 8 atau 9 jam, maka akan terhitung lost time nya, jadi perlu di pas in sesuai totalnya

                                        $explode_totaljam = explode('.', $shift->getTotaljam());
                                        $jam_totaljam = $explode_totaljam[0];
                                        $menit_totaljam = 0;

                                        if($explode_totaljam[1] == '50'){
                                            $menit_totaljam = 30;
                                        }

                                        $setTimeOut = date('H:i:s', strtotime($shift->getInTime().'+'.$jam_totaljam.' hours'));
                                        $setTimeOut = date('H:i:s', strtotime($setTimeOut.'+'.$menit_totaljam.' minutes'));

                                        $date->setTimeIn($shift->getInTime());
                                        $date->setTimeOut($setTimeOut);
                                        $mp->process($date);
                                    } else {
                                        $date->setTimeIn($shift->getInTime());
                                        $date->setTimeOut($shift->getOutTime());
    ...
    public function transferinfoRead() {
        ...
        $res = $dao_projectpt->setTransferAbsent($appData);
        ...
        $errorMsg = $importer->getErrorMessage();
        $arrayRespon = array("STATUS" => $hasil ? TRUE : FALSE, "FPNUMBERLIST" => $newDataFix, "ERRORMSG" => $errorMsg);
    ...
    public function batch_updateRead() {  
        ...
        //added by anas 08122021
        if($project_id == 4049 || $project_id == 5096){ // CitraDream atau Project dummy
            $dao_absent = new Hrd_Models_AbsentDao();
            $dao_absent->updateextraleave($lates_month, $lates_year, $this->getAppSession());            
        }        
    ...
    */
    ...
    public function attachfingerprintinfokpRead() {
        $data = $this->getAppData();
        $ch = curl_init();

        curl_setopt($ch, CURLOPT_URL, "https://api.ciputragroup.com/proses_absensi/proses_absensi_kp_transfer.php?month=".$data["month"]."&year=".$data["year"]);
            ...
            foreach ($allAbsentFingerPrint[0] as $row) {
                ...
                if (count($allFingerPrint[0]) > 0) {
                    foreach ($allFingerPrint[0] as $fpRow) {
                        ...
                        // edited by michael 07/04/2022
                        // if ($row["employee_fingerprintcode"] > 0) {
                        if ($row["employee_fingerprintcode"]) {
    ...
    */
    ...
    public function uploadRead() {
        ...
        if ($modeUpload == "csv") {
            //update by michael 2022-08-04, tambahin . "_" . time(), agar tidak nimpa data sebelumnya
            $fileUpload = new Box_Models_App_FileUpload("/" . Box_Config::SHIFTTYPE_EXCEL_PATH, "absent_" . $ses->getProject()->getId() . "_" . $ses->getPt()->getId(). "_" . time(), "csv");
        } else if ($modeUpload == "excel") {
            //update by michael 2022-08-04, tambahin . "_" . time(), agar tidak nimpa data sebelumnya
            $fileUpload = new Box_Models_App_FileUpload("/" . Box_Config::SHIFTTYPE_EXCEL_PATH, "absentshift_" . $ses->getProject()->getId() . "_" . $ses->getPt()->getId(). "_" . time(), "xlsx");
        } else if ($modeUpload == "dokumen") { // added by Michael 2021.06.15 
        ...
        } else if ($modeUpload == "excel2") {
            $fileUpload = new Box_Models_App_FileUpload("/" . Box_Config::ABSENT_CSV_PATH, "transfer_" . $ses->getProject()->getId() . "_" . $ses->getPt()->getId(). "_" . time(), $data["fileType"]);
        }
    ...
    public function getdatacutiintranetRead() {
        ...
        if (strtolower($checkSH['subholding_subname']) == 'sh1a') {
            $dao = new Hrd_Models_Intranet_CutiDaoSh1a();
                ...
                strtolower($checkSH['subholding_subname']) == 'art') {
    ...
    public function cutidetailintranetRead() {
        ...
        foreach ($param as $rowh) {
            ...
            foreach ($resultdetail[1] as $rowd) {
                ...
                //added by anas 25102023 | untuk menampilkan data name & nik
                $employee_name = $rowh["name"];
                $employee_nik = $rowh["nik"];
                //end added by anas

                //updated by anas 25102023 | add enddate
                $rowabsentdetail = $dao->getAbsentdetail_byempstartdate($employee_id_ces, $startdate, $enddate);
                if (empty($rowabsentdetail)) {
                    $absentdetail_id = 0;
                } else {
                    //updated by anas 25102023 | karena arraynya dilempar beda
                    $absentdetail_id = $rowabsentdetail[0][0]['absentdetail_id'];
                }
        ...
        if ($isValid) {
            ...
            foreach($employeeIds as $key => $item){
                if($item){
                    ...
                    $temp_item_leaveentitlements = null;
                    ...
                    if($check_proses[0] && array_key_exists("0",$check_proses[0])){
                        if($check_proses[0][0]['proses'] == 0){
                            ...
                            $item_leaveentitlements_choose = null;
                        ...
                        if($get_sanksiketerlambatan_leaveentitlements_use[0][0]['totalRow'] > 0){
                            foreach($get_sanksiketerlambatan_leaveentitlements_use[1] as $key_item => $item_item){
                                ...
                                if($check_leaveentitlements[0][0]['totalRow'] > 0){
                                    ...
                                    if($check_leaveentitlements[1][0]['description']){
                                        $data['description'] = $check_leaveentitlements[1][0]['description'] .',<br>'. $data['description'].' (Cancel SanksiKeterlambatanId: '. $data['sanksiketerlambatan_id'].') ';
    ...
    public function generalparametergeteditableRead() {
        $postdata = $this->getAppData();
        $dao = new Hrd_Models_AbsentDao();

        $hasil = $dao->getGeneralparametergeteditable($this->getAppSession(), $postdata);

        $arrayRespon = array("HASIL" => $hasil,"MSG" => $hasil['message']);
        return Box_Tools::instantRead($arrayRespon);
    }

    public function getshifttimeRead() {
        $postdata = $this->getAppData();
        $dao = new Hrd_Models_AbsentDao();

        $hasil = $dao->getShiftTime($this->getAppSession(), $postdata);

        $arrayRespon = array("HASIL" => $hasil,"MSG" => $hasil['message']);
        return Box_Tools::instantRead($arrayRespon);
    }

    public function gettimeinlogRead() {
        $postdata   = $this->getAppData();
        $dm         = new Box_Models_App_Hermes_DataModel();
        $dataList   = new Box_Models_App_DataListCreator('', 'timelog', array(), array());
        $dao        = new Hrd_Models_AbsentDao();

        if($postdata['project_id'] == '4065' && $postdata['pt_id'] == '4223'){
            $hasil       = $dao->getTempAll($this->getAppSession(), $postdata);
    ...
    public function correctionabsentRead(){
        $hasil = FALSE;
        $msg   = "Proses...";
        ...
        $hasil = $dao->correctionAbsent($this->getAppSession(), $data);

        //added by anas 15032024 | untuk menghitung losttime
        if (array_key_exists("projectptid_opsi",$data) && $data['projectptid_opsi']){
        ...
        $absentDetail = new Hrd_Models_Master_General_Date();
        $data["time_in"] = $data["timein_id"];
        $data["time_out"] = $data["timeout_id"];
        $absentDetail->setArrayTable($data);
        ...
        $mp = new Hrd_Models_Absent_MasterProcess($this->getAppSession(),$data);
        $mp->process($absentDetail); //menghitung lost time

        $hasil2 = $dao->updateTime_($absentDetail);
        //end added by anas

        $arrayRespon = array("HASIL" => $hasil,"MSG" => ($hasil) ? 'Berhasil mengganti absen': 'Gagal mengganti absen');
        return Box_Tools::instantRead($arrayRespon);
    }

    public function getlogabsentdetailRead(){
        $postdata    = $this->getAppData();
        $dm = new Box_Models_App_Hermes_DataModel();
        $dataList = new Box_Models_App_DataListCreator('', 'logabsentdetail', array(), array());

        $dao         = new Hrd_Models_AbsentDao();
        $hasil       = $dao->getLogAbsentdetail($this->getAppSession(), $postdata);
    ...
    public function readinfoexcelRead() {
        $hasil      = FALSE;
        $errorMsg   = "...";
        $isDelete   = FALSE;
        $request    = $this->getAppData();
        ...
        $data       = new Hrd_Models_Absent_Data_Excel2($appData["file_name"], $this->getAppSession(), $appData);
        ...
        $errorMsg = $importer->getErrorMessage();
        $errorCells = $data->getErrorCells();
        $arrayRespon = array("STATUS" => $hasil ? TRUE : FALSE, "FPNUMBERLIST" => $newDataFix, "ERRORMSG" => $errorMsg, "ERROR" => $errorCells);
        return Box_Tools::instantRead($arrayRespon);
    }

    public function getprevmonthRead(){
        $postdata = $this->getAppData();
        $dao = new Hrd_Models_AbsentDao();

        $hasil = $dao->getPrevNextMonths($this->getAppSession(), $postdata);
