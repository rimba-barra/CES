...
class Erems_Box_Models_Dbtable_Db extends Zend_Db_Table_Abstract {
    ...
    public function __construct($config = array()) {
        $this->_schema = Erems_Box_Config::DB_NAME;
    ...
    private function logrunSp() {
        $file = 'erems_runsp_log.txt';
        $file = APPLICATION_PATH . '/../public/app/erems/log/' . $file;
        $current = file_get_contents($file);
        $current .= "[" . date("d-m-Y H:i:s") . "][SQL] " . $this->preparedSql . "\r\n";
        file_put_contents($file, $current);
    ...
    public function execSP_3($spname = '', $args = ''){
        return $this->execSP3($spname, $args);
    }
    ...
    protected function runSP($converParams, $execute = FALSE) {
        ...
        if (is_array($converParams)) {
            ...
            Main_Box_GlobalParams::$EXEC_SP[] = $this->preparedSql; ////// Add by Erwin.St 020822 for debug
    ...
    public function spToQuery2($args){
        ...
        return $this->execSP3($args['sp_name'], $args['params']);
    }
