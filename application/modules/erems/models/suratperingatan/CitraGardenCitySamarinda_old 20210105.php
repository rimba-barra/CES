 ...
 * @author David-MIS
 */
class Erems_Models_Suratperingatan_CitraGardenCitySamarinda {
    ...
    public function getListTagihanHTML($params){
        ...
        foreach($schedule as $row){
            ...
            $table .='<tr>
                        ...
                        <td>Rp. '.number_format($row["remaining_balance"]).'</td>
                        <!--<td>'.$row["hari_terlambatb"].'</td><td>'.$row["sp_no"].'</td>
                        <td>Rp. '.number_format($denda).'</td>
                        <td>Rp. '.number_format($row["remaining_balance"]+$denda).'</td>--> 
                    </tr>';
        }

        $table .='<tr><td>&nbsp</td><td>&nbsp</td><td>Total </td><td>Rp. '.number_format($ttlremaining_balance).'</td>
                <!--<td></td><td></td> <td width="150px">Rp. '.number_format($ttldenda_terlambatb).'</td><td width="150px">Rp. '.number_format($ttlremaining_balance+$ttldenda_terlambatb).'--></td></tr>';
		$table .='<tr><td><b>Terbilang : '.$data['total_tunggakan_text'].'</b></td></tr>';
    ...
    public function getHTML($spke=null){
        ...
        '<style>
                ...
                <br/>
                <br/>
				<p class="spstyle_p">Samarinda, <b>{{tanggal_print_w}}</b><br/>
                <p class="spstyle_p">No : <b>{{max_sp_no}}</b><br/>
					Lamp : Poto Rumah Blok {{unit_number}}<br/>
                    Hal : Peringatan ke <b>{{max_sp}}</b></p>
            ...
            $tpl2 = 
                'Menindaklanjuti <b>Surat Peringatan {{sp_ke_before}}</b> kami tanggal {{sp_date_before}} No. {{sp_no_before}}. ';

            $tpl3 = 
                'Bersama ini kami beritahukan kepada Bapak/Ibu selaku pembeli rumah Blok <b> {{unit_number}} </b> {{cluster}}, bahwa berdasarkan catatan kami, Bapak/Ibu sampai dengan tanggal <b>{{tanggal_print_w}}</b> belum melaksanakan pembayaran angsuran rumah yang telah jatuh tempo, dengan rincian sebagai berikut (dalam rupiah):
                </div>
                <br/>
                ...
                <table style="border:none;">
                    <tr style="border:none;">
                        ...
                        <td align="justify" style="border:none;">
                            Transfer ke <b>CITRA SUKSES JO  a/c. 148.0099.626.999</b>  Bank Mandiri Cabang <b>Jend.Sudirman Samarinda</b>. Mohon mencantumkan blok, no. rumah, dan nama pemesan pada kolom berita di slip transfer (bukti transfer mohon email ke <b>zainal.mustofa@ciputra.co.id</b>) atau <b>WA ke 0811556136</b>
                        </td>
                    </tr>
                    <tr>
                        <td valign="top" style="border:none;" width="1" align="left">b.</td>
                        <td align="justify" style="border:none;">
                            Bukti Transfer/Pembayaran dapat ditukar Kwitansi 1 Minggu dari Tanggal Transaksi ke kasir <b>Citra Sukses JO</b> Jln. DI.Panjaitan, Komplek CitraLand City Samarinda, Marketing Gallery Ruko Golden Boulevard No.1-3 No Telpon (0541) 7283388, 7283488.
                        </td>

                    </tr>
                </table>
                <br/>
                Kami memberikan waktu sampai dengan tanggal <b>{{max_date_w}}</b> untuk penyelesaian pembayaran diatas. Untuk tindak lanjut penyelesaian dapat menghubungi petugas collection kami, sdr. <b>Zainal Mustofa</b> di no. telp. <b>(0541) 7283388,7283488.</b>
                 <br/><br/>
