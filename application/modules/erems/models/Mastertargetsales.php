...
class Erems_Models_Mastertargetsales extends Zend_Db_Table_Abstract {

	protected $_schema       = Erems_Box_Config::DB_NAME;
	protected $_bulan        = 'm_targetsales';
	...
	function init() {
        ...
        if(Zend_Registry::isRegistered('config_eremsdb')){
	        $this->_db = Zend_Registry::get('config_eremsdb');
        }
	}

	function dataRead($param = array(), $export = null) {
		if (is_array($param) && count($param)) {
			$spbulan = 'sp_targetsales_read';
			try {
				if ($export == 1) {
					...
					$result = $this->execSP3($spbulan, isset($searchParam['targetsales_id']) ?: 0, $searchParam['tahun'], $param['page'], $param['limit'], $searchParam['view_grid_param'], $this->session->getCurrentPtId());
				}
				$idx = 0;
				$resultTambahan = [];
				foreach ($result[1] as $r) {
					...
					foreach ($r as $field => $val) {
						if (!isset($resultTambahan[$field])) {
							...
							if (in_array($field, ['targetsales_id', 'tahun', 'bulan'])) {
	...
	function dataUpdate($param = array()) {
		if (is_array($param) && count($param)) {
			...
			$spbulan = 'sp_targetsales_update';
			$spparamkey = array(
				$postData['targetsales_id'],
				$fieldName,
				$fieldValue,
...
//				echo $e;
	...
	function dataDelete($param = array()) {
		if (is_array($param) && count($param)) {
			$spbulan = 'sp_targetsales_destroy';
			$spparamkey = array('data' => array('targetsales_id'));
			try {
	...
	function generateTargetRead($param = array()) {
		if (is_array($param) && count($param)) {
			...
			$sptarget = 'sp_targetsalesgenerate_create';
			$param = (array) json_decode($param['data']);
			for ($i = 1; $i <= 12; $i++) {
				$month = $i;
				$result = $this->execSP3($sptarget, [$param['tahun'], $month, 1, $param['project_id'], $this->session->getCurrentPtId(), $this->session->getUserId()]);
				$this->returned['total'] = $result[0];
				$this->returned['success'] = $result[0] > 0;
			}
		}
