...
class Erems_Models_Library_JSON2Excel {
	...
	public function process($variables, $jsonFile, $excelFile, $pembagi = 0, $borderRow = 'none') {
		$hasil = FALSE;
						...
						for ($i = "A"; $i <= $kolomAkhir; $i++) {
							...
							if (preg_match('/{(.*?)}/', $tempCell->getValue(), $matches) === 1) {
								...
								if ($tempFieldSum == NULL && strlen($matches[1]) > 0) {
									$tempFieldSum = Erems_Models_Library_JSON2Excel::$OPERATORTOTAL_TEXT;
				...
				if (count($json) > 0) {
					foreach ($json as $k => $v) {
						...
						foreach ($this->groupingFields as $k2 => $field2) {
							...
							if ($field2["TYPE"] == Erems_Models_Library_JSON2Excel::$OPERATORTOTAL_COUNT) {
								if (isset($v->$field2["FIELD"])) {
									$this->groupingFields[$k2]["VALUE"] ++; // reset
								}
							} else if ($field2["TYPE"] == Erems_Models_Library_JSON2Excel::$OPERATORTOTAL_SUM) {
...
//   $start = $row;
		...
		if (is_array($pageFooterGroupingFields) && count($pageFooterGroupingFields) > 0) {
			...
			foreach ($pageFooterGroupingFields as $gpk => $gpv) {
				if ($gpv["TYPE"] == Erems_Models_Library_JSON2Excel::$OPERATORTOTAL_TEXT) {
	...
	private function cetakGroupFooter($objPHPExcel, &$dataFooter, $row, $record, $kolomAkhir) {
		foreach ($dataFooter as $gpk => $gpv) {
			...
			if ($gpv["TYPE"] == Erems_Models_Library_JSON2Excel::$OPERATORTOTAL_COUNT || $gpv["TYPE"] == Erems_Models_Library_JSON2Excel::$OPERATORTOTAL_SUM) {
