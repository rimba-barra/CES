...
class Erems_Models_Purchaseletter_ExportExcelSiapKomisi {
	...
	public function process($appData) {
		...
		foreach ($appData as $row) {
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('L' . $count, ($row["Addon"] != "" ? date("d-m-Y", strtotime($row["Addon"])) : ""));
			$count++;
		}

		$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
		$this->fileResult = 'List_popup_siap_komisi_' . time() . '.xlsx';
