...
class Erems_Models_Report_ReportAgingesrow extends Zend_Db_Table_Abstract {

    protected $_schema = 'erems';

    function init() {
        $this->setting = new Erems_Models_General_Setup;
        $this->destination = getcwd() . '/app/erems/downloadfile/report/agingesrow/';
        $this->httpdirect = 'app/erems/downloadfile/report/agingesrow/';
    }
            ...
            try {
                ...
                switch ($param['mode_read']) {
                    case 'exportexcel_detail':
                        $result = $this->execSP3('sp_reportcollagingescrowdetail_read', $paramsp);
    ...
    function createexcelDetail($paramdata) {
        ...
        // rename the sheet
        $objSheet->setTitle('agingesrowdetail');
        ...
        $objSheet->getCell('G1')->setValue('Tipe Rumah');
        ...
        ob_end_clean();
        $filedata = $this->destination . "agingesrowdetail" . date('Ymd_Hi') . "_" . $this->setting->_user_id . ".xlsx";
        $directfile = $this->httpdirect . "agingesrowdetail" . date('Ymd_Hi') . "_" . $this->setting->_user_id . ".xlsx";
    ...
    function createexcelRekap($paramdata) {
        ...
        $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
        ob_end_clean();
        $filedata = $this->destination . "agingesrowrekap" . date('Ymd_Hi') . "_" . $this->setting->_user_id . ".xlsx";
        $directfile = $this->httpdirect . "agingesrowrekap" . date('Ymd_Hi') . "_" . $this->setting->_user_id . ".xlsx";
        $objWriter->save($filedata);
        return array("filedata" => $filedata, "direct" => $directfile);
    }
