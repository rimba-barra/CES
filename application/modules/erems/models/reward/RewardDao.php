...
class Erems_Models_Reward_RewardDao extends Erems_Box_Models_App_AbDao {
    ...
    public function getAllWOPLDropdown($projectId,$ptId,$groupId=-1) {
        $res = $this->dbTable->SPExecute('sp_gen_mreward_read_dropdown',$projectId,$ptId,$groupId);
        $hasil = array();
    ...
    public function save(Erems_Models_Reward_Reward $hs) {
        $hasil = 0;
        $hasil = $this->dbTable->SPUpdate('sp_gen_mreward_create', $hs->getAddBy(), $hs->getProject()->getId(), $hs->getPt()->getId(), $hs->getCode(), $hs->getGroup_id(), $hs->getName(), $hs->getActive(), $hs->getGenerate_notes(), $hs->getIs_over_target());
        return $hasil;
    }
    ...
    public function update(Erems_Models_Reward_Reward $hs) {
        ...
        $hasil = $this->dbTable->SPUpdate('sp_gen_mreward_update', $hs->getModiBy(), $hs->getId(), $hs->getCode(), $hs->getGroup_id(), $hs->getName(), $hs->getActive(), $hs->getGenerate_notes(), $hs->getIs_over_target());
        return $hasil;
    }
