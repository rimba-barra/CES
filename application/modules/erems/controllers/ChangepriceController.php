...
class Erems_ChangepriceController extends Erems_Box_Models_App_Hermes_AbstractController {
	...
	public function detailRead() {
		...
		if ($genco->getAucoMode() == 0) {
			$cfgUser = Erems_Box_Projectptconfig_ProjectPtConfigSelector::getAuthorizeUser($projectId, $ptId, "CHANGEPRICE_APPROVEUSER");
			if(is_array($cfgUser)){
				$isAuthorizerUser = in_array($userId, Erems_Box_Projectptconfig_ProjectPtConfigSelector::getAuthorizeUser($projectId, $ptId, "CHANGEPRICE_APPROVEUSER")) ? TRUE : FALSE;
			}
			else{
