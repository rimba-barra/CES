...
class Erems_KartupiutangallreportController extends Zend_Controller_Action {
   ...
   // return (date('d-m-Y', strtotime($x)) == $x);
	...
	function exportAction() {
        ...
        $model = new Erems_Models_Kartupiutangallreport();
		$post_data['cluster_id'] = $this->getRequest()->getPost('cluster_id');
			
		// $result = $model->KartupiutangallreportRead($post_data);
		// $resultCount = $model->kartupiutangallreportReadCount($post_data);
		// if($resultCount['data'][0]['total'] > 10000){
			$pythonResult = $model->kartupiutangallreportPython($post_data);
