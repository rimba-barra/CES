...
class Erems_PurchaseletterController extends Erems_Box_Models_App_Hermes_AbstractController {
	...
	public function detailRead() {
		...
		$salesman->setProject($project);
		$salesman->setPt($pt);
		$hasil = $employee_dao->getAll($salesman);

		$allSalesman = array();
		$this->fillData($hasil[1], $allSalesman, $creator, 'salesman');

		/// collector 
		$collector = new Erems_Models_Sales_Collector();
		$collector->setProject($project);
		$collector->setPt($pt);
		$hasil = $employee_dao->getAll($collector);

		$allCollector = array();
		$this->fillData($hasil[1], $allCollector, $creator, 'collector');

		$masterCC    = new Erems_Models_App_Masterdata_SalesLocation();
		$allSalelLoc = $masterCC->prosesDataWithSession($ses, TRUE);

		$masterMP    = new Erems_Models_App_Masterdata_MediaPromotion();
		$allMediaPro = $masterMP->prosesDataWithSession($ses, TRUE);
		...
		$bR = new Erems_Models_Sales_BillingRules();
		$bR->setProject($project);
		$bR->setPt($pt);
		$allBalloon = $masterBillR->getDao()->getAllBallon($bR);
		$balloon = array();
		$this->fillData($allBalloon[0], $balloon, $creator, 'billingrulesballoon');

		$masterClub = new Erems_Models_App_Masterdata_CitraClub();
		$allClubs   = $masterClub->prosesDataWithSession($ses, TRUE);

		/// MASTER CAC
		// $cacDao     = new Erems_Models_Master_CACDao();
		// $master_cac = new Erems_Models_Master_CAC();
		// $master_cac->setProject($project);
		// $master_cac->setPt($pt);
		// $allCac = $cacDao->getAllWOPL($master_cac);

		// $allCac = Erems_Box_Tools::toObjectResult($allCac, $master_cac);

		/// MASTER UPLINE
		$allMk = $employee_dao->getAllWOPLUpline($projectid, $ptid);
		$allMk = Erems_Box_Tools::toObjectResult($allMk, new Erems_Models_Hrd_Employee());

		$masterCity = new Erems_Models_App_Masterdata_City();
		$allCty     = $masterCity->prosesDataWithSession($ses, TRUE);

		$paramsRequestResult = Erems_Box_Tools::globalParamsExistNew($ses, "PURCHASELETTER");

		$masterSM = new Erems_Models_App_Masterdata_SourceMoney();
		$allSM    = $masterSM->prosesDataWithSession($ses, TRUE);

		$appDao         = new Erems_Models_Master_AppDao();
		$master_pt      = new Erems_Box_Models_Master_Pt();
		$master_project = new Erems_Box_Models_Master_Project();

		if (Erems_Box_Config::IS_PROJECTPT_CONSTANT) {
			$master_pt->setName('CONSTANT_PT');
		} 
		else {
			$ptInfo = $appDao->getPt($ptid);
			$master_pt->setArrayTable($ptInfo[0][0]);
			$projectInfo = $appDao->getProject($projectid);
			$master_project->setArrayTable($projectInfo[0][0]);
		}
		...
		// end payment scheme template
		// cek ada pengalihan hak
		$params = $this->getAppData();
		...
		if (array_key_exists("purchaseletter_id", $params)) {
			...
			if (count($dataChangeName) == 0) {
				...
				$dataPayOther = $payDao->getOtherPaymentByPaymentType($projectid, $ptid, 1, 5, $dataPurchase->getId(), "GANTI NAMA~BIAYA GANTI NAMA~BY GANTI NAMA~ADMINISTRASI GANTI NAMA");
		...
		// end cek ada pengalihan hak
		// master downline
		$generalDao  = new Erems_Models_Master_GeneralDao();
		$allDownline = $generalDao->getAllDownline($projectid, $ptid);
		// end master downline

		$show_reward = $genco->isPurchaseletterShowReward();
		//master reward
		$allRewardSales    = array();
		$allRewardCustomer = array();
		$allRewardTambahan = array();
		// if($show_reward === 2){
			$rewardDao         = new Erems_Models_Reward_RewardDao();
			$allRewardSales    = $rewardDao->getAllWOPL($projectid, $ptid, 0);
			$allRewardSales    = Erems_Box_Tools::toObjectResult($allRewardSales, new Erems_Models_Reward_Reward("rewardsales_"));
			$allRewardCustomer = $rewardDao->getAllWOPL($projectid, $ptid, 2);
			$allRewardCustomer = Erems_Box_Tools::toObjectResult($allRewardCustomer, new Erems_Models_Reward_Reward("rewardcustomer_"));
			$allRewardTambahan = $rewardDao->getAllWOPL($projectid, $ptid, 1);
			$allRewardTambahan = Erems_Box_Tools::toObjectResult($allRewardTambahan, new Erems_Models_Reward_Reward("rewardtambahan_"));
		// }

		//purpose buy
		$purposeBuyDao = new Erems_Models_Master_PurposeBuyDao();
		$allPurposeBuy = $purposeBuyDao->getAllWOPL();
		$allPurposeBuy = Erems_Box_Tools::toObjectResult($allPurposeBuy, new Erems_Models_Reward_Reward("purposebuy_"));

		//addon 2019-01-11
		$isPurchasePrintoutCentralized = $genco->isPurchasePrintoutCentralized();
		...
		$masterSG = new Erems_Models_App_Masterdata_Salesgroup();
		$allSG    = $masterSG->prosesDataWithSession($ses, TRUE);
		...
		$otherAT = array(array(
			...
			"USE_RUBIY_PROLIBS"                   => $genco->useRumusBiayaProlibs(),
			...
			"MASTER_IM"                           => $model_mim->masterimRead(array('limit' => 0, 'page' => 1, 'internalmemo_id' => 0, 'nomor_im' => '')),
		...
		// $dm->setHasil(array($allSalesman, $allCollector, $allSalelLoc, $allMediaPro, $allBank, $allBillR, $allClubs, $allSM, $allCac, $allMk, $allCty, $allRewardSales, $allRewardTambahan, $allRewardCustomer, $allPurposeBuy, $otherAT, $allBalloon, $allSG));
		$dm->setHasil(array($allSalesman, $allCollector, $allSalelLoc, $allMediaPro, $allBank, $allBillR, $allClubs, $allSM, $allMk, $allCty, $allRewardSales, $allRewardTambahan, $allRewardCustomer, $allPurposeBuy, $otherAT, $allBalloon, $allSG));
	...
	public function assetCACRead() {
		...
		$master_cac->setProject($project);
		$master_cac->setPt($pt);
		$allCac = $cacDao->getAllDropdown($master_cac);

		$result = array();
		if(isset($allCac[1]) && count($allCac[1])){
			$result = $allCac[1];
		}

		echo Zend_Json::encode(Erems_Box_Tools::utf8ize(array('data' => $result)));
		exit;
	}
}
?>
