...
class Erems_PopupbelumajbController extends Erems_Models_App_Template_AbstractMasterController {
    ...
    public function _getMainDataModel() {
      ...
      //  $dao->setSession($this->getAppSession());
        ...
        $dm->setDataList(new Erems_Box_Models_App_DataListCreator('', 'purchaseletter', array('clusterb','customerprofile','unit','pricetype','salesman'), array()));
     ...
     public function allRead() {
        ...
        if ($dm instanceof Erems_Box_Models_App_Hermes_DataModel) {
            ...
            $hasil = $dao->getPopubelumajb($this->getAppRequest(),
    ...
    public function exportAction(){
        ...
        $hasil          = $dao->getPopubelumajb($requestRead, $project_id, $pt_id,$this->getRequest()->getPost('customer_name'),$this->getRequest()->getPost('unit_number'));
        ...
        if (count($hasil[1]) > 0) {
            ...
            $objPHPExcel->getActiveSheet()->SetCellValue('E1', 'Salesman');
            $objPHPExcel->getActiveSheet()->SetCellValue('F1', 'Pricetype');
            $objPHPExcel->getActiveSheet()->SetCellValue('G1', 'Handphone');
            $objPHPExcel->getActiveSheet()->SetCellValue('H1', 'Homephone');
            ...
            foreach($hasil[1] as $row){
                ...
                $objPHPExcel->getActiveSheet()->SetCellValue('D'.$count, $row["purchaseletter_no"]);
                $objPHPExcel->getActiveSheet()->SetCellValue('E'.$count, $row["salesman_employee_name"]);
                $objPHPExcel->getActiveSheet()->SetCellValue('F'.$count, $row["pricetype_pricetype"]);
                $objPHPExcel->getActiveSheet()->SetCellValue('G'.$count, $row["customer_mobile_phone"]);
                $objPHPExcel->getActiveSheet()->SetCellValue('H'.$count, $row["customer_home_phone"]);
            ...
            $fileResult = 'List_bisa_ajb_' . time() . '.xlsx';
