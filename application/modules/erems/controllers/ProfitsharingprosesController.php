...
class Erems_ProfitsharingprosesController extends Zend_Controller_Action {
	...
	function readAction() {
		...
		$model = new Erems_Models_Profitsharingproses();
		...
		} else if ($read_type_mode == 'th_profitsharing') {
			$post_data['start'] = $this->getRequest()->getPost('start');
			$post_data['limit'] = $this->getRequest()->getPost('limit');
			$post_data['page'] = $this->getRequest()->getPost('page');

			$result = $model->dataprosesdateRead($post_data);
		} else if ($read_type_mode == 'deletePS') {
			$userDeletePS = Erems_Box_Projectptconfig_ProjectPtConfigSelector::getGeneralConfig($this->session->getCurrentProjectId(), $this->session->getCurrentPtId())->getUserDeletePS();
			
			$result['deletePS'] = in_array($this->session->getUserId(), $userDeletePS) ? 1 : 0;
		} else if ($read_type_mode == 'td_profitsharing_detail') {
			$post_data['profitsharing_id'] = $this->getRequest()->getPost('profitsharing_id');
...
//		 $jsonExcel = new Erems_Models_Library_JsonToExcelWithTemplate();
		...
		if($this->session->getCurrentProjectId() == 62){
			$jsonExcel->fileTemplate = APPLICATION_PATH . '/../public/app/erems/uploads/exceltemplate/ProfitSharingProsesVidaView.xlsx';		
		}else{
			$jsonExcel->fileTemplate = APPLICATION_PATH . '/../public/app/erems/uploads/exceltemplate/ProfitSharingProsesBanjarBaru.xlsx';		
		}
		
		$jsonExcel->fieldAwal = "ID";
	...
	public function printout($post_data) {
		...
		$model = new Erems_Models_Profitsharingproses();
		$param = [
			'profitsharing_id' => $post_data['profitsharing_id'],
...
//		 $jsonExcel = new Erems_Models_Library_JsonToExcelWithTemplate();
		...
		if ($post_data['rg_print'] == "detail_rs") {
			if($this->session->getCurrentProjectId() == 62){
				$jsonExcel->fileTemplate = APPLICATION_PATH . '/../public/app/erems/uploads/exceltemplate/ProfitSharingDetailRSVidaView.xlsx';		
			}else{
				$jsonExcel->fileTemplate = APPLICATION_PATH . '/../public/app/erems/uploads/exceltemplate/ProfitSharingDetailRSBanjarBaru.xlsx';		
			}
			$jsonExcel->fieldAwal = "ID";
			$firstRow = 6;
			if($this->session->getCurrentProjectId() == 62){
			...
			$jsonExcel->fileTemplate = APPLICATION_PATH . '/../public/app/erems/uploads/exceltemplate/ProfitSharingDetailTerjual.xlsx';
			$jsonExcel->fieldAwal = "ID";
			$firstRow = 5;
			$columnSumStart[] = ['O', 'AA'];
			$columnSumStart[] = ['AE', 'AK'];
		} else if ($post_data['rg_print'] == "rekap_terjual") {
			$jsonExcel->fileTemplate = APPLICATION_PATH . '/../public/app/erems/uploads/exceltemplate/ProfitSharingRekapTerjual.xlsx';
			$jsonExcel->fieldAwal = "ID";
			$firstRow = 5;
			$columnSumStart[] = ['D', 'AF'];
		}
...
//			$objPHPExcel->getActiveSheet()->getColumnDimension($columnID)->setAutoSize(true);
		...
		$fileResult = 'ProfitSharingProsesRekapRS_' . time() . '.xlsx';
