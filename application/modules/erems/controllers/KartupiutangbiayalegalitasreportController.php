...
class Erems_KartupiutangbiayalegalitasreportController extends Zend_Controller_Action {
   ...
   // return (date('d-m-Y', strtotime($x)) == $x);
	...
	function exportAction() {
        ...
        $model = new Erems_Models_Kartupiutangbiayalegalitasreport();
		$post_data['cluster_id'] = $this->getRequest()->getPost('cluster_id');
			
		// $result = $model->KartupiutangbiayalegalitasreportRead($post_data);
		// $resultCount = $model->kartupiutangbiayalegalitasreportReadCount($post_data);
		// if($resultCount['data'][0]['total'] > 10000){
			$pythonResult = $model->kartupiutangbiayalegalitasreportPython($post_data);
