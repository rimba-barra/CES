...
class Erems_TablestockController extends Zend_Controller_Action {
    // private $sm;
    ...
    function readAction() {
        ...
        }else if ($post_data['moderead_type'] == 'filetablestock'){
            $result = Erems_Box_Projectptconfig_ProjectPtConfigSelector::getGeneralConfig($this->session->getCurrentProjectId(), $this->session->getCurrentPtId())->getfiletablestock();
        }else if($post_data['moderead_type'] == 'getunit'){
            $model_tablestock = new Erems_Models_Tablestock();

            $post_data['cluster_id'] = $this->getRequest()->getPost('cluster_id');
            $result['unit'] = $model_tablestock->tablestockIndex($post_data);
...
//        $this->getResponse()->setHeader('Content-Type', 'application/json');
        ...
        $result['block'] = $model_tablestock->tablestockIndexBlock($post_data);
        $result['nomor'] = $model_tablestock->tablestockIndexNomor($post_data);
        $result['detail'] = $model_tablestock->tablestockIndex($post_data);
        
//        $this->view->data['block'] = $model_tablestock->tablestockIndexBlock($post_data);
...
//        var_dump($result);        exit();
    ...
    function detailAction() {
        ...
        $result['dtl'] = $model_tablestock->tablestockDetail($post_data);
        $this->view->assign('data', $result);
    }
    ...
    function exportdata($param) {
        $model = new Erems_Models_Tablestock();

        $result['block'] = $model->tablestockIndexBlock($param);
        $result['nomor'] = $model->tablestockIndexNomor($param);
        $result['detail'] = $model->tablestockIndex($param);
    ...
    //                                          var_dump($data);
            ...
            $fileResult = 'Tablestock' . '_' . time() . '.xlsx';
    ...
    function fixtableAction() {
                ...
                $path = array(APPLICATION_PATH,'modules','erems','views', 'scripts','tablestock');
    ...
    function exportdata2($param) {
        $model = new Erems_Models_Tablestock();
        $file = Erems_Box_Projectptconfig_ProjectPtConfigSelector::getGeneralConfig($this->session->getCurrentProjectId(), $this->session->getCurrentPtId())->getfiletablestock();

        // $file = $this->filename;
        // var_dump(dirname(__DIR__)); die();
        require_once dirname(__DIR__) . '\\views\\scripts\\tablestock\\'.$file.'.phtml';
        ...
        $unit_number = implode('', array_filter($unit,function($a){if(ctype_alpha($a)){return $a;}else{return null;}}));
                ...
                foreach ($value as $xx) {
                    ...
                    if ($xx === "0") {
                        ...
                        foreach ($resultdetail as $val) {
                            // added by rico 04042022
                            $unit_x = str_split($xx);
                            $x = implode('', array_filter($unit_x,function($a){if(ctype_alpha($a)){return $a;}else{return null;}}));
