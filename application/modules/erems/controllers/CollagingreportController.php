...
class Erems_CollagingreportController extends Zend_Controller_Action {

    function init() {
        $this->session = Zend_Controller_Action_HelperBroker::getStaticHelper('session');
        $this->_model = new Erems_Models_Report_ReportAging;
    }

    function readAction() {
		$read_type_mode = ($this->getRequest()->getPost('read_type_mode') ? $this->getRequest()->getPost('read_type_mode') : '');
		
		$model = new Erems_Models_Collagingreport();
		
		if($read_type_mode == 'aging_prosesdate'){
			$post_data['aging_type'] = $this->getRequest()->getPost('aging_type');
			
			$return = $model->agingGetProsesDate($post_data);
		} 
		else{
