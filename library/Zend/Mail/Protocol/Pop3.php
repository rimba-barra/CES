...
class Zend_Mail_Protocol_Pop3
{
    ...
    public function connect($host, $port = null, $ssl = false)
    {
        ...
        if ($ssl === 'TLS') {
            $this->request('STLS');
            // $result = stream_socket_enable_crypto($this->_socket, true, STREAM_CRYPTO_METHOD_TLS_CLIENT); -- test ubah metode tls 26/01/2022 krisna
            $result = stream_socket_enable_crypto($this->_socket, true, STREAM_CRYPTO_METHOD_SSLv23_CLIENT);
            if (!$result) {
                /**
                 * @see Zend_Mail_Protocol_Exception
                 */
