...
abstract class Zend_Db_Table_Abstract
{
    ...
    protected function _setup()
    {
        ...
        if ($this->_schema)
        {
            $this->_adaptName = explode('_', get_class($this->_db));
            $this->_adaptName = strtolower($this->_adaptName[count($this->_adaptName)-1]);

            switch($this->_adaptName) {
                case ($this->_adaptName=='mssql' || $this->_adaptName=='sqlsrv'):
                    $this->_schema .= '.dbo';
                    break;
                default:
                    break;
    ...
    public function execSP($args='')
    {
        if ($args)
        {
            ...
            Main_Box_GlobalParams::$EXEC_SP[] = $sql; ////// Add by Erwin.St 020822 for debug

            try { $result = $stmt->fetchAll(); } catch (Exception $e) { $result = $stmt->rowCount(); }
            return $result;
        }
    }
     ...
     * 1900-01-01 00:00 AM
     * OUTPUT PARAMETER still not supported
     * Multi Result Set (1st Edition)
     *
     */
    public function execSP2($args='')
    {
        if ($args)
        {
            $args = func_get_args();
            ...
            try {
                ...
                $result['success'] = $stmt->rowCount()>0;
                $next = $stmt->nextRowset();
                while($next){
                    try { $result['data'] = $stmt->fetchAll(); } catch (Exception $e) { }
                    try { $next = $stmt->nextRowset(); } catch (Exception $e) { $next = false; }
                }
            }
            return $result;
        }
	...
	public function execSP3Prep($spname = '', $paramSql = [], $paramsApps = []) {
		if ($spname) {
			$paramPrep = '';
			...
			switch ($this->_adapterName) {
				...
				case ($this->_adapterName == 'mssql' || $this->_adapterName == 'sqlsrv'):
					$sqlEXEC = 'EXEC ';
					$spcheck = ($this->_schema ? $this->_schema . '.' : '') . "sp_view_parameter_sp ?";
					$sqlcheck = $sqlEXEC . $spcheck;
					$stmt1 = new Zend_Db_Statement_Sqlsrv($this->_db, $sqlcheck);
					$stmt1->execute([$spname]);
					$typeParameter = $stmt1->fetchAll();
					break;
				default:
					return;
					break;
			...
			if (count($paramSql)) {
				foreach ($paramSql as $key => $val) {
					$paramPrep .= $paramPrep != '' ? ',' : '';
					$paramPrep .= '?';

					if (isset($typeParameter[$key]['data_type']) && in_array($typeParameter[$key]['data_type'], ['money', 'int', 'decimal', 'bigint']) && $val == "") {
						$paramSql[$key] = (int) $val;
					} else {
						if (!is_numeric($val)) {
							$paramSql[$key] = (string) $val;
//							$paramSql[$key] = "'" . $val . "'";
						}
					}
					$paramLog = " " . str_replace("%$%", ", ", implode('%$%', $paramSql)) . " ";
				}
			}

			$sql = $sqlEXEC . $spname . ' ' . $paramPrep;
//			echo $sqlEXEC.$spname . ' ' . $paramLog;
...
//				$result[$qryIdx] = $stmt->fetchAll();
				...
				$config["dbname"] = "dbwebsec";
				$this->_db = new Zend_Db_Adapter_Sqlsrv($config);
				try {
					$sqlRunning = base64_encode($sqlEXEC.$spname . ' ' . $paramLog);
					$sql = "IF (NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE	TABLE_NAME = 't_log'))
							BEGIN
								CREATE TABLE [dbo].[t_log] (
										[log_id] int IDENTITY(1,1) NOT NULL ,
										[error_type] varchar(50) COLLATE SQL_Latin1_General_CP1_CI_AS  NULL,
										[apps] varchar(30) COLLATE SQL_Latin1_General_CP1_CI_AS  NULL,
										...
										[error_log] text COLLATE SQL_Latin1_General_CP1_CI_AS  NULL,
										CONSTRAINT PK PRIMARY KEY (log_id)
									)
							END";
					$stmtCreatelog = $this->_db->query($sql);
					$sql2 = "INSERT INTO t_log (error_type, apps,module,user_id,user_name, log_time,query,error_log) VALUES ";
					$sql2 .= "(
								'query',
								'" . $paramsApps['module'] . "',
								'" . $paramsApps['controller'] . "',
								'" . $_SESSION['Ciputra']['common']['user']['user_id'] . "',
								'" . $_SESSION['Ciputra']['common']['user']['user_name'] . "',
								GETDATE(),
								erems.dbo.fnBase64Decode('".$sqlRunning."'),
								erems.dbo.fnBase64Decode('".base64_encode(json_encode($e->getMessage()))."')
							)";
					$this->_db->query($sql2);
				} catch (Exception $e) {}
				throw $e;
	...
	public function execSP3($spname = '', $args = '') {

		$session = Zend_Controller_Action_HelperBroker::getStaticHelper('session');
		$paramsApps = $session->getActionController()->getAllParams();
		if ($spname) {
			$param = array();
			$args = func_get_args();
			if (count($args) > 1) {
			...
			if ($paramsApps['module'] == 'erems' && method_exists($genco_main, 'antiSQLINJECTION') && $genco_main->antiSQLINJECTION() == 1) {
				return $this->execSP3Prep($spname, $param, $paramsApps);
			} else {
				switch ($this->_adapterName) {
					case 'mysql':
						$sql = 'CALL ';
					...
					case ($this->_adapterName == 'mssql' || $this->_adapterName == 'sqlsrv'):
						$sql = 'EXEC ';
						if ($paramsApps['module'] == 'erems') {
							$spcheck = ($this->_schema ? $this->_schema . '.' : '') . "sp_view_parameter_sp ?";
							$sqlcheck = $sql . $spcheck;
							$stmt1 = new Zend_Db_Statement_Sqlsrv($this->_db, $sqlcheck);
							$stmt1->execute([$spname]);
							$typeParameter = $stmt1->fetchAll();
						}
				...
				if (count($param)) {
					foreach ($param as $key => $val) {
						if ($paramsApps['module'] == 'erems') {
							if (isset($typeParameter[$key]['data_type']) && in_array($typeParameter[$key]['data_type'], ['money', 'int', 'decimal', 'bigint']) && $val == "") {
								$param[$key] = (int) $val;
							} else if (isset($typeParameter[$key]['data_type']) && in_array($typeParameter[$key]['data_type'], ['varchar'])) {
								$param[$key] = "'" . $val . "'";
	...
	public function execSP5($spname='', $args='')
    {
        if ($spname)
        {
            $param = array();
            $args = func_get_args();
            if (count($args)>1) { $args = array_slice($args, 1); foreach($args as $val){ if (is_array($val)) { $param = array_merge($param, $val); } else { array_push($param, $val); } } }
            switch ($this->_adapterName)
            {
                case 'mysql':
                    $sql = 'CALL ';
            ...
            return array('result'=>$result,'sql'=>$sql);
        }
    }

    public function execFn($args='')
    {
        if ($args)
        {
            $args = func_get_args();
            $fnname = ($this->_schema ? $this->_schema . '.' : '') . $args[0];
            $param = '';
            $select = '';

            $select = ($args[1]!='') ? $args[1]:'*';
            ...
            $sql = 'SELECT '.$select.' FROM '. $fnname .'('.$param.')';
     ...
     */
     ...
     /*Modi by David P. 2021-03-29*/
    public function generateSPParam(&$spparamkeys = array() , $spparamvalues = array())
    {
        $this->generateSPParamKey($spparamkeys);
        if (is_array($spparamvalues))
        {
            foreach ($spparamvalues as $key => $item)
            {
                if (is_string($item))
                {
                    $spparamvalues[$key] = Zend_Json::decode($item);
                }
            }
            $this->generateSPParamKey($spparamvalues);
            foreach ($spparamkeys as $k => $v)
            {
                if (isset($spparamvalues[$k]) && $spparamvalues[$k])
                {
                    $spparamkeys[$k] = $spparamvalues[$k];
                }
            }
        }
        return $spparamkeys;
    }
    protected function generateSPParamKey(&$spparamkeys, $args = '')
    {
        $phpver = explode('.',phpversion());
        $result = array();
        if (is_array($spparamkeys))
        {
            foreach ($spparamkeys as $k => $v)
            {
                $i = $v;
                ...
                elseif (is_array($i))
                {
                    ...
                    if($phpver[0] > 5){
                        $is_countable = 0;
                        if(isset($i[0]) && is_countable($i[0])){
                            $is_countable = 1;
                        }
                    }

                    if (isset($i[0]) && $is_countable == 1 && count($i[0]) > 1)
                    {
                        $r = array();
                        foreach ($i as $m => $n)
                        {
                            if (is_array($n))
                            {
                                ...
                                foreach ($r as $m => $n)
                                {
                                    if (is_array($n))
                                    {
                                        $r[$m] = implode(isset($this->datadelimiter) ? $this->datadelimiter : '~NGN~', $n);
                                    }
                                }
                            }
                            $i = $r;
                    ...
                    $result = array_merge($result, $this->generateSPParamKey($i, $args . $k . '.'));
                }
            }
        }
        else
    ...
    public function getrawSP($spname='', $args='')
    {
        if ($spname)
        {
            $param = array();
